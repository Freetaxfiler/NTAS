...
class CommonDBTM extends CommonGLPI
{
    ...
    public function restoreInDB()
    {
        ...
        if ($this->maybeDeleted()) {
            ...
            if (isset($this->fields['date_mod'])) {
                $params['date_mod'] = $_SESSION["ntas_currenttime"];
            }
    ...
    public function deleteFromDB($force = false)
    {
        ...
        if (
            ...
            $toadd = [];
            if (isset($this->fields['date_mod'])) {
                $toadd['date_mod'] = $_SESSION["ntas_currenttime"];
            }
                ...
                foreach ($fields as $field) {
                    if (is_array($field)) {
                        ...
                        if (is_a($itemtype, IPAddress::class, true) && in_array('mainitemtype', $field) && in_array('mainitems_id', $field)) {
                            // ntas_ipaddresses relationship that does not respect naming conventions
    ...
    public function cleanRelationTable()
    {
        ...
        if (in_array(static::class, $CFG_GLPI['ticket_types'], true)) {
            ...
            foreach ($iterator as $data) {
                $cnt = countElementsInTable('ntas_items_tickets', ['tickets_id' => $data['tickets_id']]);
    ...
    public function add(array $input, $options = [], $history = true)
    {
        ...
        if ($this->input && is_array($this->input)) {
            ...
            if (isset($table_fields['date_creation']) && !isset($this->input['date_creation'])) {
                $this->fields['date_creation'] = $_SESSION["ntas_currenttime"];
            }

            // Auto set date_mod if exist
            if (isset($table_fields['date_mod']) && !isset($this->input['date_mod'])) {
                $this->fields['date_mod'] = $_SESSION["ntas_currenttime"];
            }
            ...
            $stmt = $DB->prepare(
                $DB->buildInsert(
                    ...
                    [
                        ...
                        'items_id'        => $this->fields['id'],
                        'date_creation'   => $_SESSION["ntas_currenttime"],
    ...
    **/
    ...
    public function canUnrecurs()
    {
        ...
        $entities = getAncestorsOf('ntas_entities', $this->fields['entities_id']);
        ...
        if (
            countElementsInTable(
                ['ntas_documents_items', 'ntas_documents'],
                ['ntas_documents_items.items_id' => $ID,
                    'ntas_documents_items.itemtype' => $this->getType(),
                    'FKEY' => ['ntas_documents_items' => 'documents_id','ntas_documents' => 'id'],
                    'NOT'  => ['ntas_documents.entities_id' => $entities],
                ]
            ) > '0'
        ) {
            return false;
    ...
    public function isAccessibleFromEntities(array $entities_ids): bool
    {
        ...
        if (
            ...
            && in_array(
                $this->getEntityID(),
                getAncestorsOf("ntas_entities", $entities_ids),
            )
        ) {
            return true;
        }
    ...
    public function getComments()
    {
        ...
        if ($this->isField('states_id') && $this->getType() != 'State') {
            $name = Dropdown::getDropdownName('ntas_states', $this->fields['states_id']);
            if ((string) $name !== '') {
                $toadd[] = [
                    'name'  => __s('Status'),
        ...
        if ($this->isField('locations_id') && $this->getType() != 'Location') {
            $name = Dropdown::getDropdownName("ntas_locations", $this->fields['locations_id']);
            if ((string) $name !== '') {
                $toadd[] = [
        ...
        if ($this->isField('groups_id') && $this->getType() != 'Group') {
            ...
            foreach ($groups as $group) {
                $name = Dropdown::getDropdownName("ntas_groups", $group);
                if ((string) $name !== '') {
                    $toadd[] = [
                ...
                if (!empty($fields) && !empty($fields['fields'])) {
                    ...
                    if (
                        ...
                        if ($fields['is_recursive']) {
                            $entities = getSonsOf('ntas_entities', $fields['entities_id']);
                        }
            ...
            if (isset($searchoptions['datatype'])) {
                ...
                switch ($searchoptions['datatype']) {
                    ...
                    case "dropdown":
                        ...
                        $user = new User();
                        if ($searchoptions['table'] == 'ntas_users') {
    ...
    public function addFiles(array $input, $options = [])
    {
        ...
        foreach ($input[$uploadName] as $key => $file) {
            ...
            if ($docID > 0) {
                ...
                $toadd['date'] = $options['date'] ?? $_SESSION['ntas_currenttime'];
