...
class Change extends CommonITILObject implements DefaultSearchRequestInterface
{
        ...
        if (static::canView()) {
            switch ($item::class) {
                ...
                case User::class:
                    ...
                    if ($_SESSION['glpishow_count_on_tabs']) {
                        $nb = countElementsInTable(
                            ['ntas_changes', 'ntas_changes_users'],
                            [
                                'ntas_changes_users.changes_id'  => new QueryExpression(DBmysql::quoteName('ntas_changes.id')),
                                'ntas_changes_users.users_id'    => $item->getID(),
                                'ntas_changes_users.type'        => CommonITILActor::REQUESTER,
                                'ntas_changes.is_deleted'        => 0,
                ...
                case Group::class:
                    ...
                    if ($_SESSION['glpishow_count_on_tabs']) {
                        $nb = countElementsInTable(
                            ['ntas_changes', 'ntas_changes_groups'],
                            [
                                'ntas_changes_groups.changes_id' => new QueryExpression(DBmysql::quoteName('ntas_changes.id')),
                                'ntas_changes_groups.groups_id'  => $item->getID(),
                                'ntas_changes_groups.type'       => CommonITILActor::REQUESTER,
                                'ntas_changes.is_deleted'        => 0,
                            ] + getEntitiesRestrictCriteria(self::getTable())
                        );
                    }
    ...
    public static function getListForItemRestrict(CommonDBTM $item)
    {
        ...
        switch (true) {
            case $item instanceof User:
                $restrict['ntas_changes_users.users_id'] = $item->getID();
                $restrict['ntas_changes_users.type'] = CommonITILActor::REQUESTER;
                break;

            case $item instanceof Supplier:
                $restrict['ntas_changes_suppliers.suppliers_id'] = $item->getID();
                $restrict['ntas_changes_suppliers.type'] = CommonITILActor::ASSIGN;
                break;
            ...
            case $item instanceof Group:
                ...
                $restrict['ntas_changes_groups.groups_id'] = ($tree ? getSonsOf('ntas_groups', $item->getID()) : $item->getID());
                $restrict['ntas_changes_groups.type'] = CommonITILActor::REQUESTER;
                break;

            default:
                $restrict['ntas_changes_items.items_id'] = $item->getID();
                $restrict['ntas_changes_items.itemtype'] = $item->getType();
                // you can only see your tickets
                if (!Session::haveRight(self::$rightname, self::READALL)) {
                    $or = [
                        'ntas_changes.users_id_recipient'   => Session::getLoginUserID(),
                        [
                            'AND' => [
                                'ntas_changes_users.changes_id'  => 'ntas_changes.id',
                                'ntas_changes_users.users_id'    => Session::getLoginUserID(),
                            ],
                        ],
                    ];
                    if (count($_SESSION['glpigroups'])) {
                        $or['ntas_changes_groups.groups_id'] = $_SESSION['glpigroups'];
                    }
                    $restrict[] = ['OR' => $or];
                }
        }
    ...
    public function getActiveChangesForItem($itemtype, $items_id)
    {
        ...
        return $DB->request([
            ...
            'LEFT JOIN' => [
                'ntas_changes_items' => [
                    'ON' => [
                        'ntas_changes_items' => 'changes_id',
                        $this->getTable()    => 'id',
                    ],
                ],
            ],
            'WHERE'     => [
                'ntas_changes_items.itemtype' => $itemtype,
                'ntas_changes_items.items_id'    => $items_id,
    ...
    public static function showCentralList($start, $status = "process", $showgroupchanges = true)
    {
        ...
        $search_assign = [
            'ntas_changes_users.users_id'   => Session::getLoginUserID(),
            'ntas_changes_users.type'       => CommonITILActor::ASSIGN,
        ];
        ...
        if ($showgroupchanges) {
            ...
            if (count($_SESSION['glpigroups'])) {
                $search_users_id = [
                    'ntas_changes_groups.groups_id' => $_SESSION['glpigroups'],
                    'ntas_changes_groups.type'      => CommonITILActor::REQUESTER,
                ];
                $search_assign = [
                    'ntas_changes_groups.groups_id' => $_SESSION['glpigroups'],
                    'ntas_changes_groups.type'      => CommonITILActor::ASSIGN,
                ];
            }
        }
            ...
            case "tovalidate": // changes waiting for validation
                $JOINS['LEFT JOIN'] = [
                    'ntas_changevalidations' => [
                        'ON' => [
                            'ntas_changevalidations'   => 'changes_id',
                            'ntas_changes'             => 'id',
                        ],
                    ],
                ];
                $WHERE = array_merge(
                    ...
                    [
                        ...
                        'ntas_changevalidations.status'  => CommonITILValidation::WAITING,
                        'ntas_changes.global_validation' => CommonITILValidation::WAITING,
                        'NOT'                            => [
                            'ntas_changevalidations.status'   => [self::SOLVED, self::CLOSED],
                        ],
                    ]
                );
                break;
        ...
        $criteria = [
            'SELECT'          => ['ntas_changes.id'],
            'DISTINCT'        => true,
            'FROM'            => 'ntas_changes',
            'LEFT JOIN'       => [
                'ntas_changes_users'   => [
                    'ON' => [
                        'ntas_changes_users'   => 'changes_id',
                        'ntas_changes'         => 'id',
                    ],
                ],
                'ntas_changes_groups'  => [
                    'ON' => [
                        'ntas_changes_groups'  => 'changes_id',
                        'ntas_changes'         => 'id',
                    ],
                ],
            ],
            'WHERE'           => $WHERE + getEntitiesRestrictCriteria('ntas_changes'),
    ...
    public static function showCentralCount(bool $foruser = false, bool $display = true)
    {
        ...
        if ($foruser) {
            ...
            $WHERE = ['ntas_changes_users.users_id' => Session::getLoginUserID()];

            if (
                isset($_SESSION["glpigroups"])
                && count($_SESSION["glpigroups"])
            ) {
                $criteria['LEFT JOIN']['ntas_changes_groups'] = [
                    'ON' => [
                        'ntas_changes_groups'  => 'changes_id',
                        $table                  => 'id', [
                            'AND' => [
                ...
                $WHERE['ntas_changes_groups.groups_id'] = $_SESSION['glpigroups'];
            }
            $criteria['WHERE'][] = ['OR' => $WHERE];
        }

        $deleted_criteria = $criteria;
        $criteria['WHERE']['ntas_changes.is_deleted'] = 0;
        $deleted_criteria['WHERE']['ntas_changes.is_deleted'] = 1;
        $iterator = $DB->request($criteria);
