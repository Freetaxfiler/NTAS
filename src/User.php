...
class User extends CommonDBTM implements TreeBrowseInterface
{
    ...
    {
        // ntas_users.entities_id is only a pref.
        return false;
    }
    ...
    public function getFromDBbyToken($token, $field = 'personal_token')
    {
        ...
        foreach ($iterator as $user_data) {
            if ($token === $ntas_key->decrypt($user_data[$field])) {
    ...
    public function prepareInputForAdd($input)
    {
            ...
            if (empty($input["password"])) {
                ...
                if ($input["password"] == $input["password2"]) {
                    ...
                    if ($this->validatePassword($input["password"] ?? '', $password_errors)) {
                        ...
                        $input['password_last_update'] = $_SESSION['ntas_currenttime'];
                    } else {
        ...
        foreach (['api_token', 'cookie_token', 'password_forget_token', 'personal_token'] as $token_field) {
            if (
                ...
                $input[$token_field] = $ntas_key->encrypt($input[$token_field]);
            }
        }
    ...
    public function prepareInputForUpdate($input)
    {
            ...
            if (empty($input["password"])) {
                ...
                if ($input["password"] == $input["password2"]) {
                    ...
                    if (
                        ...
                        || (isset($this->fields['password_forget_token']) && ($input['password_forget_token'] == $ntas_key->decrypt($this->fields['password_forget_token']))
                           && (strtotime($_SESSION["ntas_currenttime"]) < strtotime($this->fields['password_forget_token_date']))))
                    ) {
                        $input["password"]
                        = Auth::getPasswordHash($input["password"]);
        ...
        if (
            ...
            $input['personal_token_date'] = $_SESSION['ntas_currenttime'];
        }
        ...
        if (
            ...
            $input['api_token_date'] = $_SESSION['ntas_currenttime'];
        }
        ...
        if (Session::getLoginUserID() == $input['id']) {
            if (
                ...
                && ($_SESSION['ntas_use_mode'] !=  $input['use_mode'])
                && Config::canUpdate()
            ) {
                $_SESSION['ntas_use_mode'] = $input['use_mode'];
            ...
            if (isset($input[$f])) {
                ...
                if (Session::getLoginUserID() == $input['id']) {
                    if ($_SESSION["glpi$f"] != $pref_value) {
                        ...
                        // reinit translations
                        if ($f == 'language') {
                            $_SESSION['ntas_dropdowntranslations'] = DropdownTranslation::getAvailableTranslations($_SESSION["glpilanguage"]);
        ...
        if (
            ...
            if (isset($this->fields["id"]) && ($this->fields["id"] > 0)) {
                ...
                if (count($authtype)) {
                    ...
                    $iterator = $DB->request([
                        'SELECT'    => [
                            'ntas_groups_users.id',
                            'ntas_groups_users.groups_id',
                            'ntas_groups_users.is_dynamic',
                        ],
                        'FROM'      => 'ntas_groups_users',
                        'LEFT JOIN' => [
                            'ntas_groups'  => [
                                'FKEY'   => [
                                    'ntas_groups_users'  => 'groups_id',
                        ...
                        'WHERE'     => [
                            'ntas_groups_users.users_id' => $this->fields['id'],
                        ],
                    ]);
    ...
    public function updateUserEmails()
    {
        ...
        if ($userUpdated) {
            ...
            $user = new User();
            $user->update(['id' => $this->fields['id'], 'date_mod' => $_SESSION['ntas_currenttime']]);
        }
    }
    ...
    public function getFromLDAP($ldap_connection, array $ldap_method, $userdn, $login, $import = true)
    {
        ...
        if ($ldap_connection instanceof Connection) {
            ...
            //Store date_sync
            $this->fields['date_sync']  = $_SESSION['ntas_currenttime'];
...
HTML;
    ...
    public static function getDelegateGroupsForUser($entities_id = '')
    {
        ...
        $iterator = $DB->request([
            'SELECT'          => 'ntas_groups_users.groups_id',
            'DISTINCT'        => true,
            'FROM'            => 'ntas_groups_users',
            'INNER JOIN'      => [
                'ntas_groups'  => [
            ...
            'WHERE'           => [
                'ntas_groups_users.users_id'        => Session::getLoginUserID(),
                'ntas_groups_users.is_userdelegate' => 1,
            ] + getEntitiesRestrictCriteria('ntas_groups', '', $entities_id, true),
        ]);
    ...
    public static function getSqlSearchResult(
        ...
        switch ($right) {
            case "interface":
                $joinprofile = true;
                $WHERE = [
                    'ntas_profiles.interface' => 'central',
                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true);
                break;

            case "id":
                $WHERE = ['ntas_users.id' => Session::getLoginUserID()];
                break;
            ...
            case "delegate":
                ...
                if (count($groups)) {
                    $iterator = $DB->request([
                        'SELECT'    => 'ntas_users.id',
                        'FROM'      => 'ntas_groups_users',
                        'LEFT JOIN' => [
                            'ntas_users'   => [
                                'FKEY'   => [
                                    'ntas_groups_users'  => 'users_id',
                                    'ntas_users'         => 'id',
                                ],
                            ],
                        ],
                        'WHERE'     => [
                            'ntas_groups_users.groups_id' => $groups,
                            'ntas_groups_users.users_id'  => ['<>', Session::getLoginUserID()],
                        ],
                    ]);
                    foreach ($iterator as $data) {
            ...
            case "all":
                $WHERE = [
                    'ntas_users.id' => ['>', 0],
                    'OR' => getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                ];

                if ($with_no_right) {
                    $WHERE['OR'][] = ['ntas_profiles_users.entities_id' => null];
                }
                if (empty($WHERE['OR'])) {
                    unset($WHERE['OR']);
                }
            ...
            default:
                ...
                foreach ($right as $r) {
                    switch ($r) {
                        case 'own_ticket':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'ticket',
                                    'ntas_profilerights.rights'   => ['&', Ticket::OWN],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            break;
                        ...
                        case 'create_ticket_validate':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'  => 'ticketvalidation',
                                    'OR'                       => [
                                        ['ntas_profilerights.rights'   => ['&', TicketValidation::CREATEREQUEST]],
                                        ['ntas_profilerights.rights'   => ['&', TicketValidation::CREATEINCIDENT]],
                                    ],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            $forcecentral = false;
                        ...
                        case 'validate_request':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'ticketvalidation',
                                    'ntas_profilerights.rights'   => ['&', TicketValidation::VALIDATEREQUEST],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            $forcecentral = false;
                            break;
                        ...
                        case 'validate_incident':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'ticketvalidation',
                                    'ntas_profilerights.rights'   => ['&', TicketValidation::VALIDATEINCIDENT],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            $forcecentral = false;
                            break;
                        ...
                        case 'validate':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'changevalidation',
                                    'ntas_profilerights.rights'   => ['&', ChangeValidation::VALIDATE],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            break;
                        ...
                        case 'create_validate':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'changevalidation',
                                    'ntas_profilerights.rights'   => ['&', CREATE],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            break;
                        ...
                        case 'see_project':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'project',
                                    'ntas_profilerights.rights'   => ['&', Project::READMY],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            break;
                        ...
                        case 'faq':
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => 'knowbase',
                                    'ntas_profilerights.rights'   => ['&', KnowbaseItem::READFAQ],
                                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true),
                            ];
                            break;
                        ...
                        default:
                            ...
                            $ORWHERE[] = [
                                [
                                    'ntas_profilerights.name'     => $r,
                                    'ntas_profilerights.rights'   => [
                                        '&',
                ...
                if ($forcecentral) {
                    $WHERE['ntas_profiles.interface'] = 'central';
                }
        }
        ...
        if (!$inactive_deleted) {
            $WHERE = array_merge(
                $WHERE,
                [
                    'ntas_users.is_deleted' => 0,
                    'ntas_users.is_active'  => 1,
                    [
                        'OR' => [
                            ['ntas_users.begin_date' => null],
                            ['ntas_users.begin_date' => ['<', QueryFunction::now()]],
                        ],
                    ],
                    [
                        'OR' => [
                            ['ntas_users.end_date' => null],
                            ['ntas_users.end_date' => ['>', QueryFunction::now()]],
                        ],
                    ],
        ...
        $WHERE[] = [
            'NOT' => [
                'ntas_users.id' => $config['system_user'],
            ],
        ];
        ...
        $criteria = [
            ...
            'LEFT JOIN'       => [
                'ntas_useremails'       => [
                    'ON' => [
                        'ntas_useremails' => 'users_id',
                        'ntas_users'      => 'id',
                        ['AND' => ['ntas_useremails.is_default' => 1]],
                    ],
                ],
                'ntas_profiles_users'   => [
                    'ON' => [
                        'ntas_profiles_users'   => 'users_id',
                        'ntas_users'            => 'id',
                    ],
                ],
            ],
        ...
        if ($count) {
            $criteria['SELECT'] = ['COUNT' => 'ntas_users.id AS CPT'];
            $criteria['DISTINCT'] = true;
        } else {
            $criteria['SELECT'] = ['ntas_users.*', 'ntas_useremails.email AS default_email'];
            $criteria['DISTINCT'] = true;
        }

        if ($joinprofile) {
            $criteria['LEFT JOIN']['ntas_profiles'] = [
                'ON' => [
                    'ntas_profiles_users'   => 'profiles_id',
                    'ntas_profiles'         => 'id',
                ],
            ];
            if ($joinprofileright) {
                $criteria['LEFT JOIN']['ntas_profilerights'] = [
                    'ON' => [
                        'ntas_profilerights' => 'profiles_id',
                        'ntas_profiles'      => 'id',
                    ],
                ];
            }
        ...
        if (!$count) {
            if ((string) $search !== '') {
                ...
                $WHERE[] = [
                    'OR' => [
                        'ntas_users.name'                => ['LIKE', $txt_search],
                        'ntas_users.realname'            => ['LIKE', $txt_search],
                        'ntas_users.firstname'           => ['LIKE', $txt_search],
                        'ntas_users.phone'               => ['LIKE', $txt_search],
                        'ntas_users.registration_number' => ['LIKE', $txt_search],
                        'ntas_useremails.email'          => ['LIKE', $txt_search],
                        $concat,
                    ],
                ];
            }
            ...
            if ($_SESSION["glpinames_format"] == self::FIRSTNAME_BEFORE) {
                $criteria['ORDERBY'] = [
                    'ntas_users.firstname',
                    'ntas_users.realname',
                    'ntas_users.name',
                ];
            } else {
                $criteria['ORDERBY'] = [
                    'ntas_users.realname ASC',
                    'ntas_users.firstname ASC',
                    'ntas_users.name ASC',
                ];
            }
    ...
    public static function isValidUserForEntity(int $user_id, int|array $entity_restrict): bool
    {
        ...
        $where = [
            ...
            [
                'NOT' => ['ntas_users.id' => $config['system_user']],
            ],
        ];

        $entity_criteria = getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_restrict, true);
    ...
    public static function dropdown($options = [])
    {
        ...
        if ($p['entity'] >= 0 && $p['entity_sons']) {
            if (is_array($p['entity'])) {
                ...
                $p['entity'] = getSonsOf('ntas_entities', $p['entity']);
            }
        }
    ...
    public function showItems($tech)
    {
        ...
        foreach ($this->getEntities() as $entity_id) {
            if (Session::haveAccessToEntity($entity_id)) {
                $entity_choices[$entity_id] = Dropdown::getDropdownName("ntas_entities", $entity_id);
            }
        }
        ...
        $iterator = $DB->request([
            ...
            'WHERE'     => ['ntas_groups_users.users_id' => $ID],
        ]);
        $number = 0;
        ...
        $group_choices = [];
        foreach (getAllDataFromTable('ntas_groups') as $g_id => $row) {
                    ...
                    if ($number >= $start && $number < $start + $_SESSION['glpilist_limit']) {
                        ...
                        foreach (explode(',', $data['groups_ids'] ?? '') as $group_id) {
                            ...
                            if (!isset($group_choices[$group_id])) {
                                $group_choices[$group_id] = Dropdown::getDropdownName("ntas_groups", (int) $group_id);
                            }
                        ...
                        $entries[] = [
                            ...
                            'id'            => $data["id"],
                            'type'          => $type_name,
                            'entity'        => Dropdown::getDropdownName("ntas_entities", $data["entities_id"]),
                            'name'          => $link,
                            'serial'        => $data["serial"] ?? '',
                            'otherserial'   => $data["otherserial"] ?? '',
                            'states'        => !empty($data['states_id'])
                                ? Dropdown::getDropdownName("ntas_states", $data['states_id'], false, true, false, '')
                                : '',
    ...
    public static function getOrImportByEmail($email = '', bool $createuserfromemail = false)
    {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'     => [
                'ntas_useremails.email' => $email,
            ],
            'ORDER'     => ['ntas_users.is_active DESC', 'is_deleted ASC'],
        ]);
    ...
    public function updateForgottenPassword(array $input)
    {
        ...
        if (
            ($user->fields['begin_date'] !== null && $user->fields['begin_date'] > $_SESSION['ntas_currenttime'])
            || ($user->fields['end_date'] !== null && $user->fields['end_date'] < $_SESSION['ntas_currenttime'])
        ) {
            throw new ForgetPasswordException(
                __("Unable to reset password, please contact your administrator")
            );
    ...
    public function forgetPassword(string $email, bool $firstpassword = false): bool
    {
        ...
        if ($firstpassword) {
            $event = 'passwordinit';
            $token_date = strtotime($_SESSION["ntas_currenttime"]) + $CFG_GLPI['password_init_token_delay'];
        } else {
            $event = 'passwordforget';
            $token_date = strtotime($_SESSION["ntas_currenttime"]) + DAY_TIMESTAMP;
        }
        $condition = [
            'ntas_users.is_active'  => 1,
            'ntas_users.is_deleted' => 0, [
    ...
    public static function getUniqueToken($field = 'personal_token')
    {
        ...
        $existing_tokens = [];
        foreach ($iterator as $user_data) {
            $existing_tokens[] = $ntas_key->decrypt($user_data[$field]);
        }
    ...
    public function getAuthToken($field = 'personal_token', $force_new = false)
    {
        ...
        $this->update(['id'             => $this->getID(),
            $field           => $hash,
            $field . "_date" => $_SESSION['ntas_currenttime'],
        ]);
        ...
        if (-1 !== $notice_time) {
            ...
            foreach ($notification_data_iterator as $notification_data) {
                ...
                $is_notification_send = NotificationEvent::raiseEvent(
                    'passwordexpires',
                    $user,
                    ['entities_id' => 0] // Notication on root entity (ntas_users.entities_id is only a pref)
                );
    ...
    public static function getUserByForgottenPasswordToken(string $token): ?User
    {
        ...
        foreach ($iterator as $user_data) {
            if ($token === $ntas_key->decrypt($user_data['password_forget_token'])) {
                $user = new self();
