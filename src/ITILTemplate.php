...
abstract class ITILTemplate extends CommonDropdown
{
    ...
    public function mergeTemplateITILCategories($target_id, $source_id)
    {
        ...
        $source = [];
        foreach ($to_merge as $merge) {
            $source[$merge] = getAllDataFromTable('ntas_itilcategories', [$merge => $source_id]);
        }

        // Target categories
        $target = [];
        foreach ($to_merge as $merge) {
            $target[$merge] = getAllDataFromTable('ntas_itilcategories', [$merge => $target_id]);
        }
