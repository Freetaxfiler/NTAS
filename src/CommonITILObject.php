...
abstract class CommonITILObject extends CommonDBTM implements KanbanInterface, TeamworkInterface
{
    ...
    public function getActiveOrSolvedLastDaysForItem($itemtype, $items_id, $days)
    {
        ...
        $iterator = $DB->request([
            'FROM'      => $this->getTable(),
            'LEFT JOIN' => [
                "ntas_items_{$class_l_pl}" => [
                    'ON' => [
                        "ntas_items_{$class_l_pl}" => "{$class_l_pl}_id",
                        $this->getTable()    => 'id',
                    ],
                ],
            ],
        ...
        if (isset($input["document"]) && ($input["document"] > 0)) {
            ...
            if ($doc->getFromDB($input["document"])) {
                ...
                if (
                    ...
                    $input["date_mod"]     = $_SESSION["ntas_currenttime"];
    ...
    public function pre_updateInDB()
    {
        ...
        if (isset($this->input["status"])) {
            ...
            if (
                ...
                $this->fields["solvedate"]    = $_SESSION["ntas_currenttime"];
            ...
            if (
                ...
                $this->fields["closedate"]    = $_SESSION["ntas_currenttime"];
                // If invalid date : set open date
        ...
        if (
            ...
            if (
                ...
                $delay_time = strtotime($_SESSION["ntas_currenttime"])
        ...
        if (
            ...
            $this->updates[]                    = "begin_waiting_date";
            $this->fields["begin_waiting_date"] = $_SESSION["ntas_currenttime"];
    ...
    public function computeDefaultValuesForAdd($input)
    {
        ...
        if (!isset($input["date"]) || empty($input["date"]) || $input["date"] == 'NULL') {
            $input["date"] = $_SESSION["ntas_currenttime"];
        }
    ...
    public function updateDateMod($ID, $no_stat_computation = false, $users_id_lastupdater = 0)
    {
        ...
        if ($this->getFromDB($ID)) {
            ...
            $update = ['date_mod' => $_SESSION['ntas_currenttime']];
    ...
    public function getUsedAuthorBetween($date1 = '', $date2 = '')
    {
        ...
        $criteria = [
            'SELECT'          => [
                'ntas_users.id AS users_id',
                'ntas_users.name AS name',
                'ntas_users.realname AS realname',
                'ntas_users.firstname AS firstname',
            ],
            'DISTINCT' => true,
            'FROM'            => $ctable,
            'LEFT JOIN'       => [
    ...
    public function getUsedRequestTypeBetween($date1 = '', $date2 = '')
    {
        ...
        foreach ($iterator as $line) {
            $tab[] = [
                ...
                'link' => htmlescape(Dropdown::getDropdownName('ntas_requesttypes', $line['requesttypes_id'])),
            ];
        }
        return $tab;
    }
    ...
    public function getUsedSolutionTypeBetween($date1 = '', $date2 = '')
    {
        ...
        foreach ($iterator as $line) {
            $tab[] = [
                ...
                'link' => htmlescape(Dropdown::getDropdownName('ntas_solutiontypes', $line['solutiontypes_id'])),
            ];
        }
        return $tab;
    }
    ...
    public function getUsedSupplierBetween($date1 = '', $date2 = '')
    {
        ...
        $criteria = [
            'SELECT'          => [
                'ntas_suppliers.id AS suppliers_id_assign',
                'ntas_suppliers.name AS name',
            ],
            'DISTINCT'        => true,
            'FROM'            => $ctable,
            'LEFT JOIN'       => [
    ...
    public static function showShort($id, $options = [])
    {
        ...
        if ($item->getFromDB($id)) {
            ...
            if (count($_SESSION["glpiactiveentities"]) > 1) {
                $sixth_col = htmlescape(Dropdown::getDropdownName('ntas_entities', $item->fields['entities_id']));
            ...
            foreach ($item->getGroups(CommonITILActor::REQUESTER) as $d) {
                $eighth_col .= htmlescape(Dropdown::getDropdownName("ntas_groups", $d["groups_id"]));
                $eighth_col .= "<br>";
            }
            ...
            foreach ($item->getGroups(CommonITILActor::ASSIGN) as $d) {
                if (
                    ...
                    $ninth_col .= htmlescape(Dropdown::getDropdownName("ntas_groups", $d["groups_id"]));
                }
                $ninth_col .= "<br>";
            }

            foreach ($item->getSuppliers(CommonITILActor::ASSIGN) as $d) {
                $ninth_col .= htmlescape(Dropdown::getDropdownName("ntas_suppliers", $d["suppliers_id"]));
                $ninth_col .= "<br>";
            }
            ...
            if (!$p['ticket_stats']) {
                ...
                echo $output::showItem(
                    "<span class='b'>"
                        . htmlescape(Dropdown::getDropdownName('ntas_itilcategories', $item->fields["itilcategories_id"]))
                        . "</span>",
                    $item_num,
                    $p['row_num'],
                    $align
    ...
    public static function commonListHeader(
        ...
        if (count($_SESSION["glpiactiveentities"]) > 1) {
            $items[Entity::getTypeName(Session::getPluralNumber())] = "ntas_entities.completename";
        }
        ...
        if (!$ticket_stats) {
            ...
            $items[_n('Category', 'Categories', 1)]           = "ntas_itilcategories.completename";
            $items[__('Planification')]      = "ntas_tickettasks.begin";
        } else {
        ...
        foreach ($data as &$entry) {
            ...
            foreach ($item->getGroups(CommonITILActor::REQUESTER) as $d) {
                if (!isset($group_cache[$d['groups_id']])) {
                    $group_cache[$d['groups_id']] = Dropdown::getDropdownName(table: 'ntas_groups', id: $d['groups_id'], default: '');
                }
            ...
            foreach ($item->getSuppliers(CommonITILActor::ASSIGN) as $d) {
                if (!isset($supplier_cache[$d['suppliers_id']])) {
                    $supplier_cache[$d['suppliers_id']] = Dropdown::getDropdownName(table: 'ntas_suppliers', id: $d['suppliers_id'], default: '');
                }
    ...
    public function getITILActors()
    {
        ...
        switch ($this::class) {
            case Ticket::class:
                $groups_table = 'ntas_groups_tickets';
                break;
            case Problem::class:
                $groups_table = 'ntas_groups_problems';
                break;
            default:
    ...
    public function numberOfTasks($with_private = true)
    {
        ...
        $table = 'ntas_' . strtolower($this->getType()) . 'tasks';
    ...
    public static function getCommonCriteria()
    {
        ...
        $gtable = str_replace('ntas_', 'ntas_groups_', static::getTable());
        $itable = str_replace('ntas_', 'ntas_items_', static::getTable());
        if (static::class == Change::class) {
            $gtable = 'ntas_changes_groups';
            $itable = 'ntas_changes_items';
        }
        $utable = static::getTable() . '_users';
        $stable = static::getTable() . '_suppliers';
        if (static::class == Ticket::class) {
            $stable = 'ntas_suppliers_tickets';
        }
        $table = static::getTable();
        $criteria = [
            'SELECT'          => [
                "$table.*",
                'ntas_itilcategories.completename AS catname',
            ],
            'DISTINCT'        => true,
            'FROM'            => $table,
            'LEFT JOIN'       => [
        ...
        if (count($_SESSION["glpiactiveentities"]) > 1) {
            $criteria['LEFT JOIN']['ntas_entities'] = [
                'ON' => [
                    'ntas_entities'   => 'id',
                    $table            => 'entities_id',
                ],
            ...
            $criteria['SELECT'] = array_merge(
                $criteria['SELECT'],
                [
                    'ntas_entities.completename AS entityname',
    ...
    public function getAssociatedDocumentsCriteria($bypass_rights = false, ?User $user = null, bool $for_anonymous_user = false): array
    {
        ...
        if (
            ...
            if (count($arr_values) > 0) {
                $or_crits[] = [
                    'ntas_documents_items.itemtype' => $task_class::getType(),
                    'ntas_documents_items.items_id' => $arr_values,
                ];
            }
        }
        return ['OR' => $or_crits];
    ...
    public static function getItemsTable()
    {
        switch (static::class) {
            case Change::class:
                return 'ntas_changes_items';
            case Problem::class:
                return 'ntas_items_problems';
            case Ticket::class:
                return 'ntas_items_tickets';
            default:
        ...
        foreach ($tabentities as $entity_id => $rate) {
            ...
            $iterator = $DB->request([
                ...
                'WHERE'     => [
                    ...
                    new QueryExpression(
                        QueryFunction::dateAdd(
                            date: "ntas_entities.max_closedate$config_suffix",
                            interval: $duration,
                            interval_unit: 'DAY'
                        ) . ' <= ' . QueryFunction::now()
                    ),
            ...
            foreach ($iterator as $itil_item) {
                ...
                if (mt_rand(1, 100) <= $rate) {
                    if (
                        $inquest->add([
                            $fk             => $itil_item['id'],
                            'date_begin'    => $_SESSION["ntas_currenttime"],
            ...
            if ($nb) {
                ...
                $task->log(sprintf(
                    __('%1$s: %2$s'),
                    Dropdown::getDropdownName('ntas_entities', $entity_id),
                    $nb
                ));
            }
        }
