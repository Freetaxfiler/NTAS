...
class Infocom extends CommonDBChild
{
    ...
    public static function countForSupplier(Supplier $item)
    {
        ...
        return countElementsInTable(
            ...
            ] + getEntitiesRestrictCriteria('ntas_infocoms', '', '')
        );
    }
    ...
    public static function getDataForAssetInfocomReport(string $itemtype, string $begin, string $end): ?array
    {
        ...
        if (!empty($begin)) {
            $criteria['WHERE'][] = [
                'OR'  => [
                    'ntas_infocoms.buy_date'   => ['>=', $begin],
                    'ntas_infocoms.use_date'   => ['>=', $begin],
                ],
            ];
        }
        ...
        if (!empty($end)) {
            $criteria['WHERE'][] = [
                'OR'  => [
                    'ntas_infocoms.buy_date'   => ['<=', $end],
                    'ntas_infocoms.use_date'   => ['<=', $end],
                ],
            ];
        }
    ...
    public static function getDataForOtherInfocomReport(string $itemtype, string $begin, string $end): ?array
    {
        ...
        switch ($itemtype) {
            case 'SoftwareLicense':
                $criteria['INNER JOIN']['ntas_softwares'] = [
                    'ON'  => [
                        'ntas_softwarelicenses' => 'softwares_id',
                        'ntas_softwares'        => 'id',
                    ],
                ];
                $criteria['WHERE'][] =  getEntitiesRestrictCriteria("ntas_softwarelicenses");
                break;
            default:
    ...
    public static function manageDateOnStatusChange(CommonDBTM $item, $action_add = true)
    {
        ...
        foreach (self::getAutoManagemendDatesFields() as $date => $date_field) {
            ...
            if (
                ...
                $add_or_update    = true;
                $tmp[$date_field] = $_SESSION["ntas_currenttime"];
            }
        }
    ...
    public static function cronInfocom($task = null)
    {
        ...
        foreach (Entity::getEntitiesToNotify('use_infocoms_alert') as $entity => $value) {
            ...
            $iterator = $DB->request([
                ...
                'LEFT JOIN'  => [
                    'ntas_alerts'  => [
                        'ON' => [
                            ...
                            $table         => 'id', [
                                'AND' => [
                                    'ntas_alerts.itemtype'  => self::getType(),
                ...
                'WHERE'     => [
                    new QueryExpression(
                        '(' . $DB->quoteName('ntas_infocoms.alert') . ' & ' . 2 ** Alert::END . ') > 0'
                    ),
                    ...
                    new QueryExpression(QueryFunction::dateDiff(
                        expression1: QueryFunction::dateAdd(
                            date: 'ntas_infocoms.warranty_date',
                            interval: new QueryExpression($DB::quoteName('ntas_infocoms.warranty_duration')),
                            interval_unit: 'MONTH'
                        ),
        ...
        foreach ($items_infos as $entity => $items) {
            ...
            $deleted_expired_items = array_filter($items, static fn($item) => $item['is_deleted'] === 1 && $item['warrantyexpiration'] < $_SESSION['ntas_currenttime']);
            if (
...
HTML;
    ...
    public static function rawSearchOptionsToAdd($itemtype = null)
    {
        ...
        switch ($itemtype) {
            case 'CartridgeItem':
                ...
                $beforejoin        = ['table'      => 'ntas_cartridges',
            ...
            case 'ConsumableItem':
                // Return the infocom linked to the license, not the template linked to the software
                $beforejoin        = ['table'      => 'ntas_consumables',
                    'joinparams' => ['jointype' => 'child'],
                ];
        ...
        $complexjoinparams['beforejoin'][] = ['table'      => 'ntas_infocoms',
        ...
        $tab[] = [
            'id'                 => '173',
            'table'              => 'ntas_businesscriticities',
            'field'              => 'completename',
    ...
    public static function getWarrantyExpir($from, $addwarranty, $deletenotice = 0, $color = false, $auto_renew = false, $periodicity = 0)
    {
        ...
        if ($deletenotice > 0 && $periodicity > 0 && $periodicity != $addwarranty) {
            $current_time = strtotime($_SESSION['ntas_currenttime']);

            // Calculate the number of months elapsed since the beginning
            $current = new DateTime($_SESSION['ntas_currenttime']);
        ...
        if ($auto_renew && $periodicity > 0 && $deletenotice == 0) {
            ...
            while ($timestamp < strtotime($_SESSION['ntas_currenttime'])) {
        ...
        if ($color && ($timestamp < strtotime($_SESSION['ntas_currenttime']))) {
