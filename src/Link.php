...
class Link extends CommonDBTM
{
        ...
        if (self::canView()) {
            ...
            if ($_SESSION['glpishow_count_on_tabs']) {
                ...
                $nb = countElementsInTable(
                    ['ntas_links_itemtypes','ntas_links'],
                    [
                        'ntas_links_itemtypes.links_id'  => new QueryExpression(DBmysql::quoteName('ntas_links.id')),
                        'ntas_links_itemtypes.itemtype'  => $item->getType(),
                    ] + $entity_criteria
                );
            }
    ...
    private static function getIPAndMACForItem(CommonDBTM $item, bool $get_ip = false, bool $get_mac = false): array
    {
        ...
        if ($item::class === NetworkEquipment::class) {
            if ($get_ip) {
                $iterator = $DB->request([
                    'SELECT' => [
                        'ntas_ipaddresses.id',
                        'ntas_ipaddresses.name AS ip',
                    ],
                    'FROM'   => 'ntas_networknames',
                    'INNER JOIN'   => [
                        'ntas_ipaddresses'   => [
                            'ON' => [
                                'ntas_ipaddresses'   => 'items_id',
                                'ntas_networknames'  => 'id', [
                                    'AND' => [
                                        'ntas_ipaddresses.itemtype' => 'NetworkName',
                                    ],
                                ],
                            ],
                        ],
                    ...
                    'WHERE'        => [
                        'ntas_networknames.items_id'  => $item->getID(),
                        'ntas_networknames.itemtype'  => ['NetworkEquipment'],
                    ],
                ]);
        ...
        if ($get_ip) {
            $iterator = $DB->request([
                'SELECT' => [
                    'ntas_ipaddresses.id',
                    'ntas_ipaddresses.name AS ip',
                    'ntas_networkports.mac',
                ],
                'FROM'   => 'ntas_networkports',
                'INNER JOIN'   => [
                    'ntas_networknames'   => [
                        'ON' => [
                            'ntas_networknames'  => 'items_id',
                            'ntas_networkports'  => 'id', [
                                'AND' => [
                                    'ntas_networknames.itemtype' => 'NetworkPort',
                                ],
                            ],
                        ],
                    ],
                ...
                'WHERE'        => [
                    'ntas_networkports.items_id'  => $item->getID(),
                    'ntas_networkports.itemtype'  => $item::class,
                ],
            ]);
            foreach ($iterator as $data2) {
                $ipmac['ip' . $data2['id']]['ip']  = $data2["ip"];
        ...
        if ($get_mac) {
            ...
            if ($get_ip) {
                ...
                $criteria['WHERE']['ntas_networknames.id'] = null;
            }
    ...
    public static function generateLinkContents($link, CommonDBTM $item, bool $safe_url = true, array $custom_vars = [])
    {
        ...
        $vars = [
            ...
            'DOMAIN' => '',
            'NETWORK' => $item->isField('networks_id') ? Dropdown::getDropdownName('ntas_networks', $item->getField('networks_id')) : '',
            'USER' => $item->isField('users_id') ? Dropdown::getDropdownName('ntas_users', $item->getField('users_id')) : '',
        ...
        if (Toolbox::hasTrait($item::class, AssignableItem::class)) {
            $group_names = array_map(static fn($group_id) => Dropdown::getDropdownName('ntas_groups', $group_id), $item->fields['groups_id']);
            ...
            $vars['GROUPS'] = [];
            // GROUP - BC for < GLPI 11
            $vars['GROUP'] = $item->isField('groups_id') ? Dropdown::getDropdownName('ntas_groups', $item->getField('groups_id')) : '';
        }
        ...
        $vars['LOCATION'] = $item->isField('locations_id')
            ? Dropdown::getDropdownName('ntas_locations', $item->getField('locations_id')) : '';
...
TWIG, $buttons_params);
    ...
    public static function rawSearchOptionsToAdd($itemtype = null)
    {
        ...
        // "Fake" search options, processing is done in Search::giveItem() for ntas_links._virtual
        $newtab = [
            'id'                 => '145',
            'table'              => 'ntas_links',
            'field'              => '_virtual',
    ...
    public static function getLinksDataForItem(CommonDBTM $item)
    {
        ...
        return $DB->request([
            'SELECT'       => [
                'ntas_links.id',
                'ntas_links.link AS link',
                'ntas_links.name AS name',
                'ntas_links.data AS data',
                'ntas_links.open_window AS open_window',
            ],
            'FROM'         => 'ntas_links',
            'INNER JOIN'   => [
                'ntas_links_itemtypes'  => [
                    'ON' => [
                        'ntas_links_itemtypes'  => 'links_id',
                        'ntas_links'            => 'id',
                    ],
                ],
            ],
            'WHERE'        => [
                'ntas_links_itemtypes.itemtype'  => $item::class,
            ] + getEntitiesRestrictCriteria('ntas_links', 'entities_id', $restrict, true),
            'ORDERBY'      => 'name',
        ]);
    }
