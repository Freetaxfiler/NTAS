...
class Profile extends CommonDBTM implements LinkableToTilesInterface
{
    ...
    public static function getUnderActiveProfileRestrictCriteria()
    {
        ...
        $criteria = ['ntas_profiles.interface' => $current_interface];
        ...
        foreach (ProfileRight::getAllPossibleRights() as $key => $default) {
            ...
            if (
                ...
                $right_subqueries[] = [
                    'ntas_profilerights.name'     => $key,
                    'RAW'                         => [
                        '(' . $DB::quoteName('ntas_profilerights.rights') . ' | ' . $DB::quoteValue($val) . ')' => $val,
                    ],
                ];
            }
        }
        ...
        $sub_query = new QuerySubQuery([
            ...
            'COUNT'  => 'cpt',
            'WHERE'  => [
                'ntas_profilerights.profiles_id' => new QueryExpression($DB::quoteName('ntas_profiles.id')),
                'OR'                             => $right_subqueries,
            ],
        ]);
        ...
        if (Session::getCurrentInterface() === 'central') {
            return [
                'OR'  => [
                    'ntas_profiles.interface' => 'helpdesk',
                    $criteria,
                ],
            ];
        }
    ...
    public static function currentUserHaveMoreRightThan($IDs = [])
    {
        ...
        if (count($IDs) === 0) {
            ...
            return (countElementsInTable('ntas_profiles')
                     === countElementsInTable(
                         'ntas_profiles',
                ...
                'INNER JOIN' => [
                    ...
                    'ntas_profiles_users' => [
                        'FKEY' => [
                            ...
                            'ntas_profiles'       => 'id',
                            [
                                'AND' => ['ntas_profiles_users.users_id' => $user_id],
                            ],
                        ],
                    ],
                ],
                'WHERE'      => [
                    'ntas_profilerights.name'   => $rightname,
                    'ntas_profilerights.rights' => ['&',  $rightvalue],
                ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $entity_id, true),
            ]
        );
