...
class Supplier extends CommonDBTM
{
    ...
    private function getInfocomsForItemtype(string $itemtype)
    {
        ...
        if ($itemtype === Cartridge::class) {
            $criteria['INNER JOIN']['ntas_cartridgeitems'] = [
                'ON' => [
        ...
        if ($itemtype === Consumable::class) {
            $criteria['INNER JOIN']['ntas_consumableitems'] = [
                'ON' => [
        ...
        if ($itemtype === Item_DeviceControl::class) {
            $criteria['INNER JOIN']['ntas_devicecontrols'] = [
                'ON' => [
                    'ntas_items_devicecontrols'   => 'devicecontrols_id',
                    'ntas_devicecontrols'         => 'id',
                ],
            ];
        ...
        $where = [
            ...
            'ntas_infocoms.suppliers_id'  => $this->getID(),
        ];
            ...
            if ($nb > $_SESSION['glpilist_limit']) {
                ...
                foreach ($items as $data) {
                    ...
                    if (!isset($entity_names_cache[$data["entities_id"]])) {
                        $entity_names_cache[$data["entities_id"]] = Dropdown::getDropdownName("ntas_entities", $data["entities_id"]);
                    }
                    $entries[] = [
