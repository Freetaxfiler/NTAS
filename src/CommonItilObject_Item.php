        ...
        foreach ($types_iterator as $row) {
            ...
            foreach ($iterator as $data) {
                ...
                $entry = [
                    ...
                    'entity' => Dropdown::getDropdownName("ntas_entities", $data['entity']),
                    'state' => isset($data['states_id']) ? Dropdown::getDropdownName("ntas_states", $data['states_id']) : "",
                    'location' => isset($data['locations_id']) ? Dropdown::getDropdownName("ntas_locations", $data['locations_id']) : "",
                    'kb' => $item->getKBLinks(),
            ...
            foreach ($software_helpdesk_types as $itemtype) {
                if (isset($devices[$itemtype]) && count($devices[$itemtype])) {
                    $iterator = $DB->request([
                        ...
                        'WHERE'        => [
                            'ntas_items_softwareversions.items_id' => $devices[$itemtype],
