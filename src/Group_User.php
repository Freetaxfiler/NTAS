...
class Group_User extends CommonDBRelation
{
    ...
    public static function getUserGroups($users_id, $condition = []): array
    {
        ...
        $iterator = $DB->request([
            'SELECT' => [
                'ntas_groups.*',
                'ntas_groups_users.id AS IDD',
                'ntas_groups_users.id AS linkid',
                'ntas_groups_users.is_dynamic AS is_dynamic',
                'ntas_groups_users.is_manager AS is_manager',
                'ntas_groups_users.is_userdelegate AS is_userdelegate',
            ],
            'FROM'   => self::getTable(),
            'LEFT JOIN'    => [
                Group::getTable() => [
            ...
            'WHERE'        => [
                'ntas_groups_users.users_id' => $users_id,
            ] + $condition,
            'ORDER'        => 'ntas_groups.name',
        ]);
    ...
    public static function getGroupUsers($groups_id, $condition = [])
    {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'        => [
                'ntas_groups_users.groups_id' => $groups_id,
            ] + $condition,
            'ORDER'        => 'ntas_users.name',
        ]);
    ...
    public static function getDataForGroup(
        ...
        if ($group->fields['is_recursive']) {
            $entityrestrict = getSonsOf('ntas_entities', $group->fields['entities_id']);
            ...
            if (
                ...
                $entityrestrict = getSonsOf('ntas_entities', $_SESSION['glpiactive_entity']);
            }
        } else {
        ...
        if ($tree) {
            $restrict = getSonsOf('ntas_groups', $group->getID());
        } else {
    ...
    public function post_addItem()
    {
        ...
        foreach ($users as $user) {
            ...
            if ($users_id === Session::getLoginUserID()) {
                $_SESSION['ntas_plannings'] = $plannings;
            }
