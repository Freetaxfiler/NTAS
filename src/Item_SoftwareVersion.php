...
class Item_SoftwareVersion extends CommonDBRelation
{
    ...
    public static function countForVersion($softwareversions_id, $entity = '')
    {
        ...
        foreach ($target_types as $itemtype) {
            ...
            $request = [
                ...
                'WHERE'        => [
                    'ntas_items_softwareversions.softwareversions_id'     => $softwareversions_id,
                    'ntas_items_softwareversions.is_deleted'              => 0,
    ...
    public static function countForSoftware($softwares_id)
    {
        ...
        foreach ($target_types as $itemtype) {
            ...
            $request = [
                ...
                'WHERE'        => [
                    'ntas_softwareversions.softwares_id'      => $softwares_id,
                    'ntas_items_softwareversions.is_deleted'  => 0,
    ...
    private static function showInstallations($searchID, $crit)
    {
        ...
        foreach ($CFG_GLPI['software_types'] as $itemtype) {
            ...
            $query = [
                'SELECT' => [
                    $item_version_table . '.*',
                    'ntas_softwareversions.name AS version',
                    'ntas_softwareversions.softwares_id AS sID',
                ...
                'WHERE'     => [
                    "ntas_softwareversions.$crit"                => $searchID,
                    'ntas_items_softwareversions.is_deleted'     => 0,
                ],
            ];
            ...
            if ($DB->fieldExists($itemtable, 'users_id')) {
                $query['SELECT'][] = 'ntas_users.name AS username';
                $query['SELECT'][] = 'ntas_users.id AS userid';
                $query['SELECT'][] = 'ntas_users.realname AS userrealname';
                $query['SELECT'][] = 'ntas_users.firstname AS userfirstname';
                $query['LEFT JOIN']['ntas_users'] = [
                    'FKEY'   => [
                        $itemtable     => 'users_id',
                        'ntas_users'   => 'id',
                    ],
            ...
            if ($DB->fieldExists($itemtable, 'entities_id')) {
                $query['SELECT'][] = 'ntas_entities.completename AS entity';
                $query['LEFT JOIN']['ntas_entities'] = [
                    'FKEY'   => [
            ...
            if ($DB->fieldExists($itemtable, 'locations_id')) {
                $query['SELECT'][] = 'ntas_locations.completename AS location';
                $query['LEFT JOIN']['ntas_locations'] = [
                    'FKEY'   => [
            ...
            if ($DB->fieldExists($itemtable, 'states_id')) {
                $query['SELECT'][] = 'ntas_states.name AS state';
                $query['LEFT JOIN']['ntas_states'] = [
                    'FKEY'   => [
                        $itemtable     => 'states_id',
                        'ntas_states'   => 'id',
                    ],
            ...
            if ($DB->fieldExists($itemtable, 'groups_id')) {
                $query['SELECT'][] = 'ntas_groups.name AS groupe';
                $query['LEFT JOIN']['ntas_groups'] = [
                    'FKEY'   => [
                        $itemtable     => 'groups_id',
                        'ntas_groups'   => 'id',
                    ],
    ...
    public static function showForVersionByEntity(SoftwareVersion $version)
    {
        ...
        $iterator = $DB->request([
            ...
            'FROM'   => 'ntas_entities',
            'WHERE'  => getEntitiesRestrictCriteria('ntas_entities'),
            'ORDER'  => ['completename'],
        ]);
    ...
    public static function getFromItem(CommonDBTM $item, $sort = null, $order = null, array $filters = []): DBmysqlIterator
    {
        ...
        $select = [
            ...
            'ntas_softwares.name AS softname',
            "ntas_items_softwareversions.id",
            'ntas_states.name as state',
            'ntas_softwareversions.id AS verid',
            'ntas_softwareversions.softwares_id',
            'ntas_softwareversions.name AS version',
            'ntas_softwareversions.arch AS arch',
            'ntas_softwares.is_valid AS softvalid',
            'ntas_items_softwareversions.date_install AS dateinstall',
        ...
        $request = [
            ...
            ] + getEntitiesRestrictCriteria('ntas_softwares', '', '', true),
            'ORDER'     => ['softname', 'version'],
        ];

        if (count($filters)) {
            if (($filters['name'] ?? "") !== '') {
                $request['WHERE']['ntas_softwares.name'] = ['LIKE', '%' . $filters['name'] . '%'];
            }
            if (($filters['state'] ?? "") !== '') {
                $request['WHERE']['ntas_states.name'] = ['LIKE', '%' . $filters['state'] . '%'];
            }
            if (($filters['version'] ?? "") !== '') {
                $request['WHERE']['ntas_softwareversions.name'] = ['LIKE', '%' . $filters['version'] . '%'];
            }
            if (($filters['arch'] ?? "") !== '') {
                $request['WHERE']['ntas_softwareversions.arch'] = ['LIKE', '%' . $filters['arch'] . '%'];
            }
            ...
            if (($filters['software_category'] ?? "") !== '') {
                $request['WHERE']['ntas_softwarecategories.name'] = ['LIKE', '%' . $filters['software_category'] . '%'];
            }
            if (($filters['date_install'] ?? "") !== '') {
                $request['WHERE']['ntas_items_softwareversions.date_install'] = $filters['date_install'];
            }
        }
        ...
        if ($crit > -1) {
            $request['WHERE']['ntas_softwares.softwarecategories_id'] = (int) $crit;
        }
    ...
    public static function showForItem(CommonDBTM $item, $withtemplate = 0)
    {
        ...
        $lic_where = [];
        if (count($installed)) {
            $lic_where['NOT'] = ['ntas_softwarelicenses.id' => $installed];
        }

        $lic_request = [
            'SELECT'       => [
                'ntas_softwarelicenses.*',
                'ntas_items_softwarelicenses.id AS linkid',
                'ntas_softwares.name AS softname',
                'ntas_softwareversions.name AS version',
                'ntas_states.name AS state',
            ],
            ...
            'INNER JOIN'   => [
                'ntas_softwares'  => [
                    'FKEY'   => [
                        'ntas_softwarelicenses' => 'softwares_id',
                        'ntas_softwares'        => 'id',
                    ],
                ],
            ],
            'LEFT JOIN'    => [
                'ntas_items_softwarelicenses'   => [
                    'FKEY'   => [
                        'ntas_items_softwarelicenses' => 'softwarelicenses_id',
                        'ntas_softwarelicenses'       => 'id',
                    ],
                ],
                'ntas_softwareversions'   => [
                    'FKEY'   => [
                        'ntas_softwareversions' => 'id',
                        'ntas_softwarelicenses' => 'softwareversions_id_use',
                        [
                            'AND' => [
                                'ntas_softwarelicenses.softwareversions_id_use' => 0,
                                'ntas_softwarelicenses.softwareversions_id_buy' => new QueryExpression(DBmysql::quoteName('ntas_softwareversions.id')),
                            ],
                        ],
                    ],
                ],
            ...
            'WHERE'     => [
                'ntas_items_softwarelicenses.items_id'  => $items_id,
                'ntas_items_softwarelicenses.itemtype'  => $itemtype,
            ] + $lic_where,
            'ORDER'     => ['softname', 'version'],
        ];
        if ($item->maybeDeleted()) {
            $lic_request['WHERE']['ntas_items_softwarelicenses.is_deleted'] = 0;
        }
    ...
    private static function softwareByCategory(
        ...
        $iterator = $DB->request([
            'SELECT'       => [
                'ntas_softwarelicenses.*',
                'ntas_softwarelicensetypes.name AS type',
            ],
            ...
            'WHERE'        => [
                ...
                'ntas_items_softwarelicenses.itemtype'    => $itemtype,
                'OR'                                            => [
                    'ntas_softwarelicenses.softwareversions_id_use' => $verid,
                    [
                        'ntas_softwarelicenses.softwareversions_id_use' => 0,
                        'ntas_softwarelicenses.softwareversions_id_buy' => $verid,
                    ],
                ],
            ],
        ]);
        ...
        if ($display) {
            ...
            echo "<td>" . htmlescape(Dropdown::getDropdownName("ntas_softwarecategories", $data['softwarecategories_id']));
            echo "</td>";
