...
class KnowbaseItem_Item extends CommonDBRelation
{
    ...
    public static function getItems(CommonDBTM $item, $start = 0, $limit = 0, $used = false)
    {
        ...
        $criteria = [
            ...
            'FIELDS'    => ['ntas_knowbaseitems_items' => '*'],
            'ORDER'     => ['itemtype', 'items_id DESC'],
            'GROUPBY'   => [
                'ntas_knowbaseitems_items.id',
                'ntas_knowbaseitems_items.knowbaseitems_id',
                'ntas_knowbaseitems_items.itemtype',
                'ntas_knowbaseitems_items.items_id',
                'ntas_knowbaseitems_items.date_creation',
                'ntas_knowbaseitems_items.date_mod',
            ],
        ];

        if ($item::class === KnowbaseItem::class) {
            $criteria['WHERE'][] = [
                'ntas_knowbaseitems_items.knowbaseitems_id' => $item->getID(),
            ];
        } else {
            $criteria = array_merge_recursive($criteria, self::getVisibilityCriteriaForItem($item));
            $criteria['WHERE'][] = [
                'ntas_knowbaseitems_items.items_id' => $item->getID(),
                'ntas_knowbaseitems_items.itemtype' => $item::class,
            ];
        }
    ...
    private static function getCountForItem(CommonDBTM $item): int
    {
        ...
        return countElementsInTable('ntas_knowbaseitems_items', $criteria);
    }
