...
class QueuedWebhook extends CommonDBChild
{
    ...
    public function prepareInputForAdd($input)
    {
        ...
        if (!isset($input['create_time']) || empty($input['create_time'])) {
            $input['create_time'] = $_SESSION["ntas_currenttime"];
        }
        if (!isset($input['send_time']) || empty($input['send_time'])) {
            ...
            if ($toadd > 0) {
                $input['send_time'] = date(
                    "Y-m-d H:i:s",
                    strtotime($_SESSION["ntas_currenttime"])
                    + $toadd * MINUTE_TIMESTAMP
                );
            } else {
                $input['send_time'] = $_SESSION["ntas_currenttime"];
            }
        }
        $input['sent_try'] = 0;
    ...
    public static function sendById(int $ID): bool
    {
        ...
        $input = [
            ...
            'sent_time' => $_SESSION["ntas_currenttime"],
        ];
        if ($response !== null) {
        ...
        $badge .= Html::scriptBlock(<<<JS
            $("#{$btn_id}").click(function() {
                ...
                $.ajax({
                    ...
                    success: () => {
                        ntas_toast_info(__('Retried to send webhook'));
                    },
                    error: () => {
                        ntas_toast_error(__('Failed to send webhook'));
                    },
                    complete: () => {
