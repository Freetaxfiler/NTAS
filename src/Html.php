...
TWIG,
    ...
    public static function includeHeader(
        ...
        if ($_SESSION['ntas_use_mode'] === Session::DEBUG_MODE) {
            $tpl_vars['ntas_request_id'] = DebugProfile::getCurrent()->getID();
        }
    ...
    public static function footer()
    {
        ...
        // On demand scripts
        foreach ($_SESSION['ntas_js_toload'] ?? [] as $scripts) {
            if (!is_array($scripts)) {
                $scripts = [$scripts];
            }
    ...
    private static function getPageHeaderTplVars(): array
    {
        ...
        return [
            'is_debug_active'       => $_SESSION['ntas_use_mode'] == Session::DEBUG_MODE,
     ...
     * @return void
     */
    public static function ntas_flush()
    {
        trigger_error(
            '`Html::ntas_glush()` no longer has any effect.',
            E_USER_WARNING
        );
    }
...
JS;
    ...
    public static function computeGenericDateTimeSearch($val, $force_day = false, $specifictime = '')
    {
        ...
        if (empty($specifictime)) {
            $specifictime = strtotime($_SESSION["ntas_currenttime"]);
        }
...
JAVASCRIPT
    ...
    public static function getSimpleForm(
        ...
        $fields['_ntas_csrf_token'] = Session::getNewCSRFToken();
        $fields['_ntas_simple_form'] = 1;
        $button                      = $btname;
        if (!is_array($btname)) {
            $button          = [];
    ...
    {
        $out = Html::hidden('_ntas_csrf_token', ['value' => Session::getNewCSRFToken()]);
...
JS);
    ...
    {
        $message = "<span class=\"copyright\">Naipunya ";
        if ($withVersion) {
    ...
    public static function requireJs($name)
    {
        ...
        if (isset($_SESSION['ntas_js_toload'][$name])) {
            //already in stack
            return;
        }
        switch ($name) {
            case 'ntas_dialog':
                $_SESSION['ntas_js_toload'][$name][] = 'js/ntas_dialog.js';
                break;
            case 'clipboard':
                $_SESSION['ntas_js_toload'][$name][] = 'js/clipboard.js';
                break;
            case 'tinymce':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/tinymce.js';
                $_SESSION['ntas_js_toload'][$name][] = 'js/RichText/FormTags.js';
                $_SESSION['ntas_js_toload'][$name][] = 'js/RichText/UserMention.js';
                $_SESSION['ntas_js_toload'][$name][] = 'js/RichText/ContentTemplatesParameters.js';
                break;
            case 'planning':
                $_SESSION['ntas_js_toload'][$name][] = 'js/planning.js';
                break;
            case 'flatpickr':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/flatpickr.js';
                $_SESSION['ntas_js_toload'][$name][] = 'js/flatpickr_buttons_plugin.js';
                if (isset($_SESSION['glpilanguage'])) {
                    $filename = "lib/flatpickr/l10n/"
                    . strtolower($CFG_GLPI["languages"][$_SESSION['glpilanguage']][3]) . ".js";
                    if (file_exists(GLPI_ROOT . '/public/' . $filename)) {
                        $_SESSION['ntas_js_toload'][$name][] = $filename;
                        break;
                    }
                }
                break;
            case 'fullcalendar':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/fullcalendar.js';
                if (isset($_SESSION['glpilanguage'])) {
                    foreach ([2, 3] as $loc) {
                ...
                break;
            case 'rateit':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/jquery.rateit.js';
                break;
            case 'fileupload':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/jquery-file-upload.js';
                $_SESSION['ntas_js_toload'][$name][] = 'js/fileupload.js';
                break;
            case 'charts':
                $_SESSION['ntas_js_toload']['charts'][] = 'lib/echarts.js';
                break;
            case 'notifications_ajax':
                $_SESSION['ntas_js_toload']['notifications_ajax'][] = 'js/notifications_ajax.js';
                break;
            case 'fuzzy':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/fuzzy.js';
                break;
            case 'marketplace':
                $_SESSION['ntas_js_toload'][$name][] = 'js/marketplace.js';
                break;
            case 'gridstack':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/gridstack.js';
                break;
            case 'masonry':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/masonry.js';
                break;
            case 'sortable':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/sortable.js';
                break;
            case 'rack':
                $_SESSION['ntas_js_toload'][$name][] = 'js/rack.js';
                break;
            case 'leaflet':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/leaflet.js';
                break;
            case 'log_filters':
                $_SESSION['ntas_js_toload'][$name][] = 'js/log_filters.js';
                break;
            case 'photoswipe':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/photoswipe.js';
                break;
            case 'reservations':
                $_SESSION['ntas_js_toload'][$name][] = 'js/reservations.js';
                break;
            case 'cable':
                $_SESSION['ntas_js_toload'][$name][] = 'js/cable.js';
                break;
            case 'altcha':
                $_SESSION['ntas_js_toload'][$name][] = 'lib/altcha.js';
                break;
            default:
                $found = false;
                if (isset($PLUGIN_HOOKS[Hooks::JAVASCRIPT][$name])) {
                    ...
                    foreach ($jslibs as $jslib) {
                        $_SESSION['ntas_js_toload'][$name][] = $jslib;
                    }
                }
                if (!$found) {
    ...
    private static function loadJavascript()
    {
        ...
        if (isset($_SESSION['ntas_js_toload'])) {
            foreach ($_SESSION['ntas_js_toload'] as $key => $script) {
                if (is_array($script)) {
                    foreach ($script as $s) {
                        echo Html::script($s);
                    }
                ...
                unset($_SESSION['ntas_js_toload'][$key]);
            }
        }
    ...
    {
        $debug = (isset($_SESSION['ntas_use_mode'])
         && $_SESSION['ntas_use_mode'] == Session::DEBUG_MODE);
...
CSS;
    ...
    public static function timestampToRelativeStr($ts)
    {
        ...
        $diff = strtotime($_SESSION['ntas_currenttime']) - $ts;
        $date = new DateTime(date('Y-m-d', $ts));
        $today = new DateTime(date('Y-m-d', strtotime($_SESSION['ntas_currenttime'])));
        if ($diff == 0) {
            return __('Now');
        } elseif ($diff > 0) {
