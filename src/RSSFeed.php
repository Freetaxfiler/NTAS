...
class RSSFeed extends CommonDBVisible implements ExtraVisibilityCriteria
{
    ...
    public static function getVisibilityCriteria(bool $forceall = false): array
    {
        ...
        // JOINs
        // Users
        $join['ntas_rssfeeds_users'] = [
            'ON' => [
                'ntas_rssfeeds_users'   => 'rssfeeds_id',
                'ntas_rssfeeds'         => 'id',
            ],
        ];
        ...
        $where = [
            'OR' => [
                ...
                'ntas_rssfeeds_users.users_id'   => Session::getLoginUserID(),
            ],
        ];
        $orwhere = [];
        ...
        if (
            ...
            $join['ntas_groups_rssfeeds'] = [
                'ON' => [
                    'ntas_groups_rssfeeds'  => 'rssfeeds_id',
                    'ntas_rssfeeds'         => 'id',
                ],
            ];
        }
        ...
        if (isset($_SESSION["glpigroups"]) && count($_SESSION["glpigroups"])) {
            $restrict = getEntitiesRestrictCriteria('ntas_groups_rssfeeds', '', '', true);
            $orwhere[] = [
                'ntas_groups_rssfeeds.groups_id' => count($_SESSION["glpigroups"])
                                                      ? $_SESSION["glpigroups"]
                                                      : [-1],
                'OR' => [
                    'ntas_groups_rssfeeds.no_entity_restriction' => 1,
                ] + $restrict,
            ];
        }
        ...
        if ($forceall || isset($_SESSION["glpiactiveprofile"]['id'])) {
            $join['ntas_profiles_rssfeeds'] = [
                'ON' => [
                    'ntas_profiles_rssfeeds'   => 'rssfeeds_id',
                    'ntas_rssfeeds'            => 'id',
                ],
            ];
        }
        ...
        if (isset($_SESSION["glpiactiveprofile"]['id'])) {
            $restrict = getEntitiesRestrictCriteria('ntas_entities_rssfeeds', '', '', true);
            if (!count($restrict)) {
                $restrict = [true];
            }
            $ors = [
                'ntas_profiles_rssfeeds.no_entity_restriction' => 1,
                $restrict,
            ];

            $orwhere[] = [
                'ntas_profiles_rssfeeds.profiles_id' => $_SESSION["glpiactiveprofile"]['id'],
                'OR' => $ors,
            ];
        }
        ...
        if (
            ...
            $join['ntas_entities_rssfeeds'] = [
                'ON' => [
                    'ntas_entities_rssfeeds'   => 'rssfeeds_id',
                    'ntas_rssfeeds'            => 'id',
                ],
            ];
        }
        ...
        if (isset($_SESSION["glpiactiveentities"]) && count($_SESSION["glpiactiveentities"])) {
            ...
            $restrict = getEntitiesRestrictCriteria('ntas_entities_rssfeeds', '', '', true, true);
            if (count($restrict)) {
                $orwhere[] = $restrict;
            }
        }
