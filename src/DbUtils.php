...
final class DbUtils
{
    ...
    {
        if (!str_starts_with($table, 'ntas_')) {
            return "";
        }
    ...
    public function getTableNameForForeignKeyField($fkname)
    {
        ...
        return "ntas_" . preg_replace("/_id.*/", "", $fkname);
    }
    ...
    public function getExpectedTableNameForClass(string $classname): string
    {
        ...
        if ($plug = isPluginItemType($singular)) {
            /* PluginFooBar   => ntas_plugin_foos_bars */
            /* GlpiPlugin\Foo\Bar => ntas_plugin_foos_bars */
    ...
    public function getItemTypeForTable($table)
    {
        ...
        if (isset($CFG_GLPI['glpiitemtypetables'][$table])) {
            ...
            if (class_exists($base_itemtype)) {
                ...
                $is_ntas_class = $class_file !== false && (
            ...
            if ($itemtype === null) {
                ...
                if (class_exists($namespaced_itemtype)) {
                    ...
                    // For example, with the `ntas_namespace1_namespace2_items_filters` table
    ...
    public function getEntitiesRestrictRequest(
        ...
        if ($is_recursive) {
            ...
            if (
                ...
                if (is_array($value)) {
                    $ancestors = $this->getAncestorsOf("ntas_entities", $value);
    ...
    public function getEntitiesRestrictCriteria(
        ...
        if ($is_recursive === 'auto' && !empty($table) && $table != 'ntas_entities') {
    ...
    public function getAncestorsOf($table, $items_id)
    {
        ...
        $lowest_valid_id = $table === 'ntas_entities' ? 0 : 1;
        if (!is_array($items_id)) {
    ...
    public function getTreeLeafValueName($table, $ID, $withcomment = false, $translate = true)
    {
        ...
        if ($translate) {
            if (Session::haveTranslations($this->getItemTypeForTable($table), 'name')) {
                ...
                $JOINS['ntas_dropdowntranslations AS namet'] = [
                    'ON' => [
                        'namet'  => 'items_id',
                        $table   => 'id', [
                            'AND' => [
            ...
            if (Session::haveTranslations($this->getItemTypeForTable($table), 'comment')) {
                ...
                $JOINS['ntas_dropdowntranslations AS namec'] = [
                    'ON' => [
                        'namec'  => 'items_id',
                        $table   => 'id', [
                            'AND' => [
            ...
            foreach ($table_relations as $target_table_key => $target_fields) {
                ...
                foreach ($target_fields as $target_field) {
                    if (is_string($target_field)) {
                        ...
                        if (
                            in_array($source_table, ['ntas_authldaps', 'ntas_authmails'])
                            && $target_table === 'ntas_users'
                        ...
                        if (
                            $source_table === 'ntas_requesttypes'
                            && $target_table === 'ntas_users'
                        ...
                        if (
                            $source_table === 'ntas_knowbaseitems_comments'
                            && $target_table === 'ntas_knowbaseitems_comments'
