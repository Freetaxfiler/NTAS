                ...
                <script type="module">
                    $('button[name="test_ldap_replicate"]').on('click', (e) => {
                        ...
                        $.post(
                            ...
                            ntas_toast_success('{{ __("Test successful")|e('js') }}');
                        }, () => {
                            ntas_toast_error('{{ __("Test failed")|e('js') }}');
                        }).always(() => {
...
TWIG, ['authldaps_id' => $ID]);
    ...
    /**
     * Get LDAP fields to sync to GLPI data from a ntas_authldaps array
     *
...
TWIG, $twig_params);
    ...
    public static function getAllUsers(array $options, &$results, &$limitexceeded)
    {
        ...
        foreach ($iterator as $user) {
            ...
            if ($values['mode'] === self::ACTION_IMPORT) {
                $ntas_users[$user['name']] = $user['name'];
            } else {
        ...
        if ($values['mode'] === self::ACTION_IMPORT) {
            $diff    = array_diff_ukey($ldap_users, $ntas_users, 'strcasecmp');
            $list    = [];
            $tmpuser = new User();
        ...
        if ($ds) {
            ...
            if (!empty($infos)) {
                ...
                $iterator = $DB->request([
                    ...
                    'FROM'   => 'ntas_groups',
                    'WHERE'  => getEntitiesRestrictCriteria('ntas_groups'),
                ]);
                ...
                foreach ($iterator as $group) {
                    ...
                    if (!empty($group["ldap_group_dn"])) {
                        $ntas_groups[$group["ldap_group_dn"]] = 1;
                    } elseif (!empty($group["ldap_value"])) {
                        $ntas_groups[$group["ldap_value"]] = 1;
                    }
                }
                $ligne = 0;
            ...
            try {
                ...
                if ($infos && $infos['dn']) {
                    ...
                    if (
                        ...
                        // Add the auth method
                        // Force date sync
                        $user->fields["date_sync"] = $_SESSION["ntas_currenttime"];
    ...
    public static function getLdapServers(bool $active_only = false)
    {
        ...
        return getAllDataFromTable('ntas_authldaps', $criteria);
    }
    ...
    {
        return (countElementsInTable('ntas_authldaps', ['is_active' => 1]) > 0);
    }
    ...
    {
        return countElementsInTable('ntas_authldaps', ['is_active' => 1]);
    }
    ...
    public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
    {
        ...
        if (
            ...
            if ($_SESSION['glpishow_count_on_tabs']) {
                $count = countElementsInTable('ntas_authldapreplicates', ['authldaps_id' => $item->getID()]);
            }
