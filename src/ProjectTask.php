...
class ProjectTask extends CommonDBChild implements CalDAVCompatibleItemInterface, TeamworkInterface
{
    ...
    public static function getAllTicketsForProject($ID)
    {
        ...
        $iterator = $DB->request([
            'FROM'         => 'ntas_projecttasks_tickets',
            'INNER JOIN'   => [
                'ntas_projecttasks'  => [
                    'ON' => [
            ...
            'FIELDS' =>  'tickets_id',
            'WHERE'        => [
                'ntas_projecttasks.projects_id'   => $ID,
            ],
        ]);
    ...
    public static function getTotalEffectiveDuration($projecttasks_id)
    {
        ...
        $iterator = $DB->request([
            ...
            'LEFT JOIN' => [
                ...
                'ntas_tickets'                => [
                    'FKEY'   => [
                        'ntas_projecttasks_tickets'   => 'tickets_id',
                        'ntas_tickets'                => 'id',
                    ],
                ],
            ],
    ...
    public static function showFor($item)
    {
        ...
        switch ($item::class) {
            case Project::class:
                $criteria['WHERE']['ntas_projecttasks.projects_id'] = $ID;
                break;
            case ProjectTask::class:
                $criteria['WHERE']['ntas_projecttasks.projecttasks_id'] = $ID;
                break;
...
TWIG, $twig_params);
        ...
        if (Session::haveTranslations('ProjectTaskType', 'name')) {
            ...
            $criteria['LEFT JOIN']['ntas_dropdowntranslations AS namet2'] = [
                'ON'  => [
                    'namet2'             => 'items_id',
                    'ntas_projecttasks'  => 'projecttasktypes_id', [
                        'AND' => [
        ...
        if (Session::haveTranslations('ProjectState', 'name')) {
            ...
            $criteria['LEFT JOIN']['ntas_dropdowntranslations AS namet3'] = [
                'ON'  => [
                    'namet3'             => 'items_id',
                    'ntas_projectstates' => 'id', [
                        'AND' => [
...
TWIG, $twig_params);
    ...
    public static function populatePlanning($options = []): array
    {
        ...
        if ($who > 0) {
            $ADDWHERE['ntas_projecttaskteams.itemtype'] = 'User';
            $ADDWHERE['ntas_projecttaskteams.items_id'] = $who;
        }

        if ($whogroup > 0) {
            $ADDWHERE['ntas_projecttaskteams.itemtype'] = 'Group';
            $ADDWHERE['ntas_projecttaskteams.items_id'] = $whogroup;
        }

        if (!count($ADDWHERE)) {
            $ADDWHERE = [
                'ntas_projecttaskteams.itemtype' => 'User',
                'ntas_projecttaskteams.items_id' => new QuerySubQuery([
                    'SELECT'          => 'ntas_profiles_users.users_id',
                    'DISTINCT'        => true,
                    'FROM'            => 'ntas_profiles',
                    'LEFT JOIN'       => [
                        'ntas_profiles_users'   => [
                    ...
                    ] + getEntitiesRestrictCriteria('ntas_profiles_users', '', $_SESSION['glpiactive_entity'], true),
                ]),
            ];
        }

        if (!$options['state_done']) {
            $ADDWHERE['ntas_projecttasks.percent_done'] = ['<', 100];
            $ADDWHERE[] = [
                'OR' => [
                    ['ntas_projectstates.is_finished' => 0],
                    ['ntas_projectstates.is_finished' => null],
                ],
            ];
        }
        ...
        $iterator = $DB->request([
            'SELECT'       => $SELECT,
            'FROM'         => 'ntas_projecttaskteams',
            'INNER JOIN'   => [
                $ttask_table => [
                    'ON' => [
                        'ntas_projecttaskteams' => 'projecttasks_id',
                        $ttask_table      => 'id',
                    ],
                ],
            ],
