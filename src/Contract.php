...
class Contract extends CommonDBTM implements StateInterface
{
    ...
    public function rawSearchOptions()
    {
        ...
        $tab[] = [
            ...
            'joinparams'         => [
                'beforejoin'         => [
                    'table'              => 'ntas_contracts_suppliers',
                    'joinparams'         => [
                        'jointype'           => 'child',
                    ],
                ],
    ...
    public function getSuppliersNames()
    {
        ...
        $iterator = $DB->request([
            'SELECT'       => 'ntas_suppliers.id',
            'FROM'         => 'ntas_suppliers',
            'INNER JOIN'   => [
                'ntas_contracts_suppliers' => [
                    'ON' => [
        ...
        foreach ($iterator as $data) {
            $out .= htmlescape(Dropdown::getDropdownName("ntas_suppliers", $data['id'])) . "<br>";
        }
        return $out;
    }
    ...
    public static function cronContract(?CronTask $task = null)
    {
        ...
        $end_date = QueryFunction::dateAdd(
            date: 'ntas_contracts.begin_date',
            interval: new QueryExpression($DB::quoteName('ntas_contracts.duration')),
            interval_unit: 'MONTH'
        );
        ...
        $notice_date = QueryFunction::dateAdd(
            date: 'ntas_contracts.begin_date',
            interval: new QueryExpression($DB::quoteName('ntas_contracts.duration') . ' - ' . $DB::quoteName('ntas_contracts.notice')),
            interval_unit: 'MONTH'
        );
        ...
        foreach (Entity::getEntitiesToNotify('use_contracts_alert') as $entity => $value) {
            ...
            $query_notice = [
                ...
                'LEFT JOIN' => [
                    'ntas_alerts' => [
                        'FKEY' => [
                            ...
                            [
                                'AND' => [
                                    'ntas_alerts.itemtype' => 'Contract',
                                    'ntas_alerts.type'     => Alert::NOTICE,
                                ],
                            ],
                        ],
                    ],
                ...
                'WHERE'     => [
                    [
                        'RAW' => [
                            DBmysql::quoteName('ntas_contracts.alert') . ' & ' . 2 ** Alert::NOTICE => ['>', 0],
                        ],
                    ],
                    'ntas_alerts.date'           => null,
                    'ntas_contracts.is_deleted'  => 0,
                    [
                        'NOT' => ['ntas_contracts.begin_date' => null],
                    ],
                    'ntas_contracts.duration'    => ['!=', 0],
                    'ntas_contracts.notice'      => ['!=', 0],
                    'ntas_contracts.entities_id' => $entity,
                    new QueryExpression("$end_date_diff_now > 0"),
            ...
            $query_end = [
                ...
                'WHERE'     => [
                    [
                        'RAW' => [
                            DBmysql::quoteName('ntas_contracts.alert') . ' & ' . 2 ** Alert::END => ['>', 0],
                        ],
                    ],
                    'ntas_alerts.date'           => null,
                    'ntas_contracts.is_deleted'  => 0,
            ...
            $query_periodicity = ['FROM' => 'ntas_contracts',
                'WHERE' => ['alert' => ['&', $valPow],
                    'entities_id' => $entity,
                    'is_deleted' => 0,
                ],
    ...
    public static function dropdown($options = [])
    {
        ...
        $WHERE = [];
        if ($p['entity'] >= 0) {
            $WHERE += getEntitiesRestrictCriteria('ntas_contracts', 'entities_id', $p['entity'], true);
        }
        if (count($p['used'])) {
            $WHERE['NOT'] = ['ntas_contracts.id' => $p['used']];
        }
        if (!$p['expired']) {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'     => array_merge([
                'ntas_contracts.is_deleted'   => 0,
                'ntas_contracts.is_template'  => 0,
            ], $WHERE),
            'ORDERBY'   => [
                'ntas_entities.completename',
                'ntas_contracts.name ASC',
                'ntas_contracts.begin_date DESC',
            ],
        ]);
        ...
        foreach ($iterator as $data) {
            if (
                ...
                if ($data["entities_id"] != $prev) {
                    $group = Dropdown::getDropdownName("ntas_entities", $data["entities_id"]);
