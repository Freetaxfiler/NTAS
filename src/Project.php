...
class Project extends CommonDBTM implements ExtraVisibilityCriteria, KanbanInterface, TeamworkInterface
{
    ...
    public static function getVisibilityCriteria(bool $forceall = false): array
    {
        ...
        $join['ntas_projectteams'] = [
            'ON' => [
                'ntas_projectteams'  => 'projects_id',
                'ntas_projects'      => 'id',
            ],
        ];
        ...
        if (count($_SESSION['glpigroups'])) {
            $ors['ntas_projects.groups_id'] = $_SESSION['glpigroups'];
            $ors[] = [
    ...
    public static function getDatatableEntries(array $data): array
    {
        ...
        foreach ($data as &$entry) {
            ...
            $entry['status'] = [
                'content' => Dropdown::getDropdownName('ntas_projectstates', $item->fields['projectstates_id']),
            ...
            if (Session::isMultiEntitiesMode()) {
                if (!isset($entities[$item->fields['entities_id']])) {
                    $entities[$item->fields['entities_id']] = Dropdown::getDropdownName('ntas_entities', $item->fields['entities_id']);
                }
            ...
            if ($item->fields['groups_id']) {
                if (!isset($groups[$item->fields['groups_id']])) {
                    $groups[$item->fields['groups_id']] = sprintf(
                        __s('%1$s %2$s'),
                        htmlescape(Dropdown::getDropdownName('ntas_groups', $item->fields["groups_id"])),
                        Html::showToolTip(
                            Dropdown::getDropdownComments('ntas_groups', $item->fields["groups_id"]),
                            ['display' => false]
                        )
                    );
                }
...
TWIG, $twig_params);
    ...
    public static function getAllForKanban($active = true, $current_id = -1)
    {
        ...
        $iterator = $DB->request(array_merge_recursive([
            'SELECT'   => [
                'ntas_projects.id',
                'ntas_projects.name',
                'ntas_projects.is_deleted',
                'ntas_projectstates.is_finished',
            ],
            'DISTINCT' => true,
            'FROM'     => 'ntas_projects',
            'LEFT JOIN' => $joins,
    ...
    public static function getDataToDisplayOnKanban($ID, $criteria = [])
    {
        ...
        foreach ($required_project_fields as $field) {
            $request['SELECT'][] = 'ntas_projects.' . $field;
        }
        if ($ID > 0) {
            $request['WHERE']['ntas_projects.projects_id'] = $ID;
