...
class Log extends CommonDBTM
{
    ...
    public static function history($items_id, $itemtype, $changes, $itemtype_link = '', $linked_action = 0)
    {
        ...
        if ($result && $DB->affectedRows() > 0) {
            return $_SESSION['ntas_maxhistory'] = $DB->insertId();
        }
        return false;
    }
    ...
    public static function showForItem(CommonDBTM $item, $withtemplate = 0)
    {
        ...
        $total_number    = countElementsInTable("ntas_logs", ['items_id' => $items_id, 'itemtype' => $itemtype ]);
        $filtered_number = countElementsInTable("ntas_logs", ['items_id' => $items_id, 'itemtype' => $itemtype ] + $sql_filters);
     ...
     *      - int id: the           id of the entry in the `ntas_logs` table
                ...
                if (empty($tmp['change'])) {
                    ...
                    if ($data['id_search_option'] == '70') {
                        ...
                        if ($oldval_expl[0] == '&nbsp;') {
                            ...
                            $old_iterator = $DBread->request(['FROM' => 'ntas_users', 'WHERE' => ['name' => $oldval_expl[0]]]);
                        ...
                        if ($newval_expl[0] == '&nbsp;') {
                            ...
                            $new_iterator = $DBread->request(['FROM' => 'ntas_users', 'WHERE' => ['name' => $newval_expl[0]]]);
    ...
    {
        $_SESSION['ntas_maxhistory'] = $this->fields['id'];
    }
