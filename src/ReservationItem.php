...
TWIG, $twig_params);
    ...
    public static function showListSimple()
    {
        ...
        if (isset($_SESSION['ntas_saved']['ReservationItem'])) {
            $_POST = $_SESSION['ntas_saved']['ReservationItem'];
        }
...
TWIG, $twig_params);
        ...
        $iterator = $DB->request([
            'SELECT'          => 'itemtype',
            'DISTINCT'        => true,
            'FROM'            => 'ntas_reservationitems',
            'WHERE'           => [
                'is_active' => 1,
            ] + getEntitiesRestrictCriteria('ntas_reservationitems', 'entities_id', '', true),
        ]);
        ...
        $iterator = $DB->request([
            'SELECT'    => [
                'ntas_peripheraltypes.name',
                'ntas_peripheraltypes.id',
            ],
            'FROM'      => 'ntas_peripheraltypes',
            'LEFT JOIN' => [
                'ntas_peripherals'      => [
                    'ON' => [
                        'ntas_peripheraltypes'  => 'id',
                ...
                'ntas_reservationitems' => [
                    'ON' => [
                        'ntas_reservationitems' => 'items_id',
                        'ntas_peripherals'      => 'id',
                    ],
                ],
            ],
            ...
            'ORDERBY'   => 'ntas_peripheraltypes.name',
        ]);
        ...
        foreach ($CFG_GLPI["reservation_types"] as $itemtype) {
            ...
            $criteria = [
                'SELECT' => [
                    'ntas_reservationitems.id',
                    'ntas_reservationitems.comment',
                    "$itemtable.$itemname AS name",
                    "$itemtable.entities_id AS entities_id",
                    $otherserial,
                    'ntas_locations.id AS location',
                    'ntas_reservationitems.items_id AS items_id',
                ],
                'FROM'   => self::getTable(),
                'INNER JOIN'   => [
                    $itemtable  => [
                        'ON'  => [
                            ...
                            $itemtable              => 'id', [
                                'AND' => [
                                    'ntas_reservationitems.itemtype' => $itemtype,
                                ],
                            ],
                        ],
                    ],
                ...
                'WHERE'        => [
                    'ntas_reservationitems.is_active'   => 1,
            ...
            if (isset($_POST['submit'], $begin, $end)) {
                $criteria['LEFT JOIN']['ntas_reservations'] = [
                    'ON'  => [
                        'ntas_reservationitems' => 'id',
                        'ntas_reservations'     => 'reservationitems_id', [
                            'AND' => [
                                'ntas_reservations.end'    => ['>', $begin],
                                'ntas_reservations.begin'  => ['<', $end],
                            ],
                        ],
                    ],
                ];
                $criteria['WHERE'][] = ['ntas_reservations.id' => null];
            }
            if (!empty($_POST["reservation_types"])) {
                $tmp = explode('#', $_POST["reservation_types"]);
                $criteria['WHERE'][] = ['ntas_reservationitems.itemtype' => $tmp[0]];
                if (
                    isset($tmp[1]) && ($tmp[0] === Peripheral::class)
                    && ($itemtype === Peripheral::class)
                ) {
                    $criteria['LEFT JOIN']['ntas_peripheraltypes'] = [
                        'ON' => [
                            'ntas_peripherals'      => 'peripheraltypes_id',
                            'ntas_peripheraltypes'  => 'id',
                        ],
            ...
            if ((int) ($_POST['locations_id'] ?? 0) > 0) {
                $criteria['WHERE'][] = [
                    'ntas_locations.id' => getSonsOf('ntas_locations', (int) $_POST['locations_id']),
                ];
            }
            ...
            foreach ($iterator as $row) {
                ...
                if (!isset($location_cache[$row["location"]])) {
                    $location_cache[$row["location"]] = Dropdown::getDropdownName("ntas_locations", $row["location"]);
                }
                ...
                if ($showentity) {
                    if (!isset($entity_cache[$row["entities_id"]])) {
                        $entity_cache[$row["entities_id"]] = Dropdown::getDropdownName("ntas_entities", $row["entities_id"]);
                    }
    ...
    public static function cronReservation($task = null)
    {
        ...
        foreach (Entity::getEntitiesToNotify('use_reservations_alert') as $entity => $value) {
            ...
            $criteria = [
                'SELECT' => [
                    'ntas_reservationitems.*',
                    'ntas_reservations.end AS end',
                    'ntas_reservations.id AS resaid',
                ],
                'FROM'   => 'ntas_reservations',
                'LEFT JOIN' => [
                    'ntas_alerts'  => [
                        'ON'  => [
                            'ntas_reservations'  => 'id',
                            'ntas_alerts'        => 'items_id', [
                                'AND' => [
                                    'ntas_alerts.itemtype'  => 'Reservation',
                                    'ntas_alerts.type'      => Alert::END,
                                ],
                            ],
                        ],
                    ],
                ...
                'WHERE'     => [
                    'ntas_reservationitems.entities_id' => $entity,
                    new QueryExpression(
                        QueryFunction::unixTimestamp('ntas_reservations.end') . ' - ' . $secs
                            . ' < ' . QueryFunction::unixTimestamp()
                    ),
                    'ntas_reservations.begin'  => ['<', QueryFunction::now()],
