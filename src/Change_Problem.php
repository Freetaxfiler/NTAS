...
class Change_Problem extends CommonITILObject_CommonITILObject
{
    ...
    public static function showForProblem(Problem $problem)
    {
        ...
        $iterator = $DB->request([
            'SELECT' => [
                'ntas_changes_problems.id AS linkid',
                'ntas_changes.*',
            ],
            'DISTINCT'        => true,
            'FROM'            => 'ntas_changes_problems',
            'LEFT JOIN'       => [
                'ntas_changes' => [
                    'ON' => [
                        'ntas_changes_problems' => 'changes_id',
                        'ntas_changes'          => 'id',
                    ],
                ],
            ],
            'WHERE'           => [
                'ntas_changes_problems.problems_id' => $ID,
            ],
            'ORDERBY'         => 'ntas_changes.name',
        ]);
    ...
    public static function showForChange(Change $change)
    {
        ...
        $iterator = $DB->request([
            ...
            'LEFT JOIN'       => [
                'ntas_problems' => [
                    'ON' => [
                        'ntas_changes_problems' => 'problems_id',
                        'ntas_problems'         => 'id',
                    ],
                ],
            ],
            'WHERE'           => [
                'ntas_changes_problems.changes_id' => $ID,
            ],
            'ORDERBY'         => 'ntas_problems.name',
        ]);
