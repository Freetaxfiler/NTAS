...
class Planning extends CommonGLPI
{
    ...
    public static function getAdditionalMenuLinks()
    {
        ...
        if ($_SESSION['ntas_use_mode'] === Session::DEBUG_MODE) {
...
JAVASCRIPT;
    ...
    public static function showPlanning($fullview = true)
    {
        ...
        if ($fullview) {
            $options = [
                'full_view'    => true,
                'default_view' => $_SESSION['ntas_plannings']['lastview'] ?? 'timeGridWeek',
    ...
    {
        $resources = [];
        foreach ($_SESSION['ntas_plannings']['plannings'] as $planning_id => $planning) {
    ...
    /**
     * Init $_SESSION['ntas_plannings'] var with thses keys :
     ...
     *
     * Also manage color index in $_SESSION['ntas_plannings_color_index']
     *
     * @return void
     */
    public static function initSessionForCurrentUser()
    {
        ...
        if (!isset($_SESSION['ntas_plannings']['filters'])) {
            $_SESSION['ntas_plannings']['filters']   = [];
            $_SESSION['ntas_plannings']['plannings'] = ['user_' . $_SESSION['glpiID'] => [
        ...
        $filters = &$_SESSION['ntas_plannings']['filters'];
        $index_color = 0;
        ...
        $_SESSION['ntas_plannings_color_index'] = 0;
        foreach ($_SESSION['ntas_plannings']['plannings'] as $planning) {
            if ($planning['type'] === 'group_users') {
                $_SESSION['ntas_plannings_color_index'] += count($planning['users']);
            } else {
                $_SESSION['ntas_plannings_color_index']++;
            }
        }
    }
...
TWIG, $twig_params);
    ...
    {
        $used = [];
        foreach (array_keys($_SESSION['ntas_plannings']) as $actor) {
...
TWIG, $twig_params);
    ...
    public static function sendAddUserForm($params = [])
    {
        ...
        $_SESSION['ntas_plannings']['plannings']["user_" . $params['users_id']]
         = ['color'   => self::getPaletteColor('bg', $_SESSION['ntas_plannings_color_index']),
             'display' => true,
             'type'    => 'user',
         ];
...
TWIG, $twig_params);
    ...
    public static function sendAddGroupUsersForm($params = [])
    {
        ...
        $current_group = &$_SESSION['ntas_plannings']['plannings']["group_" . $params['groups_id'] . "_users"];
        $current_group = [
            'display' => true,
            'type'    => 'group_users',
            'users'   => [],
    ...
    public static function editEventForm($params = [])
    {
        ...
        if ($item instanceof CommonDBTM) {
            ...
            $callback = "ntas_close_all_dialogs();
...
TWIG, $twig_params);
    ...
    public static function sendAddGroupForm($params = [])
    {
        ...
        $_SESSION['ntas_plannings']['plannings']["group_" . $params['groups_id']]
...
TWIG, $twig_params);
    ...
    public static function sendAddExternalForm($params = [])
    {
        ...
        $_SESSION['ntas_plannings']['plannings']['external_' . md5($params['url'])] = [
            'color'   => self::getPaletteColor('bg', $_SESSION['ntas_plannings_color_index']),
            'display' => true,
            'type'    => 'external',
            'name'    => $params['name'],
...
TWIG, $twig_params);
    ...
    /**
     * toggle display for selected line of $_SESSION['ntas_plannings']
     *
     * @since 9.1
     *
    ...
    public static function toggleFilter($options = [])
    {
        ...
        if (empty($options['parent'])) {
            $_SESSION['ntas_plannings'][$key][$options['name']]['display'] = ($options['display'] === 'true');
        } else {
            $_SESSION['ntas_plannings']['plannings'][$options['parent']]['users']
    ...
    /**
     * change color for selected line of $_SESSION['ntas_plannings']
     *
     * @since 9.1
     *
     * @param  array $options: should contain:
    ...
    public static function colorFilter($options = [])
    {
        ...
        if (empty($options['parent'])) {
            $_SESSION['ntas_plannings'][$key][$options['name']]['color'] = $options['color'];
        } else {
            $_SESSION['ntas_plannings']['plannings'][$options['parent']]['users']
    ...
    /**
     * delete selected line in $_SESSION['ntas_plannings']
     *
     * @since 9.1
     *
     * @param  array $options: should contain:
    ...
    {
        $current = $_SESSION['ntas_plannings']['plannings'][$options['filter']];
        if ($current['type'] === 'group_users') {
            $_SESSION['ntas_plannings_color_index'] -= count($current['users']);
        } else {
            $_SESSION['ntas_plannings_color_index']--;
        }

        unset($_SESSION['ntas_plannings']['plannings'][$options['filter']]);
        self::savePlanningsInDB();
    }
    ...
    public static function savePlanningsInDB()
    {
        ...
        $user->update(['id' => $_SESSION['glpiID'],
            'plannings' => exportArrayToDB($_SESSION['ntas_plannings']),
        ]);
    }
    ...
    public static function constructEventsArray($options = [])
    {
        ...
        if (!$_SESSION['ntas_plannings']['filters']['StateDone']['display']) {
        ...
        foreach ($CFG_GLPI['planning_types'] as $planning_type) {
            ...
            if ($_SESSION['ntas_plannings']['filters'][$planning_type]['display']) {
                $event_type_color = $_SESSION['ntas_plannings']['filters'][$planning_type]['color'];
                foreach ($_SESSION['ntas_plannings']['plannings'] as $actor => $actor_params) {
        ...
        foreach ($raw_events as $event) {
            if (
                $_SESSION['ntas_plannings']['filters']['OnlyBgEvents']['display']
            ...
            $index_color = array_search("user_$users_id", array_keys($_SESSION['ntas_plannings']));
            $new_event = [
                'title'       => $event['name'],
                'content'     => $content,
                'tooltip'     => $tooltip,
    ...
    public static function constructEventsArraySingleLine($actor, $params = [], &$raw_events = [], &$not_planned = [])
    {
        if ($params['display']) {
            ...
            if ($params['type'] === "group_users") {
                ...
                if (
                    $_SESSION['ntas_plannings']['filters']['NotPlanned']['display']
        ...
        if ($_SESSION['ntas_plannings']['filters']['NotPlanned']['display']) {
            $not_planned = array_map(static fn($arr) => $arr + [
                'not_planned' => true,
                'resourceId' => $actor,
                'event_type_color' => $_SESSION['ntas_plannings']['filters']['NotPlanned']['color'],
            ], $not_planned);
        }
    }
    ...
    private static function getExternalCalendarRawEvents(string $limit_begin, string $limit_end): array
    {
        ...
        foreach ($_SESSION['ntas_plannings']['plannings'] as $planning_id => $planning_params) {
    ...
    public static function displayPlanningItem(array $val, $who, $type = "", $complete = false)
    {
        ...
        if (!$complete && $_SESSION['ntas_plannings']['filters']['OnlyBgEvents']['display']) {
            return "";
        }
...
TWIG, ['msg' => __('Your planning')]);
    ...
    {
        $_SESSION['ntas_plannings']['lastview'] = $view_name;
    }
