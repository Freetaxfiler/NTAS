...
abstract class CommonDBRelation extends CommonDBConnexity
{
    ...
    public function canRelationItem($method, $methodNotItem, $check_entity = true, $forceCheckBoth = false)
    {
        ...
        if ($check_entity && static::$check_entity_coherency) {
            ...
            if ($item1->isEntityAssign() && $item2->isEntityAssign()) {
                ...
                if (
                    ($item1->isRecursive())
                    && in_array($entity1, getAncestorsOf("ntas_entities", $entity2))
                ) {
                    return true;
                }
                if (
                    ($item2->isRecursive())
                    && in_array($entity2, getAncestorsOf("ntas_entities", $entity1))
                ) {
                    return true;
                }
                return false;
    ...
    protected static function getListForItemParams(CommonDBTM $item, $noent = false)
    {
        ...
        if ($noent === false && $link->isEntityAssign() && $link_type != Entity::getType()) {
            $params['SELECT'][] = 'ntas_entities.id AS entity';
            $params['INNER JOIN']['ntas_entities'] = [
                'FKEY'   => [
            ...
            $params['ORDER'] = ['ntas_entities.completename', $params['ORDER']];
        }
