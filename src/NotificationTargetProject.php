...
class NotificationTargetProject extends NotificationTarget
{
    ...
    public function addDataForTemplate($event, $options = [])
    {
        ...
        $items    = getAllDataFromTable('ntas_projectteams', $restrict);
        ...
        foreach ($tasks as $task) {
            ...
            if ($task['projectstates_id']) {
                $tmp['##task.state##'] = Dropdown::getDropdownName('ntas_projectstates', $task['projectstates_id']);
            }
            $tmp['##task.type##']           = '';
            if ($task['projecttasktypes_id']) {
                $tmp['##task.type##'] = Dropdown::getDropdownName('ntas_projecttasktypes', $task['projecttasktypes_id']);
            }
        ...
        foreach ($costs as $cost) {
            ...
            $tmp['##cost.budget##']       = '';
            if ($cost['budgets_id']) {
                $tmp['##cost.budget##'] = Dropdown::getDropdownName('ntas_budgets', $cost['budgets_id']);
            }
        ...
        $iterator = $DB->request([
            ...
            'WHERE'     => [
                'ntas_documents_items.itemtype'  => Project::class,
                'ntas_documents_items.items_id'  => $item->fields['id'],
            ],
        ]);
        ...
        foreach ($iterator as $data) {
            ...
            if ($data['documentcategories_id']) {
                $tmp['##document.heading##'] = Dropdown::getDropdownName('ntas_documentcategories', $data['documentcategories_id']);
            }
        ...
        // Items infos
        $items                = getAllDataFromTable('ntas_items_projects', $restrict);
                ...
                if ($item2 = getItemForItemtype($data['itemtype'])) {
                    if ($item2->getFromDB($data['items_id'])) {
                        ...
                        if ($item2->isField('locations_id') && $item_loc_id = $item2->getField('locations_id')) {
                            $tmp['##item.location##'] = Dropdown::getDropdownName('ntas_locations', $item_loc_id);
                        }
