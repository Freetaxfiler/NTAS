...
class SoftwareLicense extends CommonTreeDropdown implements AssignableItemInterface, StateInterface
{
        ...
        foreach (array_keys($tonotify) as $entity) {
            ...
            $criteria = [
                    ...
                    'ntas_alerts'  => [
                        'ON'  => [
                            ...
                            'ntas_alerts'           => 'items_id', [
                                'AND' => [
                                    'ntas_alerts.itemtype'  => 'SoftwareLicense',
                                ],
                            ],
                        ],
                    ],
                ...
                'WHERE'        => [
                    ...
                    'NOT'                => ['ntas_softwarelicenses.expire' => null],
                    new QueryExpression(
                        QueryFunction::datediff(
                            expression1: $DB::quoteName('ntas_softwarelicenses.expire'),
                            expression2: QueryFunction::curdate()
                        )
                    ) . ' < ' . $before,
                    'ntas_softwares.is_template'  => 0,
                    'ntas_softwares.is_deleted'   => 0,
                    'ntas_softwares.entities_id'  => $entity,
                ],
            ];
    ...
    public static function countForVersion($softwareversions_id, $entity = '')
    {
        ...
        $result = $DB->request([
            ...
            ] + getEntitiesRestrictCriteria('ntas_softwarelicenses', '', $entity),
        ])->current();
    ...
    public static function countForSoftware($softwares_id)
    {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'  => [
                ...
                'is_template'  => 0,
                'number'       => -1,
            ] + getEntitiesRestrictCriteria('ntas_softwarelicenses', '', '', true),
        ]);
    ...
    public static function showForSoftware(Software $software)
    {
        ...
        $number = countElementsInTable(
            ...
            [
                ...
                'ntas_softwarelicenses.is_template'  => 0,
            ] + getEntitiesRestrictCriteria('ntas_softwarelicenses', '', '', true)
        );
...
TWIG, $twig_params);
        ...
        $iterator = $DB->request([
            'SELECT'    => [
                ...
                'ntas_softwarelicensetypes.name AS typename',
                'ntas_states.name AS statename',
            ],
            'FROM'      => 'ntas_softwarelicenses',
            'LEFT JOIN' => [
                'ntas_softwareversions AS buyvers'  => [
                    'ON' => [
                        'ntas_softwarelicenses' => 'softwareversions_id_buy',
                        'buyvers'               => 'id',
                    ],
                ],
                'ntas_softwareversions AS usevers'  => [
                    'ON' => [
                ...
                'extraparams' => [
                    'options' => [
                        'ntas_softwareversions.name' => [
                            'condition' => ["ntas_softwareversions.softwares_id" => $softwares_id],
                        ],
                        'ntas_softwarelicenses.name' => ['itemlink_as_string' => true],
                    ],
                ],
            ],
        ]);
