...
class NotificationTarget extends CommonDBChild
{
     ...
     * @var string
     */
    public $table                       = 'ntas_notificationtargets';
        ...
        if (isset($data['users_id']) && ($data['users_id'] > 0)) {
            ...
            if (
                ...
                || (!is_null($user->getField('begin_date'))
                  && ($user->getField('begin_date') > $_SESSION["ntas_currenttime"]))
                || (!is_null($user->getField('end_date'))
                  && ($user->getField('end_date') < $_SESSION["ntas_currenttime"]))
            ) {
    ...
    final public function addForGroup($manager, $group_id)
    {
        ...
        if ($manager == 1) {
            $criteria['WHERE']['ntas_groups_users.is_manager'] = 1;
        } elseif ($manager == 2) {
            $criteria['WHERE']['ntas_groups_users.is_manager'] = 0;
        }
    ...
    final public function addGroupsToTargets($entity)
    {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'  => [
                'is_usergroup' => 1,
                'is_notify'    => 1,
            ] + getEntitiesRestrictCriteria('ntas_groups', 'entities_id', $entity, true),
            'ORDER'  => 'name',
        ]);
