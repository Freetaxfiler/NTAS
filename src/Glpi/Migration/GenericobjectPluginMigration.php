...
class GenericobjectPluginMigration extends AbstractPluginMigration
{
    ...
    {
        return 'ntas_11_assets_migration';
    }

    #[Override]
    protected function getMainPluginTables(): array
    {
        return ['ntas_plugin_genericobject_objects'];
    }

    protected function validatePrerequisites(): bool
    {
        $required_fields = [
            'ntas_plugin_genericobject_types' => [
                'id',
                'itemtype',
                'name',
                'comment',
        ...
        // Add required table fields depending on the ntas_plugin_genericobject_types entries
        if (
            $this->db->tableExists('ntas_plugin_genericobject_types')
            && $this->db->fieldExists('ntas_plugin_genericobject_types', 'itemtype')
        ) {
    ...
    {
        if ($this->db->tableExists('ntas_plugin_genericobject_typefamilies')) {
            $families_count = $this->db->request(['COUNT' => 'count', 'FROM' => 'ntas_plugin_genericobject_typefamilies'])->current()['count'];
                ...
                $this->db->listTables(
                    ...
                    [
                        'NOT' => [
                            'TABLE_NAME' => [
                                'ntas_plugin_genericobject_types',
                                'ntas_plugin_genericobject_typefamilies',
                            ],
                        ],
                    ]
                )
        ...
        foreach ($this->getGenericobjectTypesIterator() as $type_data) {
            ...
            $profilerights_iterator = $this->db->request([
                ...
                'WHERE' => [
                    ...
                    'name' => preg_replace('/^ntas_/', '', $this->getExpectedTableForPluginClassName($plugin_itemtype)),
                ],
            ]);
    ...
    private function getGenericobjectTypesIterator(): iterable
    {
        if ($this->types_iterator === null) {
            $this->types_iterator = $this->db->request(['FROM' => 'ntas_plugin_genericobject_types']);
        }
     ...
     *
     * @param array<string, mixed> $type_data A row from the `ntas_plugin_genericobject_types` table.
    ...
    {
        $table_matches = [];
        if (preg_match('/^ntas_plugin_genericobject_(?<itemtype_chunk>.+)$/', $table, $table_matches) !== 1) {
    ...
    private function getExpectedTableForPluginClassName(string $classname): string
    {
        ...
        if (!$this->db->tableExists($expected_table)) {
            ...
            // (e.g. `ntas_plugin_genericobject_item_states` instead of `ntas_plugin_genericobject_items_states`).
            // It means that `\getTableForItemType(\getItemTypeForTable($table)))` result differs
            // from the original `$table` value.
            $fallback_table = 'ntas_plugin_genericobject_' . \strtolower(\getPlural($classname_matches['itemtype_chunk']));
            ...
            $fallback_table = 'ntas_plugin_genericobject_' . \strtolower(\getSingular($classname_matches['itemtype_chunk']));
