...
class FormMigration extends AbstractPluginMigration
{
    ...
    {
        return 'ntas_11_form_migration';
    }

    #[Override]
    protected function getMainPluginTables(): array
    {
        return ['ntas_plugin_formcreator_forms'];
    }
     ...
     * must be skipped.
     *
     * @param array{show_condition: int, show_logic: int, show_value?: string} $raw_condition  Row from ntas_plugin_formcreator_conditions
    ...
    private function getFormWhereCriteria(): array
    {
        ...
        if ($this->specificFormsIds !== []) {
            $criteria[] = [
                'ntas_plugin_formcreator_forms.id' => $this->specificFormsIds,
            ];
        } else {
            $criteria[] = [
                // Exclude orphan forms that have no associated entity
                'ntas_plugin_formcreator_forms.entities_id' => new QuerySubQuery([
                    'SELECT' => 'id',
    ...
    protected function validatePrerequisites(): bool
    {
        $formcreator_schema = [
            ...
            'ntas_plugin_formcreator_targetproblems' => [
            ...
            'ntas_plugin_formcreator_targetchanges' => [
    ...
    protected function processMigration(): bool
    {
        ...
        $counts = [
            'categories' => $this->countRecords('ntas_plugin_formcreator_categories'),
            'forms' => $this->countRecords('ntas_plugin_formcreator_forms'),
            'sections' => $this->countRecords('ntas_plugin_formcreator_sections'),
            'questions' => $this->countRecords('ntas_plugin_formcreator_questions', ['NOT' => ['fieldtype' => 'description']]),
            'comments' => $this->countRecords('ntas_plugin_formcreator_questions', ['fieldtype' => 'description']),
            // Each target type is processed twice: once for destinations, once for fields
            'targets_ticket' => $this->countRecords('ntas_plugin_formcreator_targettickets') * 2,
            'targets_problem' => $this->countRecords('ntas_plugin_formcreator_targetproblems') * 2,
            'targets_change' => $this->countRecords('ntas_plugin_formcreator_targetchanges') * 2,
            'translations' => $this->countRecords('ntas_plugin_formcreator_forms_languages'),
            'conditions' => $this->countRecords('ntas_plugin_formcreator_conditions'),
            'configs' => $this->countRecords('ntas_plugin_formcreator_entityconfigs', [
                'replace_helpdesk' => [-2, 2],
            ]),
            'items_tickets_relations' => $this->countRecords('ntas_items_tickets', [
                'itemtype' => 'PluginFormcreatorFormAnswer',
            ]),
            'changes_items_relations' => $this->countRecords('ntas_changes_items', [
                'itemtype' => 'PluginFormcreatorFormAnswer',
            ]),
            'items_problems_relations' => $this->countRecords('ntas_items_problems', [
                'itemtype' => 'PluginFormcreatorFormAnswer',
            ]),
        ];
    ...
    private function processMigrationOfFormCategories(): void
    {
        ...
        if ($this->specificFormsIds !== []) {
            $additional_criteria[] = [
                'ntas_plugin_formcreator_categories.id' => new QuerySubQuery([
                    'SELECT' => 'ntas_plugin_formcreator_forms.plugin_formcreator_categories_id',
                    'FROM'   => 'ntas_plugin_formcreator_forms',
                    'WHERE'  => ['ntas_plugin_formcreator_forms.id' => $this->specificFormsIds],
                ]),
            ];
        }

        // Retrieve data from ntas_plugin_formcreator_categories table
        $raw_form_categories = $this->db->request([
            'SELECT' => ['id', 'name', 'plugin_formcreator_categories_id'],
            'FROM'   => 'ntas_plugin_formcreator_categories',
    ...
    private function processMigrationOfBasicProperties(): void
    {
        ...
        // Retrieve data from ntas_plugin_formcreator_forms table
    ...
    private function processMigrationOfSections(): void
    {
        ...
        // Retrieve data from ntas_plugin_formcreator_sections table
        $raw_sections = $this->db->request([
            'SELECT' => ['id', 'name', 'plugin_formcreator_forms_id', 'order', 'uuid'],
            'FROM'   => 'ntas_plugin_formcreator_sections',
        ]);
    ...
    private function processMigrationOfQuestions(): void
    {
        ...
        $raw_questions = array_values(iterator_to_array($this->db->request([
            ...
            'FROM'   => 'ntas_plugin_formcreator_questions',
    ...
    private function processMigrationOfComments(): void
    {
        ...
        // Retrieve data from ntas_plugin_formcreator_questions table
    ...
    private function processMigrationOfAccessControls(): void
    {
        ...
        $raw_form_access_rights = $this->db->request([
            'SELECT' => [
                'access_rights',
                new QueryExpression('ntas_plugin_formcreator_forms.id', 'forms_id'),
            ...
            'LEFT JOIN'   => [
                'ntas_plugin_formcreator_forms_users' => [
                    'ON' => [
                        'ntas_plugin_formcreator_forms_users' => 'plugin_formcreator_forms_id',
                        'ntas_plugin_formcreator_forms'       => 'id',
                    ],
                ],
                'ntas_plugin_formcreator_forms_groups' => [
                    'ON' => [
                        'ntas_plugin_formcreator_forms_groups' => 'plugin_formcreator_forms_id',
                        'ntas_plugin_formcreator_forms'        => 'id',
                    ],
                ],
                'ntas_plugin_formcreator_forms_profiles' => [
                    'ON' => [
                        'ntas_plugin_formcreator_forms_profiles' => 'plugin_formcreator_forms_id',
                        'ntas_plugin_formcreator_forms'          => 'id',
                    ],
                ],
            ],
    ...
    private function processMigrationOfFormTargets(): void
    {
        ...
        $this->processMigrationOfDestinationType(
            ...
            $this->getBasicTargetsQuery('ntas_plugin_formcreator_targetproblems')
        );
        ...
        $this->processMigrationOfDestinationType(
            ...
            $this->getBasicTargetsQuery('ntas_plugin_formcreator_targetchanges')
        );
    ...
    private function processMigrationOfITILActorsFields(
        ...
        $raw_targets_actors = $this->db->request([
            ...
            'FROM' => 'ntas_plugin_formcreator_targets_actors',
            'WHERE' => [
    ...
    private function processMigrationOfTranslations(): void
    {
        ...
        // Retrieve data from ntas_plugin_formcreator_forms_languages table
        $raw_languages = $this->db->request([
            'SELECT' => [
                'plugin_formcreator_forms_id',
                'ntas_plugin_formcreator_forms_languages.name',
            ],
            'FROM'   => 'ntas_plugin_formcreator_forms_languages',
            'LEFT JOIN'   => [
                'ntas_plugin_formcreator_forms' => [
                    'ON' => [
                        'ntas_plugin_formcreator_forms_languages' => 'plugin_formcreator_forms_id',
                        'ntas_plugin_formcreator_forms'           => 'id',
                    ],
                ],
            ],
    ...
    private function processMigrationOfVisibilityConditions(): void
    {
        ...
        // Retrieve data from ntas_plugin_formcreator_conditions table
        $raw_conditions = $this->db->request([
            'SELECT' => [
                'ntas_plugin_formcreator_conditions.itemtype',
                'ntas_plugin_formcreator_conditions.items_id',
                'ntas_plugin_formcreator_conditions.plugin_formcreator_questions_id',
                'ntas_plugin_formcreator_conditions.show_condition',
                'ntas_plugin_formcreator_conditions.show_value',
                'ntas_plugin_formcreator_conditions.show_logic',
                'ntas_plugin_formcreator_conditions.order',
                new QueryExpression(
                    'COALESCE(ntas_plugin_formcreator_forms.show_rule, ntas_plugin_formcreator_questions.show_rule, ntas_plugin_formcreator_sections.show_rule, ntas_plugin_formcreator_targettickets.show_rule, ntas_plugin_formcreator_targetchanges.show_rule, ntas_plugin_formcreator_targetproblems.show_rule)',
                    'show_rule'
                ),
            ],
            'FROM'   => 'ntas_plugin_formcreator_conditions',
            'JOIN' => [
                'ntas_plugin_formcreator_forms' => [
                    'ON' => [
                        'ntas_plugin_formcreator_conditions' => 'items_id',
                        'ntas_plugin_formcreator_forms'      => 'id',
                        ['AND' => ['ntas_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorForm']],
                    ],
                ],
                'ntas_plugin_formcreator_questions' => [
                    'ON' => [
                        'ntas_plugin_formcreator_conditions' => 'items_id',
                        'ntas_plugin_formcreator_questions'  => 'id',
                        ['AND' => ['ntas_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorQuestion']],
                    ],
                ],
                'ntas_plugin_formcreator_sections' => [
                    'ON' => [
                        'ntas_plugin_formcreator_conditions' => 'items_id',
                        'ntas_plugin_formcreator_sections'   => 'id',
                        ['AND' => ['ntas_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorSection']],
                    ],
                ],
                'ntas_plugin_formcreator_targettickets' => [
                    'ON' => [
                        'ntas_plugin_formcreator_conditions'    => 'items_id',
                        'ntas_plugin_formcreator_targettickets' => 'id',
                        ['AND' => ['ntas_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorTargetTicket']],
                    ],
                ],
                'ntas_plugin_formcreator_targetchanges' => [
                    'ON' => [
                        'ntas_plugin_formcreator_conditions'    => 'items_id',
                        'ntas_plugin_formcreator_targetchanges' => 'id',
                        ['AND' => ['ntas_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorTargetChange']],
                    ],
                ],
                'ntas_plugin_formcreator_targetproblems' => [
                    'ON' => [
                        'ntas_plugin_formcreator_conditions'     => 'items_id',
                        'ntas_plugin_formcreator_targetproblems' => 'id',
                        ['AND' => ['ntas_plugin_formcreator_conditions.itemtype' => 'PluginFormcreatorTargetProblem']],
                    ],
                ],
            ],
            'HAVING'  => [
    ...
    private function displayWarningForNonMigratedItems(): void
    {
        ...
        if ($this->specificFormsIds !== []) {
            $criteria['ntas_plugin_formcreator_forms.id'] = $this->specificFormsIds;
        }
    ...
    private function processMigrationOfValidationConditions(): void
    {
        ...
        // Retrieve data from ntas_plugin_formcreator_questionranges and ntas_plugin_formcreator_questionregexes tables
        $raw_conditions = $this->db->request([
            'SELECT' => [
                'ntas_plugin_formcreator_questions.id',
                'ntas_plugin_formcreator_questionranges.range_min',
                'ntas_plugin_formcreator_questionranges.range_max',
                'ntas_plugin_formcreator_questionregexes.regex',
            ],
            'FROM'   => 'ntas_plugin_formcreator_questions',
            'LEFT JOIN' => [
                'ntas_plugin_formcreator_questionranges' => [
                    'ON' => [
                        'ntas_plugin_formcreator_questions' => 'id',
                        'ntas_plugin_formcreator_questionranges' => 'plugin_formcreator_questions_id',
                    ],
                ],
                'ntas_plugin_formcreator_questionregexes' => [
                    'ON' => [
                        'ntas_plugin_formcreator_questions' => 'id',
                        'ntas_plugin_formcreator_questionregexes' => 'plugin_formcreator_questions_id',
                    ],
                ],
            ],
            'WHERE' => [
                'OR' => [
                    'NOT' => [
                        'ntas_plugin_formcreator_questionranges.range_min' => 'NULL',
                        'ntas_plugin_formcreator_questionranges.range_max' => 'NULL',
                        'ntas_plugin_formcreator_questionregexes.regex'    => 'NULL',
                    ],
                ],
            ],
        ]);
    ...
    private function getTicketTargetsQuery(): array
    {
        return [
                ...
                new QueryExpression(
                    'COALESCE(
                        (
                            ...
                            FROM ntas_plugin_formcreator_items_targettickets t2
                            WHERE t2.plugin_formcreator_targettickets_id = ntas_plugin_formcreator_targettickets.id
                                AND t2.link = 0
                ...
                new QueryExpression(
                    'COALESCE(
                        (
                            ...
                            FROM ntas_plugin_formcreator_items_targettickets t3
                            WHERE t3.plugin_formcreator_targettickets_id = ntas_plugin_formcreator_targettickets.id
                                AND t3.link > 0
            ...
            'FROM' => 'ntas_plugin_formcreator_targettickets',
        ];
    }
    ...
    private function processMigrationOfConfigs(): void
    {
        $entity_configs = $this->db->request([
            ...
            'FROM'   => 'ntas_plugin_formcreator_entityconfigs',
        ]);
        foreach ($entity_configs as $entity_config) {
            $this->db->update('ntas_entities', [
     ...
     *
     * @param string $table The source table name (e.g., 'ntas_items_tickets')
    ...
    private function migrateITILObjectRelations(
        ...
        $formanswers_links = $this->db->request([
            'SELECT' => [
                "$table.id",
                "$table.$itil_fk",
                'ntas_plugin_formcreator_formanswers.plugin_formcreator_forms_id',
            ],
            'FROM' => $table,
            'JOIN' => [
                'ntas_plugin_formcreator_formanswers' => [
                    'ON' => [
                        $table                                => 'items_id',
                        'ntas_plugin_formcreator_formanswers' => 'id',
                    ],
                ],
                'ntas_plugin_formcreator_forms' => [
                    'ON' => [
    ...
    private function getSourceItemtypeForTargetTable(string $targetTable): string
    {
        return match ($targetTable) {
            'ntas_plugin_formcreator_targettickets'  => 'PluginFormcreatorTargetTicket',
            'ntas_plugin_formcreator_targetproblems' => 'PluginFormcreatorTargetProblem',
            'ntas_plugin_formcreator_targetchanges'  => 'PluginFormcreatorTargetChange',
