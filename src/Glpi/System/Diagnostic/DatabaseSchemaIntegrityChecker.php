...
class DatabaseSchemaIntegrityChecker
{
    ...
    protected function getNormalizedSql(string $create_table_sql): string
    {
        ...
        if (
            $table_name === 'ntas_notimportedemails'
            && $dbversion !== null
            && version_compare(VersionParser::getNormalizedVersion($dbversion), '10.0', '<')
        ) {
            // Before GLPI 10.0.0, COLLATE property was not explicitly defined for ntas_notimportedemails,
        ...
        if (
            $table_name === 'ntas_impactcontexts'
            && $dbversion !== null
            && version_compare(VersionParser::getNormalizedVersion($dbversion), '10.0.1', '<')
        ) {
            // Remove default value on ntas_impactcontexts.positions column.
    ...
    private function getDbVersion(): ?string
    {
        if ($this->db_version === null) {
            if ($this->db->tableExists('ntas_configs') && $this->db->fieldExists('ntas_configs', 'context')) {
            ...
            } elseif ($this->db->tableExists('ntas_configs') && $this->db->fieldExists('ntas_configs', 'version')) {
                // GLPI < 0.85
