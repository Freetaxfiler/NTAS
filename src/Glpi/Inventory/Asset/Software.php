...
class Software extends InventoryAsset
{
    ...
    public function handle()
    {
        ...
        $iterator = $DB->request([
            'SELECT' => [
                'ntas_items_softwareversions.id as item_soft_version_id',
                'ntas_items_softwareversions.date_install as item_soft_version_date_install',
                'ntas_softwares.id as softid',
                'ntas_softwares.name',
                'ntas_softwareversions.id AS versionid',
                'ntas_softwareversions.name AS version',
                'ntas_softwareversions.arch',
                'ntas_softwares.entities_id',
                'ntas_softwarecategories.name AS softwarecategories_id',
                'ntas_softwares.is_recursive',
                'ntas_softwareversions.operatingsystems_id',
                'ntas_manufacturers.name AS manufacturers_id',
            ],
            ...
            'WHERE'     => [
                'ntas_items_softwareversions.items_id' => $this->item->fields['id'],
                'ntas_items_softwareversions.itemtype'    => $this->item->getType(),
                'ntas_items_softwareversions.is_dynamic'  => 1,
            ],
        ]);
    ...
    private function storeSoftware()
    {
        ...
        foreach ($this->data as $val) {
            ...
            if (!isset($this->softwares[$skey])) {
                ...
                //set create date
                $stmt_columns['date_creation'] = $_SESSION["ntas_currenttime"];
