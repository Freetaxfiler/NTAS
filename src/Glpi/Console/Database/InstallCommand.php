...
class InstallCommand extends AbstractConfigureCommand implements ConfigurationCommandInterface
{
    ...
    /**
     * Error code returned when trying to install and having a DB already containing ntas_* tables.
     *
     * @var int
     */
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($tables_result->fetch_array()[0] > 0 && !$force) {
            $output->writeln(
                '<error>' . __('Database already contains "ntas_*" tables. Use --force option to override existing database.') . '</error>'
            );
