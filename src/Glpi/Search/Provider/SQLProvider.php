...
final class SQLProvider implements SearchProviderInterface
{
    ...
    private static function buildSelect(array $data, string $itemtable): string
    {
        ...
        if ($as_map && $data['itemtype'] !== 'Entity') {
            $SELECT .= ' `ntas_locations`.`id` AS loc_id, ';
        }
    ...
    public static function getDefaultSelectCriteria(string $itemtype): array
    {
        ...
        switch ($itemtype) {
            case 'FieldUnicity':
                $ret[] = "`ntas_fieldunicities`.`itemtype` AS ITEMTYPE";
                break;
    ...
    public static function getSelectCriteria(string $itemtype, int $ID, bool $meta = false, string $meta_type = '')
    {
        ...
        switch ($table . "." . $field) {
                ...
                if ($itemtype !== User::class) {
                    if ($opt->isForceGroupBy()) {
                        ...
                        if (
                            ...
                            && in_array($opt['joinparams']['beforejoin']['table'], ['ntas_tickets_users', 'ntas_changes_users', 'ntas_problems_users'])
                        ) { // For tickets_users
            ...
            case "ntas_softwarelicenses.number":
                    ...
                    $SELECT = [
                        QueryFunction::groupConcat(
                            expression: QueryFunction::concat([
                                ...
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "ntas_profiles_users{$addtable2}.entities_id",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "ntas_profiles_users{$addtable2}.is_recursive",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "ntas_profiles_users{$addtable2}.is_dynamic",
                            ]),
                            distinct: true,
                            separator: Search::LONGSEP,
                            alias: $NAME
            ...
            case "ntas_entities.completename":
                    ...
                    $SELECT = [
                        QueryFunction::groupConcat(
                            expression: QueryFunction::concat([
                                ...
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "ntas_profiles_users{$addtable2}.profiles_id",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "ntas_profiles_users{$addtable2}.is_recursive",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "ntas_profiles_users{$addtable2}.is_dynamic",
            ...
            case "ntas_softwareversions.name":
            ...
            case "ntas_softwareversions.comment":
                $_table = ($meta && $meta_type === Software::class) ? 'ntas_softwares' : ($table . $addtable);
                    ...
                    $SELECT = [
                        QueryFunction::groupConcat(
                            expression: QueryFunction::concat([
                                "ntas_softwares.name",
                                new QueryExpression($DB::quoteValue(" - ")),
                                "ntas_softwareversions{$addtable}.name",
                                new QueryExpression($DB::quoteValue(" - ")),
                                "{$table}{$addtable2}.{$field}",
                                new QueryExpression($DB::quoteValue(Search::SHORTSEP)),
                                "{$table}{$addtable2}.id",
            ...
            case "ntas_itilfollowups.content":
            case "ntas_tickettasks.content":
            case "ntas_changetasks.content":
            case "ntas_problemtasks.content":
            case "ntas_tickettasks.state":
            case "ntas_changetasks.state":
            case "ntas_problemtasks.state":
            case "ntas_tickettasks.is_private":
            case "ntas_changetasks.is_private":
            case "ntas_problemtasks.is_private":
            case "ntas_tickettasks.actiontime":
            case "ntas_changetasks.actiontime":
            case "ntas_problemtasks.actiontime":
            case "ntas_tickettasks.begin":
            case "ntas_changetasks.begin":
            case "ntas_problemtasks.begin":
            case "ntas_tickettasks.end":
            case "ntas_changetasks.end":
            case "ntas_problemtasks.end":
        ...
        $plugin_table_pattern = "/^ntas_plugin_([a-z0-9]+)/";
    ...
    public static function getDefaultWhereCriteria(string $itemtype): array
    {
        ...
        switch ($itemtype) {
            ...
            case Notification::class:
                if (!Config::canView()) {
                    $criteria = [
                        'NOT' => ['ntas_notifications.itemtype' => ['CronTask', 'DBConnection']],
                    ];
                }
                break;
            ...
            case User::class:
                ...
                if (!Session::canViewAllEntities()) {
                    $criteria = getEntitiesRestrictCriteria("ntas_profiles_users", '', '', true);
                }
                break;
            ...
            case ProjectTask::class:
                if (!Session::haveRightsOr('project', [Project::READALL, Project::READMY])) {
                    ...
                    $teamtable = 'ntas_projecttaskteams';
                    $user_criteria = [
                    ...
                    $or_criteria = [
                        $user_criteria,
                        'ntas_projects.users_id' => Session::getLoginUserID(),
                    ];
                    if (count($_SESSION['glpigroups'])) {
                        $group_criteria = [
                            "$teamtable.itemtype" => Group::class,
            ...
            case Project::class:
                if (!Session::haveRight("project", Project::READALL)) {
                    ...
                    if (count($_SESSION['glpigroups'])) {
                        ...
                        $or_criteria[] = ["ntas_projects.groups_id" => $_SESSION['glpigroups']];
                        $or_criteria[] = $group_criteria;
                    }
                    $criteria = [
                        "OR" => $or_criteria,
            ...
            case Ticket::class:
                ...
                if (!Session::haveRight("ticket", Ticket::READALL)) {
                    ...
                    if (Session::haveRight("ticket", Ticket::READMY)) {
                        $criteria['OR'][] = [
                            'OR' => [
                                ...
                                "ntas_tickets.users_id_recipient" => Session::getLoginUserID(),
                            ],
                        ];
                    } else {
                    ...
                    if (Session::haveRight('ticket', Ticket::READNEWTICKET)) {
                        $criteria['OR'][] = [
                            'ntas_tickets.status' => CommonITILObject::INCOMING,
                        ];
                    }
            ...
            case Problem::class:
                if ($itemtype === Change::class) {
                    $right       = 'change';
                    $table       = 'changes';
                    $groupetable = "`ntas_changes_groups_";
                } elseif ($itemtype === Problem::class) {
                    $right       = 'problem';
                    $table       = 'problems';
                    $groupetable = "`ntas_groups_problems_";
                }
                // Same structure in addDefaultJoin
                $condition = '';
                if (!Session::haveRight("$right", $itemtype::READALL)) {
                    ...
                    if (Session::haveRight("$right", $itemtype::READMY)) {
                        $requester_table      = '`ntas_' . $table . '_users_'
                        ...
                        $observer_table       = '`ntas_' . $table . '_users_'
                        ...
                        $assign_table         = '`ntas_' . $table . '_users_'
                        ...
                        $criteria['OR'][] = [
                            'OR' => [
                                ...
                                "$observer_table.users_id" => Session::getLoginUserID(),
                                "$assign_table.users_id" => Session::getLoginUserID(),
                                "ntas_" . $table . ".users_id_recipient" => Session::getLoginUserID(),
                            ],
                        ];
                        if (count($_SESSION['glpigroups'])) {
                            $criteria['OR'][] = [
            ...
            case TicketTask::class:
                ...
                $criteria = [
                    'OR' => [
                        'ntas_tickettasks.is_private' => $allowed_is_private,
                        // Check for assigned or created tasks
                        'ntas_tickettasks.users_id' => Session::getLoginUserID(),
                        'ntas_tickettasks.users_id_tech' => Session::getLoginUserID(),
                    ],
                ];
            ...
            case ITILFollowup::class:
                ...
                $criteria = [
                    'ntas_itilfollowups.is_private' => $allowed_is_private,
                    'OR' => [
    ...
    public static function getWhereCriteria($nott, $itemtype, $ID, $searchtype, $val, $meta = false): ?array
    {
        ...
        switch ($inittable . "." . $field) {
            // case "ntas_users_validation.name" :
            ...
            case "ntas_users.name":
                ...
                if ($itemtype === User::class) { // ntas_users case / not link table
                    $criteria = [
                        'OR' => [],
                    ];
                ...
                if (is_a($itemtype, CommonITILObject::class, true)) {
                    $itil_user_tables = ['ntas_tickets_users', 'ntas_changes_users', 'ntas_problems_users'];
            ...
            case "ntas_tickets.status":
            case "ntas_problems.status":
            case "ntas_changes.status":
                $tocheck = [];
                $item = getItemForItemtype($itemtype);
            ...
            case "ntas_tickets_tickets.tickets_id_1":
                $tmplink = 'OR';
                $compare = '=';
                if ($nott) {
                    $tmplink = 'AND';
            ...
            case "ntas_tickets.priority":
            case "ntas_tickets.impact":
            case "ntas_tickets.urgency":
            case "ntas_problems.priority":
            case "ntas_problems.impact":
            case "ntas_problems.urgency":
            case "ntas_changes.priority":
            case "ntas_changes.impact":
            case "ntas_changes.urgency":
            case "ntas_projects.priority":
                if (is_numeric($val)) {
                    if ($val > 0) {
                        $compare = ($nott ? '<>' : '=');
                        return [
            ...
            case "ntas_tickets.global_validation":
            case "ntas_ticketvalidations.status":
            case "ntas_changes.global_validation":
            case "ntas_changevalidations.status":
        ...
        // Link with plugin tables
        if (preg_match("/^ntas_plugin_([a-z0-9]+)/", $inittable, $matches)) {
            if (count($matches) == 2) {
                $plug     = $matches[1];
                $out = Plugin::doOneHook(
                    $plug,
        ...
        if ($use_subquery_on_id_search || $use_subquery_on_text_search) {
            ...
            if ($use_subquery_on_id_search) {
                ...
                //
                // AND `ntas_tickets`.`id` NOT IN (
                //     SELECT `tickets_id`
                //     FROM `ntas_groups_tickets`
                //     WHERE `groups_id` = '4' AND `ntas_groups_tickets`.`type` = '3'
                // )
                ...
                //      ) AND `ntas_groups_tickets`.`type` = '3'
                // )
        ...
        switch ($itemtype) {
            ...
            case Problem::class:
                if ($itemtype === Change::class) {
                    $right       = 'change';
                    $table       = 'changes';
                    $groupetable = "ntas_changes_groups";
                    $linkfield   = "changes_groups_id";
                } elseif ($itemtype === Problem::class) {
                    $right       = 'problem';
                    $table       = 'problems';
                    $groupetable = "ntas_groups_problems";
    ...
    public static function getLeftJoinCriteria(
        ...
        if ($tocheck === 'ntas_groups' && ($itemtype === AllAssets::class || $itemtype === ReservationItem::class)) {
            $already_link_tables[] = 'ntas_groups_items';
            return [
                'LEFT JOIN' => [
                    'ntas_groups_items' => [
                        'ON' => [
                            'ntas_groups_items' => 'items_id',
                            $rt => 'id', [
                                'AND' => [
                                    'ntas_groups_items.itemtype' => $rt . '_TYPE', // Placeholder to be replaced at the end of the SQL construction during union case handling
                                    'ntas_groups_items.type' => Group_Item::GROUP_TYPE_NORMAL,
                                ],
                            ],
                        ],
                    ],
        ...
        if (
            $specific_leftjoin_criteria === []
            && preg_match("/^ntas_plugin_([a-z0-9]+)/", $new_table, $matches)
        ) {
            if (count($matches) == 2) {
    ...
    public static function getMetaLeftJoinCriteria(string $from_type, string $to_type, array &$already_link_tables2, array $joinparams = []): array
    {
        ...
        if ($from_referencetype === Software::class && in_array($to_type, $CFG_GLPI['software_types'], true)) {
            ...
            $softwareversions_table = "ntas_softwareversions{$alias_suffix}";
            if (!in_array($softwareversions_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $softwareversions_table;
                $joins['LEFT JOIN']["`ntas_softwareversions` AS `$softwareversions_table`"] = [
                    'ON' => [
            ...
            $items_softwareversions_table = "ntas_items_softwareversions_{$alias_suffix}";
            if (!in_array($items_softwareversions_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $items_softwareversions_table;
                $joins['LEFT JOIN']["`ntas_items_softwareversions` AS `$items_softwareversions_table`"] = [
                    'ON' => [
        ...
        if ($to_type === Software::class && in_array($from_referencetype, $CFG_GLPI['software_types'], true)) {
            ...
            $items_softwareversions_table = "ntas_items_softwareversions{$alias_suffix}";
            if (!in_array($items_softwareversions_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $items_softwareversions_table;
                $joins['LEFT JOIN']["`ntas_items_softwareversions` AS `$items_softwareversions_table`"] = [
                    'ON' => [
            ...
            $softwarelicenses_table = "ntas_softwarelicenses{$alias_suffix}";
            if (!in_array($softwarelicenses_table, $already_link_tables2, true)) {
                $already_link_tables2[] = $softwarelicenses_table;
                $joins['LEFT JOIN']["`ntas_softwarelicenses` AS `$softwarelicenses_table`"] = [
                    'ON' => [
                        $to_table_alias => 'id',
        ...
        if ($from_referencetype === Budget::class && in_array($to_type, $CFG_GLPI['infocom_types'], true)) {
            ...
            $infocom_alias = "ntas_infocoms{$alias_suffix}";
            if (!in_array($infocom_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $infocom_alias;
                $joins['LEFT JOIN']["`ntas_infocoms` AS `$infocom_alias`"] = [
                    'ON' => [
                        $from_table => 'id',
        ...
        if ($from_referencetype === Reservation::class && in_array($to_type, $CFG_GLPI['reservation_types'], true)) {
            ...
            $reservationitems_alias = "ntas_reservationitems{$alias_suffix}";
            if (!in_array($reservationitems_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $reservationitems_alias;
                $joins['LEFT JOIN']["`ntas_reservationitems` AS `$reservationitems_alias`"] = [
                    'ON' => [
        ...
        if ($to_type === Group::class && in_array($from_referencetype, $CFG_GLPI['assignable_types'], true)) {
            $relation_table_alias = 'ntas_groups_items' . $alias_suffix;
            if (!in_array($relation_table_alias, $already_link_tables2, true)) {
                $already_link_tables2[] = $relation_table_alias;
                $joins['LEFT JOIN']["`ntas_groups_items` AS `$relation_table_alias`"] = [
                    'ON' => [
        ...
        if ($from_obj && $from_obj->isField($to_fk)) {
            ...
            // ntas_$from_items table exists and has a foreign key corresponding to $to_table
            ...
            // ntas_$to_items table exists and has a foreign key corresponding to $from_table
    ...
    public static function getDropdownTranslationJoinCriteria($alias, $table, $itemtype, $field): array
    {
        ...
        return [
            'LEFT JOIN' => [
                "ntas_dropdowntranslations AS $alias" => [
                    'ON' => [
                        $alias => 'itemtype',
    ...
    public static function getHavingCriteria(string $LINK, bool $NOT, string $itemtype, int $ID, string $searchtype, string $val): array
    {
        ...
        //// Default cases
        // Link with plugin tables
        if (preg_match("/^ntas_plugin_([a-z0-9]+)/", $table, $matches)) {
            if (count($matches) === 2) {
                $plug     = $matches[1];
                $out = Plugin::doOneHook(
                    $plug,
    ...
    public static function getOrderByCriteria(string $itemtype, array $sort_fields): array
    {
        ...
        foreach ($sort_fields as $sort_field) {
                    ...
                    case "ntas_users.name":
                        if ($itemtype != User::class) {
                            ...
                            if (
                                ...
                                && in_array($searchopt[$ID]['joinparams']['beforejoin']['table'], ['ntas_tickets_users', 'ntas_changes_users', 'ntas_problems_users'])
                            ) { // For tickets_users
            ...
            // Link with plugin tables
            if ($criterion === null && preg_match("/^ntas_plugin_([a-z0-9]+)/", $table, $matches)) {
                if (count($matches) == 2) {
                    $plug = $matches[1];
                    $out = Plugin::doOneHook(
                        $plug,
        ...
        if (isset($CFG_GLPI["union_search_type"][$data['itemtype']])) {
            ...
            foreach ($CFG_GLPI[$CFG_GLPI["union_search_type"][$data['itemtype']]] as $ctype) {
                ...
                if (
                    ...
                    // SOFTWARE HACK
                    if ($ctype == 'Software') {
                        $tmpquery = str_replace("`ntas_softwares`.`serial`", "''", $tmpquery);
                        $tmpquery = str_replace("`ntas_softwares`.`otherserial`", "''", $tmpquery);
                    }
    ...
    public static function giveItem(
        ...
        if (isset($so["table"])) {
            ...
            switch ($table . '.' . $field) {
                        ...
                        for ($k = 0; $k < $data[$ID]['count']; $k++) {
                            ...
                            if (
                                ...
                                $text = sprintf(
                                    ...
                                    htmlescape(Dropdown::getDropdownName('ntas_profiles', $data[$ID][$k]['profiles_id']))
                                );
                                $comp = '';
                                if ($data[$ID][$k]['is_recursive']) {
                                    $comp = __('R');
                ...
                case "ntas_tickets_tickets.tickets_id_1":
                    ...
                    for ($k = 0; $k < $data[$ID]['count']; $k++) {
                        ...
                        if ((is_int($linkid) && $linkid > 0) && !isset($displayed[$linkid])) {
                            $link_text = Dropdown::getDropdownName('ntas_tickets', $linkid);
                ...
                case "ntas_tickets.time_to_resolve":
                case "ntas_problems.time_to_resolve":
                case "ntas_changes.time_to_resolve":
                case "ntas_tickets.time_to_own":
                case "ntas_tickets.internal_time_to_own":
                case "ntas_tickets.internal_time_to_resolve":
                    // Due date + progress
                    if (in_array($orig_id, [151, 158, 181, 186])) {
                        ...
                        if (
                            ...
                            if (
                                $table . '.' . $field == "ntas_tickets.time_to_resolve"
                                || $table . '.' . $field == "ntas_tickets.internal_time_to_resolve"
                            ) {
                                return '';
                            } else {
                                $color = '#AAAAAA';
                        ...
                        switch ($table . "." . $field) {
                            ...
                            case "ntas_changes.time_to_resolve":
                                ...
                                $is_late = !empty($value)
                                    ...
                                    && (
                                        $solve_date > $value
                                        || ($solve_date == null && $value < $_SESSION['ntas_currenttime'])
                                    );
                                break;
                            case "ntas_tickets.time_to_own":
                            case "ntas_tickets.internal_time_to_own":
                                ...
                                $is_late = !empty($value)
                                    ...
                                    && (
                                        $tia_date > $value
                                        || ($tia_date == null && $value < $_SESSION['ntas_currenttime'])
                                    );
                        }
                        if ($is_late) {
                ...
                case "ntas_reservationitems.comment":
                    if (empty($data[$ID][0]['name'])) {
                        $text = __s('None');
                    } else {
                ...
                case 'ntas_crontasks.description':
                    $tmp = new CronTask();
                ...
                case 'ntas_items_tickets.items_id':
                case 'ntas_items_problems.items_id':
                case 'ntas_changes_items.items_id':
                case 'ntas_certificates_items.items_id':
                case 'ntas_appliances_items.items_id':
                    if (!empty($data[$ID])) {
                        $items = [];
                ...
                case 'ntas_items_tickets.itemtype':
                case 'ntas_items_problems.itemtype':
                    if (!empty($data[$ID])) {
                        $itemtypes = [];
                        foreach ($data[$ID] as $key => $val) {
                            if (is_numeric($key)) {
                ...
                case 'ntas_tickets.name':
                case 'ntas_problems.name':
                case 'ntas_changes.name':
                    if (
                        isset($data[$ID][0]['id'])
                ...
                case 'ntas_ticketsatisfactions.satisfaction':
                    if ($html_output) {
                ...
                case 'ntas_changesatisfactions.satisfaction':
                    if ($html_output) {
                ...
                case 'ntas_projects._virtual_planned_duration':
                    return \htmlescape(
                ...
                case 'ntas_projects._virtual_effective_duration':
                    return \htmlescape(
                        Html::timestampToString(
                ...
                case 'ntas_cartridgeitems._virtual':
                ...
                case 'ntas_printers._virtual':
                    return Cartridge::getCountForPrinter(
                        $data["id"],
                        !$html_output
                    );

                case 'ntas_consumableitems._virtual':
                ...
                case 'ntas_reservationitems._virtual':
                ...
                case "ntas_certificates.date_expiration":
                    if (
                        ...
                        $color = ($date < $_SESSION['ntas_currenttime']) ? '#cf9b9b' : null;
                        if ($before) {
                            $before = date('Y-m-d', strtotime($_SESSION['ntas_currenttime'] . "+$before days"));
                            $color = match (true) {
                                $date < $_SESSION['ntas_currenttime'] => '#d63939',
                                $date < $before => '#de5d06',
                                $date >= $before => '#a1cf66',
                                default => null
                            };
                    ...
                    break;
                case "ntas_domains.date_expiration":
                    if (!empty($data[$ID][0]['name'])
                        && ($data[$ID][0]['name'] < $_SESSION['ntas_currenttime'])
                    ) {
                        return "<div class='badge_block' style='border-color: #cf9b9b'>
        ...
        if (isset($table) && isset($field)) {
            if (preg_match("/^ntas_plugin_([a-z0-9]+)/", $table . '.' . $field, $matches)) {
                if (count($matches) == 2) {
                    $plug     = $matches[1];
                    $out = Plugin::doOneHook(
                        $plug,
