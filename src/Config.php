...
class Config extends CommonDBTM
{
    ...
    public function prepareInputForUpdate($input)
    {
        ...
        if ($tfa_grace_days_changed || $tfa_enforced_changed) {
            ...
            if ($enforced && $grace_period > 0) {
                $input['2fa_grace_date_start'] = $_SESSION['ntas_currenttime'];
            } else {
     ...
     *
     * @param array $data data (CFG_GLPI for global config / ntas_users fields for user prefs)
     *
     * @return void
     */
    ...
    public static function loadLegacyConfiguration()
    {
        ...
        if (
            ...
            || !$DB->tableExists('ntas_configs')
        ) {
            return false;
        }

        $iterator = $DB->request(['FROM' => 'ntas_configs']);
    ...
    public function post_updateItem($history = true)
    {
        ...
        if (
            ...
            $DB->update(
                User::getTable(),
                ['password_last_update' => $_SESSION['ntas_currenttime']],
    ...
    private function logConfigChange(string $context, string $name, string $newvalue, string $oldvalue): void
    {
        ...
        if ($ntas_key->isConfigSecured($context, $name)) {
            $newvalue = $oldvalue = '********';
        }
     ...
     *
     * Used as a hacky workaround when we require a valid ntas_configs record for rights checks.
