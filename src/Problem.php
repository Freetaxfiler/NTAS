...
class Problem extends CommonITILObject implements DefaultSearchRequestInterface
{
        ...
        if (static::canView()) {
            switch ($item::class) {
                ...
                case User::class:
                    ...
                    if ($_SESSION['glpishow_count_on_tabs']) {
                        $nb = countElementsInTable(
                            ['ntas_problems', 'ntas_problems_users'],
                            [
                                'ntas_problems_users.problems_id'  => new QueryExpression(DBmysql::quoteName('ntas_problems.id')),
                                'ntas_problems_users.users_id'    => $item->getID(),
                                'ntas_problems_users.type'        => CommonITILActor::REQUESTER,
                                'ntas_problems.is_deleted'        => 0,
                ...
                case Group::class:
                    ...
                    if ($_SESSION['glpishow_count_on_tabs']) {
                        $nb = countElementsInTable(
                            ['ntas_problems', 'ntas_groups_problems'],
                            [
                                'ntas_groups_problems.problems_id' => new QueryExpression(DBmysql::quoteName('ntas_problems.id')),
                                'ntas_groups_problems.groups_id'  => $item->getID(),
                                'ntas_groups_problems.type'       => CommonITILActor::REQUESTER,
                                'ntas_problems.is_deleted'        => 0,
                            ] + getEntitiesRestrictCriteria(self::getTable())
                        );
                    }
    ...
    public static function showCentralList($start, $status = "process", $showgroupproblems = true)
    {
        ...
        $search_users_id = [
            'ntas_problems_users.users_id'   => Session::getLoginUserID(),
            'ntas_problems_users.type'       => CommonITILActor::REQUESTER,
        ];
        $search_assign = [
            'ntas_problems_users.users_id'   => Session::getLoginUserID(),
            'ntas_problems_users.type'       => CommonITILActor::ASSIGN,
        ];
        ...
        if ($showgroupproblems) {
            ...
            if (count($_SESSION['glpigroups'])) {
                $search_users_id = [
                    'ntas_groups_problems.groups_id' => $_SESSION['glpigroups'],
                    'ntas_groups_problems.type'      => CommonITILActor::REQUESTER,
                ];
                $search_assign = [
                    'ntas_groups_problems.groups_id' => $_SESSION['glpigroups'],
                    'ntas_groups_problems.type'      => CommonITILActor::ASSIGN,
                ];
            }
        }
        ...
        $criteria = [
            'SELECT'          => ['ntas_problems.id'],
            'DISTINCT'        => true,
            'FROM'            => 'ntas_problems',
            'LEFT JOIN'       => [
                'ntas_problems_users'   => [
                    'ON' => [
                        'ntas_problems_users'   => 'problems_id',
                        'ntas_problems'         => 'id',
                    ],
                ],
                'ntas_groups_problems'  => [
                    'ON' => [
                        'ntas_groups_problems'  => 'problems_id',
                        'ntas_problems'         => 'id',
                    ],
                ],
            ],
            'WHERE'           => $WHERE + getEntitiesRestrictCriteria('ntas_problems'),
    ...
    public static function showCentralCount(bool $foruser = false, bool $display = true)
    {
        ...
        if ($foruser) {
            ...
            $WHERE = ['ntas_problems_users.users_id' => Session::getLoginUserID()];

            if (
                isset($_SESSION["glpigroups"])
                && count($_SESSION["glpigroups"])
            ) {
                $criteria['LEFT JOIN']['ntas_groups_problems'] = [
                    'ON' => [
                        'ntas_groups_problems'  => 'problems_id',
                        $table                  => 'id', [
                            'AND' => [
                ...
                $WHERE['ntas_groups_problems.groups_id'] = $_SESSION['glpigroups'];
            }
            $criteria['WHERE'][] = ['OR' => $WHERE];
        }

        $deleted_criteria = $criteria;
        $criteria['WHERE']['ntas_problems.is_deleted'] = 0;
        $deleted_criteria['WHERE']['ntas_problems.is_deleted'] = 1;
        $iterator = $DB->request($criteria);
    ...
    public static function getListForItemRestrict(CommonDBTM $item)
    {
        ...
        switch (true) {
            case $item instanceof User:
                $restrict['ntas_problems_users.users_id'] = $item->getID();
                $restrict['ntas_problems_users.type'] = CommonITILActor::REQUESTER;
                break;

            case $item instanceof Supplier:
                $restrict['ntas_problems_suppliers.suppliers_id'] = $item->getID();
                $restrict['ntas_problems_suppliers.type'] = CommonITILActor::ASSIGN;
                break;
            ...
            case $item instanceof Group:
                ...
                $restrict['ntas_groups_problems.groups_id'] = ($tree ? getSonsOf('ntas_groups', $item->getID()) : $item->getID());
                $restrict['ntas_groups_problems.type'] = CommonITILActor::REQUESTER;
                break;

            default:
                $restrict['ntas_items_problems.items_id'] = $item->getID();
                $restrict['ntas_items_problems.itemtype'] = $item->getType();
                // you can only see your tickets
                if (!Session::haveRight(self::$rightname, self::READALL)) {
                    $or = [
                        'ntas_problems.users_id_recipient'   => Session::getLoginUserID(),
                        [
                            'AND' => [
                                'ntas_problems_users.problems_id'  => 'ntas_problems.id',
                                'ntas_problems_users.users_id'    => Session::getLoginUserID(),
                            ],
                        ],
                    ];
                    if (count($_SESSION['glpigroups'])) {
                        $or['ntas_groups_problems.groups_id'] = $_SESSION['glpigroups'];
                    }
                    $restrict[] = ['OR' => $or];
                }
        }
    ...
    public function getActiveProblemsForItem($itemtype, $items_id)
    {
        ...
        return $DB->request([
            ...
            'LEFT JOIN' => [
                'ntas_items_problems' => [
                    'ON' => [
                        'ntas_items_problems' => 'problems_id',
                        $this->getTable()    => 'id',
                    ],
                ],
            ],
            'WHERE'     => [
                'ntas_items_problems.itemtype'   => $itemtype,
                'ntas_items_problems.items_id'   => $items_id,
