    ...
    ExtraVisibilityCriteria
{
    ...
    public static function getVisibilityCriteria(bool $forceall = false): array
    {
        if (!Session::haveRight(self::$rightname, READ)) {
            return [
                'WHERE' => ['ntas_reminders.users_id' => Session::getLoginUserID()],
            ];
        }
        ...
        // Users
        $join['ntas_reminders_users'] = [
            'FKEY' => [
                'ntas_reminders_users'  => 'reminders_id',
                'ntas_reminders'        => 'id',
            ],
        ];
        ...
        if (Session::getLoginUserID()) {
            $where['OR'] = [
                'ntas_reminders.users_id'        => Session::getLoginUserID(),
                'ntas_reminders_users.users_id'  => Session::getLoginUserID(),
            ];
        } else {
            $where = [
                0,
        ...
        if (
            ...
            $join['ntas_groups_reminders'] = [
                'FKEY' => [
                    'ntas_groups_reminders' => 'reminders_id',
                    'ntas_reminders'        => 'id',
                ],
            ];

            $or = ['ntas_groups_reminders.no_entity_restriction' => 1];
            ...
            $where['OR'][] = [
                'ntas_groups_reminders.groups_id' => count($_SESSION["glpigroups"])
                                                      ? $_SESSION["glpigroups"]
                                                      : [-1],
                'OR' => $or,
            ];
        ...
        if (
            ...
            $join['ntas_profiles_reminders'] = [
                'FKEY' => [
                    'ntas_profiles_reminders'  => 'reminders_id',
                    'ntas_reminders'           => 'id',
                ],
            ];

            $or = ['ntas_profiles_reminders.no_entity_restriction' => 1];
            $restrict = getEntitiesRestrictCriteria(
            ...
            $where['OR'][] = [
                'ntas_profiles_reminders.profiles_id' => $_SESSION["glpiactiveprofile"]['id'],
                'OR' => $or,
            ];
        }
        ...
        if (
            ...
            $join['ntas_entities_reminders'] = [
                'FKEY' => [
                    'ntas_entities_reminders'  => 'reminders_id',
                    'ntas_reminders'           => 'id',
                ],
            ];
        }
        if (isset($_SESSION["glpiactiveentities"]) && count($_SESSION["glpiactiveentities"])) {
            $restrict = getEntitiesRestrictCriteria('ntas_entities_reminders', '', '', true, true);
            if (count($restrict)) {
                $where['OR'] += $restrict;
            }
        }
    ...
    final public static function getListCriteria(): array
    {
        ...
        $visibility_criteria = [
            [
                'OR' => [
                    ['ntas_reminders.begin_view_date' => null],
                    ['ntas_reminders.begin_view_date' => ['<', $now]],
                ],
            ], [
                'OR' => [
                    ['ntas_reminders.end_view_date'   => null],
                    ['ntas_reminders.end_view_date'   => ['>', $now]],
                ],
            ],
        ];

        $personal_criteria = [
            'SELECT' => ['ntas_reminders.*'],
            'FROM'   => 'ntas_reminders',
            'WHERE'  => array_merge([
                'ntas_reminders.users_id'  => $users_id,
                [
                    'OR'        => [
                        'end'          => ['>=', $today],
                        'is_planned'   => 0,
        ...
        $public_criteria = array_merge_recursive(
            [
                ...
                'ORDERBY'         => 'ntas_reminders.name',
            ],
            self::getVisibilityCriteria()
        );
        // Do not force the inclusion of reminders created by the current user
        unset($public_criteria['WHERE']['ntas_reminders.users_id'], $public_criteria['WHERE']['OR']['ntas_reminders.users_id']);

        if (countElementsInTable('ntas_remindertranslations') > 0) {
            $additional_criteria = [
                'SELECT'    => ["ntas_remindertranslations.name AS transname", "ntas_remindertranslations.text AS transtext"],
                'LEFT JOIN' => [
                    'ntas_remindertranslations' => [
                        'ON'  => [
                            'ntas_reminders'             => 'id',
                            'ntas_remindertranslations'  => 'reminders_id', [
                                'AND'                            => [
                                    'ntas_remindertranslations.language' => $_SESSION['glpilanguage'],
                                ],
                            ],
                        ],
                    ],
