...
TWIG, $p);
    ...
    public static function cronPlanningRecall($task = null)
    {
        ...
        $cron_status = 0;
        $iterator = $DB->request([
            'SELECT'    => 'ntas_planningrecalls.*',
            'FROM'      => 'ntas_planningrecalls',
            'LEFT JOIN' => [
                'ntas_alerts'  => [
                    'ON' => [
                        'ntas_planningrecalls'  => 'id',
                        'ntas_alerts'           => 'items_id', [
                            'AND' => [
                                'ntas_alerts.itemtype'  => 'PlanningRecall',
                                'ntas_alerts.type'      => Alert::ACTION,
                            ],
                        ],
                    ],
                ],
            ...
            'WHERE'     => [
                'NOT'                         => ['ntas_planningrecalls.when' => null],
                'ntas_planningrecalls.when'   => ['<', QueryFunction::now()],
