...
final class Transfer extends CommonDBTM
{
    ...
    private function getDestinationEntityAncestors(): array
    {
        if ($this->to_entity_ancestors === null) {
            $this->to_entity_ancestors = getAncestorsOf("ntas_entities", $this->to);
        }
    ...
    private function simulateSoftware(): void
    {
        ...
        // Clean DB
        $DB->delete('ntas_items_softwareversions', ['ntas_softwareversions.id'  => null], [
            'LEFT JOIN' => [
                'ntas_softwareversions'  => [
                    'ON' => [
                        'ntas_items_softwareversions' => 'softwareversions_id',
                        'ntas_softwareversions'       => 'id',
                    ],
                ],
            ],
        ]);
        ...
        // Clean DB
        $DB->delete('ntas_softwareversions', ['ntas_softwares.id'  => null], [
            'LEFT JOIN' => [
                'ntas_softwares'  => [
                    'ON' => [
                        'ntas_softwareversions' => 'softwares_id',
                        'ntas_softwares'        => 'id',
                    ],
                ],
            ],
        ...
        foreach ($CFG_GLPI['software_types'] as $itemtype) {
            ...
            // Clean DB
            $DB->delete('ntas_items_softwareversions', [
                "{$itemtable}.id"  => null,
                'ntas_items_softwareversions.itemtype' => $itemtype,
            ], [
                'LEFT JOIN' => [
                    $itemtable  => [
                        'ON' => [
                            'ntas_items_softwareversions' => 'items_id',
                            $itemtable                    => 'id',
                        ],
                    ],
                ],
            ...
            if ($this->haveItemsToTransfer($itemtype)) {
                $iterator = $DB->request([
                    'SELECT'       => [
                        ...
                        'ntas_softwares.is_recursive',
                        'ntas_softwareversions.id AS vID',
                    ],
                    'FROM'         => 'ntas_items_softwareversions',
                    'INNER JOIN'   => [
                        'ntas_softwareversions' => [
                            'ON' => [
                                'ntas_items_softwareversions' => 'softwareversions_id',
                    ...
                    'WHERE'        => [
                        'ntas_items_softwareversions.items_id' => $this->needtobe_transfer[$itemtype],
                        'ntas_items_softwareversions.itemtype' => $itemtype,
                    ],
                ]);
    ...
    private function simulateCertificates(): void
    {
        ...
        foreach ($CFG_GLPI["certificate_types"] as $itemtype) {
            ...
            $DB->delete(
                ...
                [
                    "$itemtable.id"                 => null,
                    "ntas_certificates_items.itemtype" => $itemtype,
                ],
                [
                    'LEFT JOIN' => [
                        $itemtable  => [
                            'ON' => [
                                'ntas_certificates_items'  => 'items_id',
                                $itemtable              => 'id',
                            ],
                        ],
                    ],
            ...
            $DB->delete(
                'ntas_certificates_items',
                [
                    'ntas_certificates.id'  => null,
                ],
                [
                    'LEFT JOIN' => [
                        'ntas_certificates'  => [
                            'ON' => [
                                'ntas_certificates_items'  => 'certificates_id',
                                'ntas_certificates'        => 'id',
                            ],
                        ],
                    ],
                ]
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    ...
                    'ntas_certificates.is_recursive',
                ],
                'FROM'      => 'ntas_certificates_items',
                'LEFT JOIN' => [
                    'ntas_certificates' => [
                        'ON' => [
                            'ntas_certificates_items'  => 'certificates_id',
    ...
    private function simulateContracts(): void
    {
        ...
        foreach ($CFG_GLPI["contract_types"] as $itemtype) {
            ...
            $DB->delete(
                ...
                [
                    "$itemtable.id"                 => null,
                    "ntas_contracts_items.itemtype" => $itemtype,
                ],
                [
                    'LEFT JOIN' => [
                        $itemtable  => [
                            'ON' => [
                                'ntas_contracts_items'  => 'items_id',
                                $itemtable              => 'id',
                            ],
                        ],
                    ],
            ...
            // Clean DB
            $DB->delete('ntas_contracts_items', ['ntas_contracts.id'  => null], [
                'LEFT JOIN' => [
                    'ntas_contracts'  => [
                        'ON' => [
                            'ntas_contracts_items'  => 'contracts_id',
                            'ntas_contracts'        => 'id',
                        ],
                    ],
                ],
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    ...
                    'ntas_contracts.is_recursive',
                ],
                'FROM'      => 'ntas_contracts_items',
                'LEFT JOIN' => [
                    'ntas_contracts' => [
                        'ON' => [
                            'ntas_contracts_items'  => 'contracts_id',
        ...
        if ($this->haveItemsToTransfer(Ticket::class)) {
            // Clean DB
            $DB->delete('ntas_tickets_contracts', ['ntas_contracts.id'  => null], [
                'LEFT JOIN' => [
                    'ntas_contracts'  => [
                        'ON' => [
                            'ntas_tickets_contracts'  => 'contracts_id',
                            'ntas_contracts'          => 'id',
                        ],
                    ],
                ],
            ...
            $DB->delete('ntas_tickets_contracts', ['ntas_tickets.id'  => null], [
                'LEFT JOIN' => [
                    'ntas_tickets'  => [
                        'ON' => [
                            'ntas_tickets_contracts'  => 'tickets_id',
                            'ntas_tickets'            => 'id',
                        ],
                    ],
                ],
    ...
    private function simulateSuppliers(): void
    {
        ...
        // Clean DB
        $DB->delete('ntas_contracts_suppliers', ['ntas_contracts.id'  => null], [
            'LEFT JOIN' => [
                'ntas_contracts'  => [
                    'ON' => [
                        'ntas_contracts_suppliers' => 'contracts_id',
                        'ntas_contracts'           => 'id',
                    ],
                ],
            ],
        ...
        // Clean DB
        $DB->delete('ntas_contracts_suppliers', ['ntas_suppliers.id'  => null], [
            'LEFT JOIN' => [
                'ntas_suppliers'  => [
                    'ON' => [
                        'ntas_contracts_suppliers' => 'suppliers_id',
                        'ntas_suppliers'           => 'id',
                    ],
                ],
            ],
        ...
        if ($this->haveItemsToTransfer(Contract::class)) {
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    ...
                    'ntas_suppliers.is_recursive',
                ],
                'FROM'      => 'ntas_contracts_suppliers',
                'LEFT JOIN' => [
                    'ntas_suppliers' => [
                        'ON' => [
                            'ntas_contracts_suppliers' => 'suppliers_id',
        ...
        if ($this->options['keep_infocom']) {
            foreach (Infocom::getItemtypesThatCanHave() as $itemtype) {
                ...
                $DB->delete(
                    ...
                    [
                        "$itemtable.id"  => null,
                        'ntas_infocoms.itemtype' => $itemtype,
                    ],
                    [
                        'LEFT JOIN' => [
                            $itemtable => [
                                'ON' => [
                                    'ntas_infocoms'   => 'items_id',
                                    $itemtable        => 'id',
                                ],
                            ],
                        ],
    ...
    private function simulateContacts(): void
    {
        ...
        // Clean DB
        $DB->delete('ntas_contacts_suppliers', ['ntas_contacts.id'  => null], [
            'LEFT JOIN' => [
                'ntas_contacts' => [
                    'ON' => [
                        'ntas_contacts_suppliers'  => 'contacts_id',
                        'ntas_contacts'            => 'id',
                    ],
                ],
            ],
        ...
        // Clean DB
        $DB->delete('ntas_contacts_suppliers', ['ntas_suppliers.id'  => null], [
            'LEFT JOIN' => [
                'ntas_suppliers' => [
                    'ON' => [
                        'ntas_contacts_suppliers'  => 'suppliers_id',
                        'ntas_suppliers'           => 'id',
                    ],
                ],
            ],
        ...
        if ($this->haveItemsToTransfer(Supplier::class)) {
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    ...
                    'ntas_contacts.is_recursive',
                ],
                'FROM'      => 'ntas_contacts_suppliers',
                'LEFT JOIN' => [
                    'ntas_contacts'  => [
                        'ON' => [
                            'ntas_contacts_suppliers'  => 'contacts_id',
    ...
    private function simulateDocuments(): void
    {
        ...
        foreach (Document::getItemtypesThatCanHave() as $itemtype) {
            ...
            $DB->delete(
                ...
                [
                    "$itemtable.id"  => null,
                    'ntas_documents_items.itemtype' => $itemtype,
                ],
                [
                    'LEFT JOIN' => [
                        $itemtable => [
                            'ON' => [
                                'ntas_documents_items'  => 'items_id',
                                $itemtable              => 'id',
                            ],
                        ],
                    ],
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    ...
                    'ntas_documents.is_recursive',
                ],
                'FROM'      => 'ntas_documents_items',
                'LEFT JOIN' => [
                    'ntas_documents'  => [
                        'ON' => [
        ...
        if (count($iterator)) {
            ...
            foreach ($iterator as $data) {
                ...
                if ($this->options['keep_cartridgeitem']) {
                    ...
                    if ($ID == $newID) {
                        $DB->delete('ntas_cartridges', ['printers_id' => $ID]);
                    }
                    $need_clean_process = true;
                }
    ...
    private function transferItemSoftwares(string $itemtype, int $ID): void
    {
        ...
        foreach ($iterator as $data) {
            if ($this->options['keep_software']) {
                ...
                $DB->delete('ntas_items_softwareversions', ['id' => $data['id']]);
            }
        }
        ...
        if ($this->options['keep_software']) {
            $iterator = $DB->request([
                'SELECT' => 'id',
                'FROM'   => 'ntas_items_softwarelicenses',
                'WHERE'  => [
                    'items_id'  => $ID,
                    'itemtype'  => $itemtype,
                ],
            ...
            $DB->delete('ntas_items_softwarelicenses', [
                'items_id'  => $ID,
                'itemtype'  => $itemtype,
            ]);
        }
    ...
    private function cleanSoftwareVersions(): void
    {
        ...
        foreach (array_keys($this->already_transfer[SoftwareVersion::class]) as $old) {
            if (
                (countElementsInTable("ntas_softwarelicenses", ['softwareversions_id_buy' => $old]) === 0)
                && (countElementsInTable("ntas_softwarelicenses", ['softwareversions_id_use' => $old]) === 0)
    ...
    public function cleanSoftwares()
    {
        ...
        foreach (array_keys($this->already_transfer[Software::class]) as $old) {
            if (
                (countElementsInTable("ntas_softwarelicenses", ['softwares_id' => $old]) == 0)
                && (countElementsInTable("ntas_softwareversions", ['softwares_id' => $old]) == 0)
            ) {
                ...
                if ($ID == $newID) {
                    if ($item_ID != $newdocID) {
                        ...
                        if ($existing['cpt'] > 0) {
                            ...
                            $DB->delete('ntas_documents_items', ['id' => $data['id']]);
                        } else {
    ...
    private function transferTickets(string $itemtype, int $ID, int $newID): void
    {
        ...
        $iterator = $DB->request([
            'SELECT'    => [
                'ntas_tickets.*',
                'ntas_items_tickets.id AS _relid',
            ],
            'FROM'      => 'ntas_tickets',
            'LEFT JOIN' => [
                'ntas_items_tickets' => [
    ...
    private function transferCompatiblePrinters(int $ID, int $newID): void
    {
        ...
        $iterator = $DB->request([
            ...
            'FROM'   => 'ntas_cartridgeitems_printermodels',
    ...
    private function transferNetworkLink(string $itemtype, int $ID, int $newID): void
    {
        ...
        $iterator = $DB->request([
            ...
            'FROM'      => 'ntas_networkports',
            'LEFT JOIN' => [
                'ntas_networkportethernets'   => [
                    'ON' => [
                        'ntas_networkportethernets'   => 'networkports_id',
                        'ntas_networkports'           => 'id',
                    ],
                ],
            ],
            'WHERE'     => [
                'ntas_networkports.items_id'  => $ID,
                'ntas_networkports.itemtype'  => $itemtype,
            ],
        ]);
