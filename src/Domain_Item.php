        ...
        if ($canedit) {
            $twig_params = [
                ...
                'entity_restrict' => $domain->fields['is_recursive']
                    ? getSonsOf('ntas_entities', $domain->fields['entities_id'])
            ...
            echo TemplateRenderer::getInstance()->renderFromStringTemplate(<<<TWIG
                ...
                <div class="mb-3">
                    <form name="domain_form{{ rand }}" id="domain_form{{ rand }}" method="post"
                        ...
                        {{ inputs.hidden('_ntas_csrf_token', csrf_token()) }}
...
TWIG, $twig_params);
        ...
        foreach ($iterator as $data) {
            ...
            $linked_criteria = [
                'SELECT' => [
                    "$itemTable.*",
                    'ntas_domains_items.id AS items_id',
                    'ntas_domains_items.domainrelations_id',
                    'ntas_entities.id AS entity',
                ],
                'FROM'   => self::getTable(),
                'INNER JOIN'   => [
                    $itemTable  => [
            ...
            foreach ($linked_iterator as $linked_data) {
                ...
                if (Session::isMultiEntitiesMode()) {
                    if (!isset($entity_names[$linked_data['entity']])) {
                        $entity_names[$linked_data['entity']] = Dropdown::getDropdownName("ntas_entities", $linked_data['entity']);
                    }
                    $entry['entity'] = $entity_names[$linked_data['entity']];
                }
                if (!isset($relation_names[$linked_data['domainrelations_id']])) {
                    $relation_names[$linked_data['domainrelations_id']] = Dropdown::getDropdownName("ntas_domainrelations", $linked_data['domainrelations_id']);
                }
    ...
    public static function getForItem(CommonDBTM $item): DBmysqlIterator
    {
        ...
        $criteria = [
            'SELECT'    => [
                'ntas_domains_items.id AS assocID',
                'ntas_domains_items.domainrelations_id',
                'ntas_domains_items.is_deleted',
                'ntas_domains_items.is_dynamic',
                'ntas_entities.id AS entity',
                'ntas_domains.name AS assocName',
                'ntas_domains.*',
            ...
            'ORDER'     => 'assocName',
            'GROUPBY' => [
                'ntas_domains_items.id',
            ],
        ];

        if ($item instanceof DomainRelation) {
            $criteria['WHERE'] = ['ntas_domains_items.domainrelations_id' => $item->getID()];
        } else {
            $criteria['WHERE'] = [
                'ntas_domains_items.itemtype' => $item::class,
                'ntas_domains_items.items_id' => $item->getID(),
            ];
        }
        ...
        $criteria['WHERE']
            ...
            += [
                'OR'  => [
                    'AND' => [
                        "ntas_domains_items.is_deleted" => 0,
                        "ntas_domains_items.is_dynamic" => 1,
                    ],
                    "ntas_domains_items.is_dynamic" => 0,
                ],
            ];
    ...
    public static function showForItem(CommonDBTM $item, $withtemplate = 0)
    {
        ...
        if (
            ...
            if ($item->isEntityAssign()) {
                ...
                if ($item->isRecursive()) {
                    $entities = getSonsOf('ntas_entities', $entity);
                } else {
                    $entities = $entity;
                }
            }
...
TWIG, $twig_params);
        ...
        foreach ($domains as $data) {
            ...
            if (Session::isMultiEntitiesMode() && !isset($entity_names[$data['entity']])) {
                $entity_names[$data['entity']] = Dropdown::getDropdownName(table: "ntas_entities", id: $data['entity'], default: '');
            }
            ...
            foreach ($groups as $group) {
                if (!isset($group_names[$group])) {
                    $group_names[$group] = Dropdown::getDropdownName(table: "ntas_groups", id: (int) $group, default: '');
                }
            ...
            if (!isset($type_names[$data['domaintypes_id']])) {
                $type_names[$data['domaintypes_id']] = Dropdown::getDropdownName(table: "ntas_domaintypes", id: $data['domaintypes_id'], default: '');
            }
            if (!$item instanceof DomainRelation && !isset($relation_names[$data['domainrelations_id']])) {
                $relation_names[$data['domainrelations_id']] = Dropdown::getDropdownName(table: "ntas_domainrelations", id: $data['domainrelations_id'], default: '');
            }
