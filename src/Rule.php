...
TWIG, $twig_params);
    ...
    final public function getDataForList(bool $display_criteria, bool $display_actions, bool $display_entity, bool $can_edit): array
    {
        ...
        if ($display_entity) {
            $entname = htmlescape(Dropdown::getDropdownName('ntas_entities', $this->fields['entities_id']));
    ...
    private function handleRankChange($new_rule = false)
    {
        ...
        // not share the same ntas_rules tables which is used by the `moveRule`
        // method.
        if (static::getTable() !== "ntas_rules") {
            return;
        }
        ...
        if (isset($action['type'])) {
            switch ($action['type']) {
                ...
                case "dropdown_groups_validate":
                    $name = Dropdown::getDropdownName('ntas_groups', $value);
                    return $name == '' ? NOT_AVAILABLE : $name;
...
TWIG, ['label' => $this->getTitle()]);
     ...
     * @param string     $table      ntas_ruleactions, ntas_rulescriterias or ntas_slalevelcriterias
                    ...
                    if ($_SESSION['glpishow_count_on_tabs']) {
                        ...
                        if (count($types)) {
                            $nb = countElementsInTable(
                                ['ntas_rules', 'ntas_ruleactions'],
                                [
                                    'ntas_ruleactions.rules_id'   => new QueryExpression(DBmysql::quoteName('ntas_rules.id')),
                                    'ntas_rules.sub_type'         => $types,
                                    'ntas_ruleactions.field'      => 'entities_id',
                                    'ntas_ruleactions.value'      => $item->getID(),
                                ]
                            );
                        }
                    }
    ...
    public static function getUuid()
    {
        ...
        // encode script current dir, ex : /var/www/ntas_X
