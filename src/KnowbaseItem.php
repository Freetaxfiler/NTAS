...
class KnowbaseItem extends CommonDBVisible implements ExtraVisibilityCriteria, ServiceCatalogLeafInterface, TreeBrowseInterface
{
    ...
    private static function getVisibilityCriteriaCommonJoin(bool $forceall = false)
    {
        ...
        if ($forceall || Session::getLoginUserID()) {
            $join['ntas_knowbaseitems_users'] = [
                'ON' => [
                    'ntas_knowbaseitems_users' => 'knowbaseitems_id',
        ...
        if ($forceall || $has_session_groups) {
            $join['ntas_groups_knowbaseitems'] = [
                'ON' => [
                    'ntas_groups_knowbaseitems' => 'knowbaseitems_id',
                    'ntas_knowbaseitems'       => 'id',
                ],
            ];
        }
        ...
        if ($forceall || $has_active_profile) {
            $join['ntas_knowbaseitems_profiles'] = [
                'ON' => [
                    'ntas_knowbaseitems_profiles' => 'knowbaseitems_id',
                    'ntas_knowbaseitems'       => 'id',
                ],
            ];
        }
        ...
        if ($forceall || $has_active_entity || $is_public_faq_context) {
            $join['ntas_entities_knowbaseitems'] = [
                'ON' => [
                    'ntas_entities_knowbaseitems' => 'knowbaseitems_id',
                    'ntas_knowbaseitems'       => 'id',
                ],
            ];
        }
    ...
    public function showFull($options = [])
    {
        ...
        foreach ($categories as $category) {
            ...
            $fullcategoryname = getTreeValueCompleteName('ntas_knowbaseitemcategories', $knowbaseitemcategories_id);
    ...
    public function searchForm($options)
    {
        ...
        echo TemplateRenderer::getInstance()->renderFromStringTemplate(<<<TWIG
            ...
            <form method="get" action="{{ 'KnowbaseItem'|itemtype_search_path }}" class="d-flex justify-content-center">
                ...
                {% endif %}
                {{ inputs.hidden('ntas_csrf_token', csrf_token()) }}
            </form>
TWIG, $twig_params);
    }
    ...
    public static function getListRequest(array $params, $type = 'search')
    {
        ...
        $criteria = [
            'SELECT' => [
                'ntas_knowbaseitems.*',
                new QueryExpression(
                    QueryFunction::count('ntas_knowbaseitems_users.id') . ' + '
                    . QueryFunction::count('ntas_groups_knowbaseitems.id') . ' + '
                    . QueryFunction::count('ntas_knowbaseitems_profiles.id') . ' + '
                    . QueryFunction::count('ntas_entities_knowbaseitems.id') . ' AS '
        ...
        switch ($type) {
            ...
            default:
                ...
                if (Session::getLoginUserID()) {
                    ...
                    // Anonymous access
                    if (Session::isMultiEntitiesMode()) {
                        $criteria['WHERE']['ntas_entities_knowbaseitems.entities_id'] = 0;
                        $criteria['WHERE']['ntas_entities_knowbaseitems.is_recursive'] = 1;
                    }
                }
                break;
        }
        ...
        if ($params['faq']) { // helpdesk
            $criteria['WHERE'][] = [
                'OR' => [
                    'ntas_knowbaseitems.is_faq' => 1,
                    'ntas_knowbaseitems_users.users_id' => Session::getLoginUserID(),
                ],
            ];
        }
        ...
        if (countElementsInTable('ntas_knowbaseitemtranslations') > 0) {
            $criteria['LEFT JOIN']['ntas_knowbaseitemtranslations'] = [
                'ON'  => [
                    'ntas_knowbaseitems'             => 'id',
                    'ntas_knowbaseitemtranslations'  => 'knowbaseitems_id', [
                        'AND'                            => [
                            'ntas_knowbaseitemtranslations.language' => $_SESSION['glpilanguage'],
                        ],
                    ],
                ],
            ];
            $criteria['SELECT'][] = 'ntas_knowbaseitemtranslations.name AS transname';
            $criteria['SELECT'][] = 'ntas_knowbaseitemtranslations.answer AS transanswer';
        }

        // a search with $contains
        switch ($type) {
            case 'allmy':
                $criteria['WHERE']['ntas_knowbaseitems.users_id'] = Session::getLoginUserID();
                break;

            case 'myunpublished':
                $criteria['WHERE']['ntas_knowbaseitems.users_id'] = Session::getLoginUserID();
                $criteria['WHERE']['ntas_entities_knowbaseitems.entities_id'] = null;
                $criteria['WHERE']['ntas_knowbaseitems_profiles.profiles_id'] = null;
                $criteria['WHERE']['ntas_groups_knowbaseitems.groups_id'] = null;
                $criteria['WHERE']['ntas_knowbaseitems_users.users_id'] = null;
                break;
            ...
            case 'search':
                if ((string) $params["contains"] !== '') {
                    ...
                    if (countElementsInTable('ntas_knowbaseitemtranslations') > 0) {
                        $addscore = [
                            'ntas_knowbaseitemtranslations.name',
                            'ntas_knowbaseitemtranslations.answer',
                        ];
                    }

                    $expr = "(MATCH(" . $DB->quoteName('ntas_knowbaseitems.name') . ", " . $DB->quoteName('ntas_knowbaseitems.answer') . ")
                    ...
                    $ors = [
                        new QueryExpression(
                            "MATCH(" . $DB->quoteName('ntas_knowbaseitems.name') . ",
                        " . $DB->quoteName('ntas_knowbaseitems.answer') . ")
                        AGAINST(" . $DB->quote($search_wilcard) . " IN BOOLEAN MODE)"
                        ),
                    ];
                    ...
                    $visibility_crit = [
                        [
                            'OR'  => [
                                ['ntas_knowbaseitems.begin_date'  => null],
                                ['ntas_knowbaseitems.begin_date'  => ['<', QueryFunction::now()]],
                            ],
                        ], [
                            'OR'  => [
                                ['ntas_knowbaseitems.end_date'    => null],
                                ['ntas_knowbaseitems.end_date'    => ['>', QueryFunction::now()]],
                            ],
                        ],
                    ];
                    ...
                    if ($numrows_search <= 0) {// not result this fulltext try with alternate search
                        ...
                        $ors = [
                            ["ntas_knowbaseitems.name"     => ['LIKE', Search::makeTextSearchValue($contains)]],
                            ["ntas_knowbaseitems.answer"   => ['LIKE', Search::makeTextSearchValue($contains)]],
                        ];
                        if (countElementsInTable('ntas_knowbaseitemtranslations') > 0) {
                            $ors[] = ["ntas_knowbaseitemtranslations.name"   => ['LIKE', Search::makeTextSearchValue($contains)]];
                            $ors[] = ["ntas_knowbaseitemtranslations.answer" => ['LIKE', Search::makeTextSearchValue($contains)]];
                        }
            ...
            case 'browse':
                ...
                $criteria['ORDERBY'] = ['ntas_knowbaseitems.name ASC'];
                break;
        }
    ...
    public static function showRecentPopular(string $type = "", bool $display = true)
    {
        ...
        $criteria = [
            'SELECT'    => ['ntas_knowbaseitems' => ['id', 'name', 'is_faq']],
            'DISTINCT'  => true,
            'FROM'      => self::getTable(),
            'WHERE'     => [],
            'LIMIT'     => 10,
        ...
        $criteria['WHERE'][] = [
            'NOT'  => [
                'ntas_entities_knowbaseitems.entities_id' => null,
                'ntas_knowbaseitems_profiles.profiles_id' => null,
                'ntas_groups_knowbaseitems.groups_id'     => null,
                'ntas_knowbaseitems_users.users_id'       => null,
            ],
        ];
        ...
        if ($faq) { // FAQ
            $criteria['WHERE']['ntas_knowbaseitems.is_faq'] = 1;
        }
...
TWIG, $twig_params);
    ...
    public function rawSearchOptions()
    {
        ...
        $tab[] = [
            'id'                 => '79',
            'table'              => 'ntas_knowbaseitemcategories',
            'field'              => 'completename',
        ...
        $tab[] = [
            'id'                 => '13',
            'table'              => 'ntas_knowbaseitems_items',
            'field'              => 'items_id',
    ...
    public static function getForCategory($category_id, $kbi = null)
    {
        ...
        $ids = $DB->request([
            ...
            'FROM'   => self::getTable(),
            'LEFT JOIN' => [
                'ntas_knowbaseitems_knowbaseitemcategories' => [
                    'ON'  => [
                        'ntas_knowbaseitems_knowbaseitemcategories'  => 'knowbaseitems_id',
                        'ntas_knowbaseitems'             => 'id',
                    ],
                ],
            ],
            'WHERE'  => ['ntas_knowbaseitems_knowbaseitemcategories.knowbaseitemcategories_id' => $category_id],
        ]);
