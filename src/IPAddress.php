...
class IPAddress extends CommonDBChild
{
    ...
    public static function countForItem(CommonDBTM $item)
    {
        ...
        switch ($item->getType()) {
            case 'IPNetwork':
                $result = $DB->request([
                    'COUNT'  => 'cpt',
                    'FROM'   => 'ntas_ipaddresses_ipnetworks',
                    'WHERE'  => [
    ...
    public function setAddressFromString($address, $itemtype = "", $items_id = -1)
    {
        ...
        if ($binary !== null && $binary !== false) {
            ...
            // ntas_ipaddresses table and to make canonical textual version
    ...
    private static function getCriteriaLinkedToNetwork(IPNetwork $network): array
    {
        ...
        $main_criteria = [
            ...
            'FROM'         => 'ntas_ipaddresses_ipnetworks AS LINK',
            'INNER JOIN'   => [
                'ntas_ipaddresses AS ADDR' => [
                    'ON' => [
                        'ADDR'   => 'id',
                        'LINK'   => 'ipaddresses_id', [
                            'AND' => [
        ...
        $criteria['INNER JOIN']['ntas_ipaddresses AS ADDR']['ON'][0]['AND']['ADDR.itemtype'] = ['!=', 'NetworkName'];
        $queries[] = $criteria;
