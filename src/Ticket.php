...
class Ticket extends CommonITILObject implements DefaultSearchRequestInterface
{
                ...
                switch (get_class($item)) {
                    case User::class:
                        $nb = countElementsInTable(
                            ['ntas_tickets', 'ntas_tickets_users'],
                            [
                                'ntas_tickets_users.tickets_id'  => new QueryExpression(DBmysql::quoteName('ntas_tickets.id')),
                                'ntas_tickets_users.users_id'    => $item->getID(),
                                'ntas_tickets_users.type'        => CommonITILActor::REQUESTER,
                                'ntas_tickets.is_deleted'        => 0,
                    ...
                    case Supplier::class:
                        $nb = countElementsInTable(
                            ['ntas_tickets', 'ntas_suppliers_tickets'],
                            [
                                'ntas_suppliers_tickets.tickets_id'    => new QueryExpression(DBmysql::quoteName('ntas_tickets.id')),
                                'ntas_suppliers_tickets.suppliers_id'  => $item->getID(),
                                'ntas_tickets.is_deleted'              => 0,
                            ] + getEntitiesRestrictCriteria(self::getTable())
                        );
                        break;
                    ...
                    case Group::class:
                        $nb = countElementsInTable(
                            ['ntas_tickets', 'ntas_groups_tickets'],
                            [
                                'ntas_groups_tickets.tickets_id' => new QueryExpression(DBmysql::quoteName('ntas_tickets.id')),
                                'ntas_groups_tickets.groups_id'  => $item->getID(),
                                'ntas_groups_tickets.type'       => CommonITILActor::REQUESTER,
                                'ntas_tickets.is_deleted'        => 0,
                            ] + getEntitiesRestrictCriteria(self::getTable())
                        );
                        $title = __('Created tickets');
    ...
    public function olaAffect($type, &$input, $manual_olas_id)
    {
        ...
        if (isset($this->fields['id']) && $this->fields['id'] > 0) {
            ...
            if (
                ...
                $date = $_SESSION['ntas_currenttime'];
    ...
    public function pre_updateInDB()
    {
        ...
        if (
            ...
            $this->fields['takeintoaccountdate']        = $_SESSION["ntas_currenttime"];
        ...
        if (
            ...
            if (($calendars_id > 0) && $calendar->getFromDB($calendars_id)) {
                return max(1, $calendar->getActiveTimeBetween(
                    $this->fields['date'],
                    $_SESSION["ntas_currenttime"]
                ));
            }
            // Not calendar defined
            return max(1, strtotime($_SESSION["ntas_currenttime"]) - strtotime($this->fields['date']));
        }
        return 0;
    }
    ...
    public function countActiveTicketsForItem($itemtype, $items_id)
    {
        ...
        $result = $DB->request([
            ...
            'LEFT JOIN' => [
                'ntas_items_tickets' => [
                    'ON' => [
                        'ntas_items_tickets' => 'tickets_id',
                        $this->getTable()    => 'id',
                    ],
                ],
            ],
            'WHERE'     => [
                'ntas_items_tickets.itemtype' => $itemtype,
                'ntas_items_tickets.items_id' => $items_id,
                'NOT'                         => [
            ...
            if (
                ...
                $this->update(
                    [
                        ...
                        'takeintoaccountdate'        => $_SESSION["ntas_currenttime"],
                        '_disablenotif'              => true,
                    ]
                );
            }
...
JAVASCRIPT;
    ...
    public function rawSearchOptions()
    {
        ...
        $tab[] = [
            ...
            'condition'          => ['ntas_slas.type' => SLM::TTO],
        ];
        ...
        $tab[] = [
            ...
            'condition'          => ['ntas_slas.type' => SLM::TTR],
        ];

        $tab[] = [
            'id'                 => '32',
            'table'              => 'ntas_slalevels',
            'field'              => 'name',
        ...
        $tab[] = [
            ...
            'condition'          => ['ntas_olas.type' => SLM::TTO],
        ];
        ...
        $tab[] = [
            ...
            'condition'          => ['ntas_olas.type' => SLM::TTR],
        ];

        $tab[] = [
            'id'                 => '192',
            'table'              => 'ntas_olalevels',
            'field'              => 'name',
        ...
        switch ($field) {
            ...
            case '_virtual_age':
                ...
                if ($calendars_id) {
                    ...
                    $time = $calendar->getActiveTimeBetween($values['date'], $_SESSION["ntas_currenttime"]);
                } else {
                    $ticket_date = new DateTime($values['date']);
                    $now = new DateTime($_SESSION["ntas_currenttime"]);
    ...
    public static function computeTco(CommonDBTM $item)
    {
        ...
        $iterator = $DB->request([
            'SELECT'    => 'ntas_ticketcosts.*',
            'FROM'      => 'ntas_ticketcosts',
            'LEFT JOIN' => [
                'ntas_items_tickets' => [
                    'ON' => [
                        'ntas_items_tickets' => 'tickets_id',
                        'ntas_ticketcosts'   => 'tickets_id',
                    ],
                ],
            ],
            'WHERE'     => [
                'ntas_items_tickets.itemtype' => get_class($item),
                'ntas_items_tickets.items_id' => $item->getField('id'),
                'OR'                          => [
                    'ntas_ticketcosts.cost_time'     => ['>', 0],
                    'ntas_ticketcosts.cost_fixed'    => ['>', 0],
                    'ntas_ticketcosts.cost_material' => ['>', 0],
                ],
            ],
        ]);
    ...
    public static function isCategoryValid(array $input): bool
    {
        ...
        if ($cat->getFromDB($input['itilcategories_id'])) {
            ...
            if (
                ...
                && !(
                    $cat->isRecursive()
                    && in_array($input['entities_id'], getSonsOf('ntas_entities', $cat->fields['entities_id']))
                )
            ) {
                return false;
            }
    ...
    public static function showCentralList($start, $status = "process", bool $showgrouptickets = true, bool $display = true)
    {
        ...
        $SELECT = ['ntas_tickets.id', 'ntas_tickets.date_mod'];
        $JOINS = [];
        $WHERE = [
            'ntas_tickets.is_deleted' => 0,
        ];
        $search_users_id = [
            'ntas_tickets_users.users_id' => Session::getLoginUserID(),
            'ntas_tickets_users.type'     => CommonITILActor::REQUESTER,
        ];
        $search_assign = [
            'ntas_tickets_users.users_id' => Session::getLoginUserID(),
            'ntas_tickets_users.type'     => CommonITILActor::ASSIGN,
        ];
        $search_observer = [
            'ntas_tickets_users.users_id' => Session::getLoginUserID(),
            'ntas_tickets_users.type'     => CommonITILActor::OBSERVER,
        ];
        ...
        if ($showgrouptickets) {
            ...
            if (count($_SESSION['glpigroups'])) {
                $search_assign = [
                    'ntas_groups_tickets.groups_id'  => $_SESSION['glpigroups'],
                    'ntas_groups_tickets.type'       => CommonITILActor::ASSIGN,
                ];
                ...
                if (Session::haveRight(self::$rightname, self::READGROUP)) {
                    ...
                    $search_observer = [
                        'ntas_groups_tickets.groups_id' => $_SESSION['glpigroups'],
                        'ntas_groups_tickets.type'      => CommonITILActor::OBSERVER,
                    ];
                }
            }
        }
        ...
        switch ($status) {
            case "waiting": // waiting tickets
                $WHERE = array_merge(
                    $WHERE,
                    $search_assign,
                    ['ntas_tickets.status' => self::WAITING]
                );
                break;
            ...
            case "process": // planned or assigned or incoming tickets
                $WHERE = array_merge(
                    $WHERE,
                    $search_assign,
                    ['ntas_tickets.status' => array_merge(self::getProcessStatusArray(), [self::INCOMING])]
                );
            ...
            case "toapprove": //tickets waiting for approval
                ...
                if (!$showgrouptickets &&  Session::haveRight('ticket', Ticket::SURVEY)) {
                    $ORWHERE[] = ['ntas_tickets.users_id_recipient' => Session::getLoginUserID()];
                }
                $WHERE[] = ['OR' => $ORWHERE];
                $WHERE['ntas_tickets.status'] = self::SOLVED;
                break;
            ...
            case "tovalidate": // tickets waiting for validation
                $JOINS['LEFT JOIN'] = [
                    'ntas_ticketvalidations' => [
                        'ON' => [
                            'ntas_ticketvalidations'   => 'tickets_id',
                            'ntas_tickets'             => 'id',
                        ],
                    ],
                ];
                $WHERE = array_merge(
                    ...
                    [
                        ...
                        'ntas_ticketvalidations.status'  => CommonITILValidation::WAITING,
                        'ntas_tickets.global_validation' => CommonITILValidation::WAITING,
                        'NOT'                            => [
                            'ntas_tickets.status'   => [self::SOLVED, self::CLOSED],
                        ],
                    ]
                );
                break;
            ...
            case "rejected": //old ambiguous key
                $WHERE = array_merge(
                    ...
                    $search_assign,
                    [
                        'ntas_tickets.status'            => ['<>', self::CLOSED],
                        'ntas_tickets.global_validation' => CommonITILValidation::REFUSED,
                    ]
                );
                break;
            ...
            case "solution.rejected": // tickets with rejected solution
                $subq = new QuerySubQuery([
                    ...
                    'FROM'   => 'ntas_itilsolutions AS last_solution',
                    'WHERE'  => [
                        'last_solution.items_id'   => new QueryExpression($DB->quoteName('ntas_tickets.id')),
                ...
                $WHERE = array_merge(
                    ...
                    [
                        'ntas_tickets.status'         => ['<>', self::CLOSED],
                        'ntas_itilsolutions.status'   => CommonITILValidation::REFUSED,
                    ]
                );
                break;
            case "observed":
            ...
            case "survey": // tickets for which the satisfaction survey has not been completed and is still valid
                $SELECT[] = 'ntas_tickets.entities_id';
                $SELECT[] = 'ntas_entities.inquest_config';
                $SELECT[] = 'ntas_ticketsatisfactions.date_begin';
                $JOINS['INNER JOIN'] = [
                    'ntas_ticketsatisfactions' => [
                        'ON' => [
                            'ntas_ticketsatisfactions' => 'tickets_id',
                            'ntas_tickets'             => 'id',
                        ],
                    ],
                    'ntas_entities'            => [
                    ...
                    [
                        ...
                        new QueryExpression(
                            QueryFunction::dateDiff(
                                ...
                                expression2: 'ntas_tickets.closedate'
                            ) . ' <= ' . Entity::MAX_INQUEST_DURATION_DAYS
                        ),
                        [
                            'OR' => [
                                [
                                    'ntas_tickets.entities_id' => ['<>', 0], // Root entity never inherits
                                    'ntas_entities.inquest_config' => Entity::CONFIG_PARENT, // We need to resolve the inquest_duration in PHP
                                ],
                                'ntas_entities.inquest_duration' => 0,
                                new QueryExpression(
                                    QueryFunction::dateDiff(
                                        expression1: QueryFunction::dateAdd(
                                            date: 'ntas_ticketsatisfactions.date_begin',
                                            interval: new QueryExpression($DB::quoteName('ntas_entities.inquest_duration')),
                                            interval_unit: 'DAY'
                                        ),
                        ...
                        'ntas_ticketsatisfactions.date_answered'  => null,
                    ]
                );
                break;
        ...
        $criteria = [
            ...
            'LEFT JOIN'       => [
                'ntas_tickets_users'    => [
                    'ON' => [
                        'ntas_tickets_users' => 'tickets_id',
                        'ntas_tickets'       => 'id',
                    ],
                ],
                'ntas_groups_tickets'   => [
                    'ON' => [
                        'ntas_groups_tickets'   => 'tickets_id',
                        'ntas_tickets'          => 'id',
                    ],
                ],
            ],
            'WHERE'           => $WHERE + getEntitiesRestrictCriteria('ntas_tickets'),
            'ORDERBY'         => 'ntas_tickets.date_mod DESC',
        ];
        if (count($JOINS)) {
        ...
        if ($status === 'survey') {
            ...
            foreach ($results as $k => $result) {
                ...
                $is_valid = $duration_cache[$entities_id] === 0
                    || (strtotime($result['date_begin']) + $duration_cache[$entities_id] * DAY_TIMESTAMP) > strtotime($_SESSION['ntas_currenttime']);
                if (!$is_valid) {
                        ...
                        if (
                            ...
                            foreach ($job->groups[CommonITILActor::REQUESTER] as $d) {
                                $requesters[] = '<i class="fs-4 ti ti-users text-muted me-1"></i>'
                                    . htmlescape(Dropdown::getDropdownName("ntas_groups", $d["groups_id"]));
                            }
                        }
    ...
    public static function showCentralCount(bool $foruser = false, bool $display = true)
    {
        ...
        $criteria['WHERE']['ntas_tickets.is_deleted'] = 0;
        $deleted_criteria['WHERE']['ntas_tickets.is_deleted'] = 1;
    ...
    public static function showVeryShort($ID, $forcetab = '')
    {
        ...
        if ($job->getFromDBwithData($ID)) {
            ...
            if (
                ...
                foreach ($job->groups[CommonITILActor::REQUESTER] as $d) {
                    echo htmlescape(Dropdown::getDropdownName("ntas_groups", $d["groups_id"]));
                    echo "<br>";
                }
            }
    ...
    public static function getCommonCriteria()
    {
        ...
        $criteria['LEFT JOIN']['ntas_tickettasks'] = [
            'ON' => [
                self::getTable()     => 'id',
                'ntas_tickettasks'   => 'tickets_id',
            ],
        ];

        $criteria['LEFT JOIN']['ntas_ticketvalidations'] = [
            'ON' => [
                self::getTable()         => 'id',
                'ntas_ticketvalidations' => 'tickets_id',
            ],
        ...
        foreach ($entities as $entity) {
            ...
            if ($delay >= 0) {
                ...
                if ($nb) {
                    $tot += $nb;
                    $task->addVolume($nb);
                    $task->log(Dropdown::getDropdownName('ntas_entities', $entity['id']) . " : $nb");
                }
            }
        }
        ...
        if (isset($field_id_or_search_options['linkfield'])) {
            switch ($field_id_or_search_options['linkfield']) {
                case 'requesttypes_id':
                    if (isset($field_id_or_search_options['joinparams']) && Toolbox::in_array_recursive('ntas_itilfollowups', $field_id_or_search_options['joinparams'])) {
    ...
    public static function buildCanViewCondition($fieldID)
    {
        ...
        if (
            ...
            FROM `ntas_ticketvalidations`
            WHERE (`itemtype_target` = 'User' AND `items_id_target` = '$user')
                OR (`itemtype_target` = 'Group' AND `items_id_target` IN (SELECT `ntas_groups_users`.`groups_id` FROM `ntas_groups_users` WHERE `ntas_groups_users`.`users_id` = '$user'))";
    ...
    public static function getCriteriaFromProfile()
    {
        ...
        $join_profile  = [];
        if ($users) {
            $join_profile['ntas_tickets_users AS tu'] = [
                'ON' => [
                    'tu'           => 'tickets_id',
                    'ntas_tickets' => 'id',
                ],
        ...
        if ($groups) {
            $join_profile['ntas_groups_tickets AS gt'] = [
                'ON' => [
                    'gt'           => 'tickets_id',
                    'ntas_tickets' => 'id',
                ],
        ...
        if ($valid) {
            $join_profile['ntas_ticketvalidations'] = [
                'ON' => [
                    'ntas_ticketvalidations' => 'tickets_id',
                    'ntas_tickets' => 'id',
                ],
    ...
    public static function getListForItemRestrict(CommonDBTM $item)
    {
        ...
        switch (true) {
            case $item instanceof User:
                $restrict['ntas_tickets_users.users_id'] = $item->getID();
                $restrict['ntas_tickets_users.type'] = CommonITILActor::REQUESTER;
                break;
            ...
            case $item instanceof Supplier:
                $restrict['ntas_suppliers_tickets.suppliers_id'] = $item->getID();
                $restrict['ntas_suppliers_tickets.type'] = CommonITILActor::ASSIGN;
                break;
            ...
            case $item instanceof Group:
                ...
                $restrict['ntas_groups_tickets.groups_id'] = ($tree ? getSonsOf('ntas_groups', $item->getID()) : $item->getID());
                $restrict['ntas_groups_tickets.type'] = CommonITILActor::REQUESTER;
                break;

            default:
                $restrict['ntas_items_tickets.items_id'] = $item->getID();
                $restrict['ntas_items_tickets.itemtype'] = $item->getType();
                ...
                if (!Session::haveRight(self::$rightname, self::READALL)) {
                    $or = [
                        ...
                        [
                            'AND' => [
                                'ntas_tickets_users.tickets_id'  => new QueryExpression('ntas_tickets.id'),
                                'ntas_tickets_users.users_id'    => Session::getLoginUserID(),
                            ],
                        ],
                    ];
                    if (Session::haveRightsOr(TicketValidation::$rightname, [TicketValidation::VALIDATEINCIDENT, TicketValidation::VALIDATEREQUEST])) {
                        $or[] = [
                            'AND' => [
                                'ntas_ticketvalidations.tickets_id'        => new QueryExpression('ntas_tickets.id'),
                                'ntas_ticketvalidations.itemtype_target'   => User::class,
                                'ntas_ticketvalidations.items_id_target' => Session::getLoginUserID(),
                            ],
                        ];
                    }
                    if (count($_SESSION['glpigroups'])) {
                        $or['ntas_groups_tickets.groups_id'] = $_SESSION['glpigroups'];
                    }
                    $restrict[] = ['OR' => $or];
                }
        }
