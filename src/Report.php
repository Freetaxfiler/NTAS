...
TWIG, $twig_params);
    ...
    private static function getOSInstallCounts(): array
    {
        ...
        $iterator = $DB->request([
            'SELECT'    => [
                'COUNT' => '* AS count',
                'ntas_operatingsystems.id AS id',
                'ntas_operatingsystems.name AS name',
            ],
            ...
            'LEFT JOIN' => [
                'ntas_operatingsystems' => [
                    'ON' => [
                        ...
                        'ntas_operatingsystems'       => 'id',
                    ],
                ],
            ],
            'WHERE'     => ['is_deleted' => 0] + getEntitiesRestrictCriteria('ntas_items_operatingsystems'),
            'GROUPBY'   => 'ntas_operatingsystems.name',
        ]);
...
TWIG, ['title' => $report['title'], 'counts' => $counts]);
    ...
    private static function getNetworkCommonCriteria(): array
    {
        ...
        return [
            ...
            'LEFT JOIN'    => [
                ...
                'ntas_networkports_networkports AS LINK'  => [
                    'ON'  => [
    ...
    private static function getNetworkLocationCriteria(int $locations_id): array
    {
        ...
        if ($locations_id > 0) {
            $sons = getSonsOf('ntas_locations', $locations_id);
            $criteria['WHERE'] = ['ntas_locations.id' => $sons];
        }
        $criteria['SELECT'] = array_merge($criteria['SELECT'], ['ntas_locations.completename AS extra']);
        $criteria['FROM'] = 'ntas_locations';
        $criteria['INNER JOIN']['ntas_sockets'] = [
            'ON' => [
                'ntas_sockets' => 'locations_id',
                'ntas_locations' => 'id',
            ],
        ];
        $criteria['INNER JOIN']['ntas_networkportethernets'] = [
            'ON' => [
                'ntas_networkportethernets' => 'networkports_id',
                'ntas_sockets' => 'networkports_id',
            ],
        ];
        $criteria['INNER JOIN']['ntas_networkports AS PORT_1'] = [
            'ON' => [
                'PORT_1' => 'id',
                'ntas_networkportethernets' => 'networkports_id',
            ],
        ];
        $criteria['ORDER'] = ['ntas_locations.completename', 'PORT_1.name'];
    ...
    {
        $criteria = self::getNetworkCommonCriteria();
        $criteria['SELECT'] = array_merge($criteria['SELECT'], ['ntas_sockets.name AS extra']);
        $criteria['FROM'] = 'ntas_sockets';
        $criteria['INNER JOIN']['ntas_networkportethernets'] = [
            'ON' => [
                'ntas_networkportethernets' => 'networkports_id',
                'ntas_sockets' => 'networkports_id',
        ...
        $criteria['LEFT JOIN']['ntas_locations'] = [
            'ON' => [
                'ntas_locations' => 'id',
                'ntas_sockets' => 'locations_id',
            ],
        ];
        $criteria['WHERE'] = ['ntas_sockets.id' => $sockets_id];
        return $criteria;
    }
    ...
    {
        $criteria = self::getNetworkCommonCriteria();
        $criteria['FROM'] = 'ntas_networkequipments AS ITEM';
        $criteria['INNER JOIN']['ntas_networkports AS PORT_1'] = [
            'ON' => [
                'PORT_1' => 'items_id',
                'ITEM' => 'id', [
        ...
        foreach ($itemtypes as $key => $itemtype) {
            ...
            $criteria = [
                'SELECT'    => [
                    ...
                    'ntas_contracttypes.id AS contracttypeID',
                    'ntas_contracttypes.name AS contracttypename',
                    'ntas_infocoms.buy_date',
                    'ntas_infocoms.warranty_duration',
                    'ntas_contracts.begin_date',
                    'ntas_contracts.duration',
                    'ntas_entities.completename AS entname',
                    'ntas_entities.id AS entID',
                ],
                'FROM'      => $itemtable,
                'LEFT JOIN' => [],
                'WHERE'     => [],
            ...
            if ($itemtype !== Project::class) {
                $location_field      = "ntas_locations.completename";
                $criteria['LEFT JOIN']['ntas_locations'] = [
                    'ON'  => [
            ...
            if ($itemtype === SoftwareLicense::class) {
                $deleted_field       = "ntas_softwares.is_deleted";
                $location_field      = null;
                $criteria['LEFT JOIN']['ntas_softwares'] = [
                    'ON'  => [
                        'ntas_softwares'        => 'id',
                        'ntas_softwarelicenses' => 'softwares_id',
                    ],
                ];
                $criteria['WHERE']['ntas_softwares.is_template'] = 0;
            }
            ...
            $criteria['SELECT'][] = ($location_field !== null
                ? "ntas_locations.id AS locationID"
            ...
            $criteria['LEFT JOIN'] += [
                ...
                'ntas_contracts'        => [
                    'ON'  => [
                        ...
                        'ntas_contracts'        => 'id', [
                            'AND' => [
                                'NOT' => ['ntas_contracts_items.contracts_id' => null],
                            ],
                        ],
                    ],
                ],
            ...
            foreach ($years as $val2) {
                $ors[] = new QueryExpression(QueryFunction::year('ntas_infocoms.buy_date') . " = " . $DB->quote($val2));
                $ors[] = new QueryExpression(QueryFunction::year('ntas_contracts.begin_date') . " = " . $DB->quote($val2));
            }
            if (count($ors)) {
                $criteria['WHERE'][] = [
                    'OR'  => $ors,
            ...
            if ($itemtype === Project::class || $itemtype === SoftwareLicense::class) {
                if ($itemtype === SoftwareLicense::class) {
                    ...
                    $criteria['SELECT'] = array_merge(
                        $criteria['SELECT'],
                        ['ntas_infocoms.buy_date', 'ntas_infocoms.warranty_duration']
                    );
                    $criteria['LEFT JOIN']['ntas_infocoms'] = [
                        'ON'  => [
                            'ntas_infocoms' => 'items_id',
                            $itemtable     => 'id', [
                                'AND' => [
                ...
                if ($DB->fieldExists($itemtable, 'locations_id')) {
                    $criteria['SELECT'][] = 'ntas_locations.completename AS locationname';
                    $criteria['SELECT'][] = 'ntas_locations.id AS locationID';
                    $criteria['LEFT JOIN']['ntas_locations'] = [
                        'ON'  => [
                            $itemtable        => 'locations_id',
                            'ntas_locations'   => 'id',
                ...
                $ors = [];
                foreach ($years as $val2) {
                    $ors[] = new QueryExpression(QueryFunction::year('ntas_infocoms.buy_date') . ' = ' . $DB::quoteValue($val2));
                    $ors[] = new QueryExpression(QueryFunction::year('ntas_contracts.begin_date') . ' = ' . $DB::quoteValue($val2));
                }
                if (count($ors)) {
                    $criteria['WHERE'][] = ['OR' => $ors];
                }
