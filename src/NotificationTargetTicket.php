...
class NotificationTargetTicket extends NotificationTargetCommonITILObject
{
    ...
    public function getDataForObject(CommonDBTM $item, array $options, $simple = false)
    {
        ...
        if ($requesttype_id = $item->getField('requesttypes_id')) {
            $data['##ticket.requesttype##'] = Dropdown::getDropdownName('ntas_requesttypes', $requesttype_id);
        }
                ...
                if (
                    ...
                    if ($hardware->isField('locations_id')) {
                        ...
                        if ($h_locations_id = $hardware->getField('locations_id')) {
                            $tmp['##ticket.item.location##'] = Dropdown::getDropdownName('ntas_locations', $h_locations_id);
                        }
                        $locations = new Location();
                    ...
                    if ($hardware->isField('groups_id')) {
                        ...
                        if ($h_group_id = $hardware->getField('groups_id')) {
                            $tmp['##ticket.item.group##'] = Dropdown::getDropdownName('ntas_groups', $h_group_id);
                        }
                    }
        ...
        if (!$simple) {
            ...
            $problems          = getAllDataFromTable('ntas_problems_tickets', $restrict);
            $data['problems'] = [];
            if (count($problems)) {
                $problem = new Problem();
            ...
            $changes          = getAllDataFromTable('ntas_changes_tickets', $restrict);
            $data['changes'] = [];
            if (count($changes)) {
                $change = new Change();
            ...
            if (isset($options['validation_id']) && $options['validation_id']) {
                $restrict['ntas_ticketvalidations.id'] = $options['validation_id'];
            }
