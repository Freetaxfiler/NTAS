...
class Document_Item extends CommonDBRelation
{
    ...
    public function post_addItem()
    {
        if ($this->fields['itemtype'] === Ticket::class) {
            ...
            $input  = [
                ...
                'date_mod'        => $_SESSION["ntas_currenttime"],
    ...
    public static function showForDocument(Document $doc): bool
    {
        ...
        if ($canedit) {
            $twig_params = [
                'doc' => $doc,
                'entity_restrict' => $doc->fields['is_recursive'] ? getSonsOf('ntas_entities', $doc->fields['entities_id']) : $doc->fields['entities_id'],
...
TWIG, $twig_params);
    ...
    public static function showAddFormForItem(CommonDBTM $item, $withtemplate = 0, $options = [])
    {
        ...
        if (
            ...
            $count = $DB->request([
                ...
                'WHERE'     => [
                    'is_deleted' => 0,
                ] + getEntitiesRestrictCriteria('ntas_documents', '', $entities, true),
            ])->current();
            $nb = $count['cpt'];
    ...
    public static function showListForItem(CommonDBTM $item, $withtemplate = 0, $options = [])
    {
        ...
        if ($item::class === Document::class) {
            ...
            $reverse_criteria['LEFT JOIN']['ntas_documents']['ON']['ntas_documents_items'] = 'items_id';
            $reverse_criteria['WHERE'] = [
                'ntas_documents_items.documents_id' => $item->getID(),
                'ntas_documents_items.itemtype' => $item::class,
            ];
    ...
    protected static function getTypeItemsQueryParams($items_id, $itemtype, $noent = false, $where = [])
    {
        ...
        if ($itemtype !== KnowbaseItem::class) {
            ...
            if (!Session::getLoginUserID()) {
                // Anonymous access
                $kb_params['WHERE'] = [
                    'ntas_entities_knowbaseitems.entities_id'    => 0,
                    'ntas_entities_knowbaseitems.is_recursive'   => 1,
                ];
            }
    ...
    public static function getDocumentForItemRequest(CommonDBTM $item, array $order = []): array
    {
        $criteria = [
            ...
            'WHERE'     => [
                'ntas_documents_items.items_id'  => $item->getID(),
                'ntas_documents_items.itemtype'  => $item::class,
            ],
            'ORDERBY'   => $order,
        ];
        ...
        if (Session::getLoginUserID()) {
            $criteria['WHERE'] += getEntitiesRestrictCriteria('ntas_documents', '', '', true);
        } else {
            // Anonymous access from FAQ
            $criteria['WHERE']['ntas_documents.entities_id'] = 0;
        }
