...
class Dropdown
{
    ...
    public static function show($itemtype, $options = [])
    {
        ...
        if (
            ...
            if (is_array($params['entity'])) {
                ...
                $params['entity'] = getSonsOf('ntas_entities', $params['entity']);
            }
        }
    ...
    public static function getDropdownName($table, $id, $withcomment = false, $translate = true, $tooltip = true, string $default = '')
    {
        ...
        if ($id) {
            ...
            if ($translate && Session::haveTranslations(getItemTypeForTable($table), 'name')) {
                ...
                $JOIN = [
                    'LEFT JOIN' => [
                        'ntas_dropdowntranslations AS namet' => [
                            'ON' => [
                                'namet'  => 'items_id',
                                $table   => 'id', [
                                    'AND' => [
    ...
    public static function getDropdownComments(string $table, int $id, bool $translate = true, bool $tooltip = true): string
    {
        ...
        if (
            ...
            $criteria['LEFT JOIN']['ntas_dropdowntranslations AS comment_translations'] = [
                'ON' => [
        ...
        if (
            ...
            $criteria['LEFT JOIN']['ntas_dropdowntranslations AS completename_translations'] = [
                'ON' => [
            ...
            if (isset($condition['WHERE'])) {
                ...
                foreach ($condition as $key => $value) {
                    ...
                    if (is_array($value) && isset($value['WHERE'])) {
                        ...
                        // Ensure condition contains table name to prevent ambiguity with fields from `ntas_entities` table
        ...
        if ($item instanceof CommonTreeDropdown) {
            ...
            if ($item->isEntityAssign()) {
                ...
                if ($multi) {
                    ...
                    array_unshift($order, "ntas_entities.completename");
                }
            }
            ...
            if (Session::haveTranslations($post['itemtype'], 'completename')) {
                ...
                $ljoin['ntas_dropdowntranslations AS namet'] = [
                    'ON' => [
                        'namet'  => 'items_id',
                        $table   => 'id', [
                            'AND' => [
            ...
            if (Session::haveTranslations($post['itemtype'], 'name')) {
                ...
                $ljoin['ntas_dropdowntranslations AS namet2'] = [
                    'ON' => [
                        'namet2' => 'items_id',
                        $table   => 'id', [
                            'AND' => [
            ...
            if (Session::haveTranslations($post['itemtype'], 'comment')) {
                ...
                $ljoin['ntas_dropdowntranslations AS commentt'] = [
                    'ON' => [
                        'commentt'  => 'items_id',
                        $table      => 'id', [
                            'AND' => [
                    ...
                    if (
                        ...
                        if (!$firstitem) {
                            if ($prev >= 0) {
                                if (count($datastoadd)) {
                                    $datas[] = [
                                        'text'     => Dropdown::getDropdownName("ntas_entities", $prev),
                                        'children' => $datastoadd,
                                        'itemtype' => "Entity",
                                    ];
                                }
            ...
            if (!empty($post['searchText'])) {
                ...
                if ($post['itemtype'] == "SoftwareLicense") {
                    $orwhere['ntas_softwares.name'] = ['LIKE', $search];
                }
                ...
                case "SoftwareLicense":
                    $criteria = [
                        'SELECT' => [
                            "$table.*",
                            QueryFunction::concat(
                                params: ['ntas_softwares.name', new QueryExpression($DB::quoteValue(' - ')), 'ntas_softwarelicenses.name'],
                                alias: $field
                            ),
                        ],
                        'FROM'   => $table,
    ...
    public static function getDropdownMyDevices($post, $json = true)
    {
        ...
        if (Session::haveRight("show_group_hardware", 1)) {
            $iterator = $DB->request([
                ...
                'WHERE'     => [
                    'ntas_groups_users.users_id'  => $userID,
                ] + getEntitiesRestrictCriteria('ntas_groups', '', $entity_restrict, true),
            ]);

            $groups = [];
            if (count($iterator)) {
                foreach ($iterator as $data) {
                    $a_groups = getAncestorsOf("ntas_groups", $data["groups_id"]);
            ...
            foreach ($software_helpdesk_types as $itemtype) {
                if (isset($user_items[$itemtype]) && count($user_items[$itemtype])) {
                    $criteria = [
                        ...
                        'WHERE'        => [
                            'ntas_items_softwareversions.items_id' => $user_items[$itemtype],
                            'ntas_items_softwareversions.itemtype' => $itemtype,
                            'ntas_softwares.is_helpdesk_visible'   => 1,
                        ] + getEntitiesRestrictCriteria('ntas_softwares', '', $entity_restrict),
                        'ORDERBY'      => 'ntas_softwares.name',
                    ];
                    ...
                    if (isset($post['searchText']) && !empty($post['searchText'])) {
                        $search = ['LIKE', Search::makeTextSearchValue($post['searchText'])];
                        $orwhere = [
                            'ntas_softwares.name' => $search,
                            'ntas_softwareversions.name' => $search,
                        ];
                        if (is_numeric($post['searchText'])) {
                            $orwhere['ntas_softwares.id'] = $post['searchText'];
                        }
                        $criteria['WHERE'][] = ['OR' => $orwhere];
                    }
            ...
            foreach ($connected_types as $itemtype) {
                if (
                    ...
                    $criteria = [
                        'SELECT'          => "$itemtable.*",
                        'DISTINCT'        => true,
                        'FROM'            => 'ntas_assets_assets_peripheralassets',
                        'LEFT JOIN'       => [
                            $itemtable  => [
                                'ON' => [
                                    'ntas_assets_assets_peripheralassets' => 'items_id_peripheral',
                                    $itemtable                            => 'id',
                                ],
                            ],
                        ],
                        'WHERE'           => [
                            'ntas_assets_assets_peripheralassets.itemtype_peripheral' => $itemtype,
                            'ntas_assets_assets_peripheralassets.itemtype_asset'      => Computer::class,
                            'ntas_assets_assets_peripheralassets.items_id_asset'      => $computer_items,
