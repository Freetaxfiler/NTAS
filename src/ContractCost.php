    ...
    public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
    {
        ...
        if (
            ...
            if ($_SESSION['glpishow_count_on_tabs']) {
                $nb = countElementsInTable('ntas_contractcosts', ['contracts_id' => $item->getID()]);
            }
...
TWIG, $twig_params);
        ...
        foreach ($iterator as $data) {
            ...
            if (!isset($budget_cache[$data['budgets_id']])) {
                $budget_cache[$data['budgets_id']] = Dropdown::getDropdownName(table: 'ntas_budgets', id: $data['budgets_id'], default: '');
            }
            $entries[] = [
                'itemtype' => self::class,
                'id' => $data['id'],
