...
class Profile_User extends CommonDBRelation
{
    ...
    public static function showForEntity(Entity $entity)
    {
        ...
        $criteria = [
            'SELECT'       => [
                "ntas_users" => ['id', 'name', 'realname', 'firstname', 'picture'],
    ...
    public static function showForProfile(Profile $prof)
    {
        ...
        foreach ($iterator as $data) {
            ...
            if (!isset($entity_names[$data['entity']])) {
                $entity_names[$data['entity']] = Dropdown::getDropdownName('ntas_entities', $data['entity']);
            }
            $username = formatUserLink(
                $data["id"],
                $data["name"],
...
TWIG, $avatar_params) . $username;
    ...
    public static function getUserEntities($user_ID, $is_recursive = true, $default_first = false)
    {
        ...
        foreach ($iterator as $data) {
            if ($data['is_recursive'] && $is_recursive) {
                $tab      = getSonsOf('ntas_entities', $data['entities_id']);
    ...
    public static function getEntitiesForProfileByUser($users_id, $profiles_id, $child = false)
    {
        ...
        foreach ($iterator as $data) {
            if (
                ...
                foreach (getSonsOf('ntas_entities', $data['entities_id']) as $id) {
                    $entities[$id] = $id;
                }
            } else {
    ...
    public static function getForUser($user_ID, $only_dynamic = false)
    {
        ...
        return getAllDataFromTable('ntas_profiles_users', $condition);
    }
    ...
    protected function computeFriendlyName()
    {
        ...
        $name = sprintf(
            __('%1$s, %2$s'),
            Dropdown::getDropdownName('ntas_profiles', $this->fields['profiles_id']),
            Dropdown::getDropdownName('ntas_entities', $this->fields['entities_id'])
        );
    ...
    protected static function getListForItemParams(CommonDBTM $item, $noent = false)
    {
        ...
        $params['SELECT'][] = 'ntas_entities.completename AS completename';
        $params['LEFT JOIN']['ntas_entities'] = [
            'FKEY'   => [
