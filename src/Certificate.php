    ...
    public static function cronCertificate($task = null)
    {
        ...
        foreach (array_keys(Entity::getEntitiesToNotify('use_certificates_alert')) as $entity) {
            ...
            if ($repeat > 0) {
                ...
                $where_date = ['ntas_alerts.date' => null];
            }
            $iterator = $DB->request(
                [
                    'SELECT'    => [
                    ...
                    'LEFT JOIN' => [
                        'ntas_alerts' => [
                            'FKEY'   => [
                                ...
                                [
                                    'AND' => [
                                        'ntas_alerts.itemtype' => self::class,
                    ...
                    'WHERE'     => [
                        $where_date,
                        'ntas_certificates.is_deleted'  => 0,
                        'ntas_certificates.is_template' => 0,
                        [
                            'NOT' => ['ntas_certificates.date_expiration' => null],
                        ],
                        [
                            'RAW' => [
                                'DATEDIFF(' . DBmysql::quoteName('ntas_certificates.date_expiration') . ', CURDATE())' => ['<', $before],
                            ],
                        ],
                        'ntas_certificates.entities_id' => $entity,
                    ],
                ]
            );
