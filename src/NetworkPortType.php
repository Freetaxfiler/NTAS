...
class NetworkPortType extends CommonDropdown
{
    ...
    public static function getInstantiationType($type)
    {
        ...
        if (($import_types = $GLPI_CACHE->get('ntas_inventory_ports_types')) === null) {
            $iterator = $DB->request([
                'FROM'   => self::getTable(),
                'WHERE'  => [
                    'is_importable'   => true,
            ...
            $GLPI_CACHE->set('ntas_inventory_ports_types', $import_types);
        }
    ...
    {
        global $GLPI_CACHE;
        $GLPI_CACHE->delete('ntas_inventory_ports_types');
    }
}
