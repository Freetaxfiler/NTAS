...
class Group extends CommonTreeDropdown
{
    ...
    public function getDataItems(bool $tech, bool $tree, bool $user, int $start, array &$res, array $extra_criteria = []): int
    {
        ...
        if ($tree) {
            $groups_ids = getSonsOf('ntas_groups', $this->getID());
        } else {
    ...
    public function showItems($tech)
    {
        ...
        foreach ($datas as $data) {
            ...
            if (!isset($entity_names[$item->getEntityID()])) {
                $entity_names[$item->getEntityID()] = Dropdown::getDropdownName(table: "ntas_entities", id: $item->getEntityID(), default: '');
            }
    ...
    {
        global $GLPI_CACHE;
        $GLPI_CACHE->set('last_group_change', $_SESSION['ntas_currenttime']);
