...
class CartridgeItem extends CommonDBTM implements AssignableItemInterface
{
    ...
    public static function addCompatibleType($cartridgeitems_id, $printermodels_id)
    {
        ...
        if (
            ...
            $result = $DB->insert('ntas_cartridgeitems_printermodels', $params);
    ...
    public function rawSearchOptions()
    {
        ...
        $tab[] = [
            ...
            'joinparams'         => [
                'beforejoin'         => [
                    'table'              => 'ntas_cartridgeitems_printermodels',
                    'joinparams'         => [
                        'jointype'           => 'child',
                    ],
                ],
        ...
        if ($CFG_GLPI["use_notifications"]) {
            ...
            foreach (Entity::getEntitiesToNotify('cartridges_alert_repeat') as $entity => $repeat) {
                $result = $DB->request(
                    [
                        'SELECT'    => [
                            'ntas_cartridgeitems.id AS cartID',
                            'ntas_cartridgeitems.entities_id AS entity',
                            'ntas_cartridgeitems.ref AS ref',
                            'ntas_cartridgeitems.name AS name',
                            'ntas_cartridgeitems.alarm_threshold AS threshold',
                        ...
                        'LEFT JOIN' => [
                            'ntas_alerts' => [
                                'FKEY' => [
                                    ...
                                    [
                                        'AND' => ['ntas_alerts.itemtype' => 'CartridgeItem'],
                                    ],
                                ],
                            ],
                        ],
                        'WHERE'     => [
                            'ntas_cartridgeitems.is_deleted'      => 0,
                            'ntas_cartridgeitems.alarm_threshold' => ['>=', 0],
                            'ntas_cartridgeitems.entities_id'     => $entity,
                            'OR'                                  => [
    ...
    public static function dropdownForPrinter(Printer $printer)
    {
        ...
        $iterator = $DB->request([
            'SELECT'       => [
                'COUNT'  => '* AS cpt',
                'ntas_locations.completename AS location',
                'ntas_cartridgeitems.ref AS ref',
                'ntas_cartridgeitems.name AS name',
            ...
            'FROM'         => self::getTable(),
            'INNER JOIN'   => [
                'ntas_cartridgeitems_printermodels' => [
                    'ON' => [
                        'ntas_cartridgeitems_printermodels' => 'cartridgeitems_id',
                        'ntas_cartridgeitems'               => 'id',
                    ],
                ],
                'ntas_cartridges'                   => [
                    'ON' => [
                        'ntas_cartridgeitems'   => 'id',
                        'ntas_cartridges'       => 'cartridgeitems_id', [
                            'AND' => [
                                'ntas_cartridges.date_use' => null,
                            ],
                        ],
                    ],
                ],
            ...
            'WHERE'        => [
                'ntas_cartridgeitems_printermodels.printermodels_id'  => $printer->fields['printermodels_id'],
            ] + getEntitiesRestrictCriteria('ntas_cartridgeitems', '', $printer->fields['entities_id'], true),
            'GROUPBY'      => 'tID',
            'ORDERBY'      => ['name', 'ref'],
        ]);
