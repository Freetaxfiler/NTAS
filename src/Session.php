...
class Session
{
    ...
    {
        if ($_SESSION['ntas_use_mode'] !== self::DEBUG_MODE) {
            session_write_close();
        }
    }
    ...
    public static function init(Auth $auth)
    {
        ...
        if ($auth->auth_succeded) {
            ...
            $tosave = ['ntas_plugins', 'glpicookietest', 'phpCAS', 'glpicsrftokens',
                'glpiskipMaintenance',
                'ntas_remote_user',
            ];
            $save   = [];
            ...
            // Define default time :
            $_SESSION["ntas_currenttime"] = date("Y-m-d H:i:s");

            // Normal mode for this request
            $_SESSION["ntas_use_mode"] = self::NORMAL_MODE;
            ...
            if (
                ...
                if (
                    ...
                    && (($auth->user->fields['begin_date'] < $_SESSION["ntas_currenttime"])
                        || is_null($auth->user->fields['begin_date']))
                    && (($auth->user->fields['end_date'] > $_SESSION["ntas_currenttime"])
                    ...
                    $_SESSION["ntas_use_mode"]       = $auth->user->fields['use_mode'];
                    $_SESSION["ntas_plannings"]      = importArrayFromDB($auth->user->fields['plannings']);
                    ...
                    if ($auth->password_expired) {
                        ...
                        $_SESSION["ntas_use_mode"] = self::NORMAL_MODE;
                        $_SESSION['ntas_password_expired'] = 1;
    ...
    public static function initVars(): void
    {
        ...
        if (!isset($_SESSION['ntas_use_mode'])) {
    ...
    public static function isMultiEntitiesMode()
    {
        ...
        if (!isset($_SESSION['ntas_multientitiesmode'])) {
            if (countElementsInTable("ntas_entities") > 1) {
                $_SESSION['ntas_multientitiesmode'] = 1;
            } else {
                $_SESSION['ntas_multientitiesmode'] = 0;
            }
        }

        return $_SESSION['ntas_multientitiesmode'];
    }
    ...
    public static function canViewAllEntities()
    {
        ...
        return (isCommandLine()
              || ((countElementsInTable("ntas_entities")) == count($_SESSION["glpiactiveentities"] ?? [])));
    }
        ...
        if (count($glpiactiveentities)) {
            ...
            if (
                ...
                foreach ($_SESSION['glpiactiveprofile']['entities'] as $val) {
                    ...
                    if ($val['is_recursive']) {
                        $sons = getSonsOf("ntas_entities", $val['id']);
                ...
                $entities = getSonsOf("ntas_entities", $glpiactive_entity);
            }
    ...
    public static function changeActiveEntities($ID = "all", $is_recursive = false)
    {
        ...
        if (isset($_SESSION['glpiactiveprofile'])) {
            if ($ID === "all") {
                foreach ($_SESSION['glpiactiveprofile']['entities'] as $val) {
                    ...
                    if ($val['is_recursive']) {
                        $entities = getSonsOf("ntas_entities", $val['id']);
                        if (count($entities)) {
                ...
                $ancestors = getAncestorsOf("ntas_entities", $ID);
                $ok        = false;
                foreach ($_SESSION['glpiactiveprofile']['entities'] as $val) {
                ...
                $newentities[$ID] = $ID;
                if ($is_recursive) {
                    $entities = getSonsOf("ntas_entities", $ID);
                    if (count($entities)) {
                        foreach (array_keys($entities) as $key2) {
                            $newentities[$key2] = $key2;
                        }
        ...
        if (count($newentities) > 0) {
            ...
            $_SESSION["glpiactive_entity_shortname"] = getTreeLeafValueName("ntas_entities", $active);
            if ($ID == "all") {
            ...
            if (countElementsInTable('ntas_entities') <= count($_SESSION['glpiactiveentities'])) {
    ...
    public static function initEntityProfiles($userID)
    {
        ...
        if (!$DB->tableExists('ntas_profiles_users')) {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'           => [
                'ntas_profiles_users.users_id'   => $userID,
            ],
            'ORDERBY'         => 'ntas_profiles.name',
        ]);
        ...
        if (count($iterator)) {
            foreach ($iterator as $data) {
                ...
                $entities_iterator = $DB->request([
                    'SELECT'    => [
                        'ntas_profiles_users.entities_id AS eID',
                        'ntas_profiles_users.id AS kID',
                        'ntas_profiles_users.is_recursive',
                        'ntas_entities.*',
                    ],
                    'FROM'      => 'ntas_profiles_users',
                    'LEFT JOIN' => [
                        'ntas_entities'   => [
                            'ON' => [
                                'ntas_profiles_users'   => 'entities_id',
                                'ntas_entities'         => 'id',
                            ],
                        ],
                    ],
                    'WHERE'     => [
                        'ntas_profiles_users.profiles_id'   => $key,
                        'ntas_profiles_users.users_id'      => $userID,
                    ],
                    'ORDERBY'   => 'ntas_entities.completename',
                ]);
    ...
    public static function loadGroups()
    {
        ...
        $_SESSION['glpigroups_cache_date'] = $_SESSION["ntas_currenttime"];
    }
    ...
    public static function checkValidSessionId()
    {
        ...
        if (array_key_exists('ntas_remote_user', $_SESSION)) {
            $ssovariable = Dropdown::getDropdownName(
                'ntas_ssovariables',
                $CFG_GLPI["ssovariables_id"]
            );
            if (!array_key_exists($ssovariable, $_SERVER) || $_SERVER[$ssovariable] !== $_SESSION['ntas_remote_user']) {
                throw new SessionExpiredException();
            }
        }
    ...
    public static function haveAccessToEntity($ID, $is_recursive = false)
    {
        ...
        /// Recursive object
        return in_array($ID, getAncestorsOf("ntas_entities", $_SESSION['glpiactiveentities']));
    }
    ...
    public static function getActiveTab($itemtype)
    {
        ...
        return $_SESSION['ntas_tabs'][strtolower($itemtype)] ?? "";
    }
    ...
    {
        $_SESSION['ntas_tabs'][strtolower($itemtype)] = $tab;
    }
    ...
    public static function getSavedOption($itemtype, $name, $defvalue)
    {
        ...
        if (isset($_REQUEST[$name])) {
            return $_SESSION['ntas_saved'][$itemtype][$name] = $_REQUEST[$name];
        }
        return $_SESSION['ntas_saved'][$itemtype][$name] ?? $defvalue;
    }
    ...
    public static function validateCSRF($data, bool $preserve_token = false)
    {
        ...
        if (!isset($data['_ntas_csrf_token'])) {
            return false;
        }
        $requestToken = $data['_ntas_csrf_token'];
    ...
    public static function haveTranslations($itemtype, $field)
    {
        ...
        return (isset($_SESSION['ntas_dropdowntranslations'][$itemtype])
              && isset($_SESSION['ntas_dropdowntranslations'][$itemtype][$field]));
    }
    ...
    public static function startImpersonating($user_id)
    {
        ...
        $session_use_mode = $_SESSION['ntas_use_mode'];

        $impersonator_info = [
            'id'                            => $impersonator_id,
            'glpiname'                      => $impersonator,
            'glpilanguage'                  => $lang,
            'ntas_use_mode'                 => $session_use_mode,
            'glpiactive_entity'             => $_SESSION['glpiactive_entity'],
        ...
        $_SESSION['ntas_use_mode'] = $session_use_mode;
    ...
    public static function stopImpersonating()
    {
        ...
        if (!empty($impersonator_info)) {
            ...
            $_SESSION['ntas_use_mode'] = $impersonator_info['ntas_use_mode'];
    ...
    {
        return array_key_exists('ntas_password_expired', $_SESSION);
    }
    ...
    public static function loadEntity($entities_id, $is_recursive): void
    {
        ...
        $_SESSION["glpiactive_entity_recursive"] = $is_recursive;
        if ($is_recursive) {
            $entities = getSonsOf("ntas_entities", $entities_id);
        } else {
        ...
        $ancestors = getAncestorsOf("ntas_entities", $entities_id);
        $_SESSION['glpiparententities']        = $ancestors;
        $_SESSION['glpiparententities_string'] = implode("', '", $ancestors);
        if (!empty($_SESSION['glpiparententities_string'])) {
            $_SESSION['glpiparententities_string'] = "'" . $_SESSION['glpiparententities_string'] . "'";
    ...
    */
    ...
    /**
     * Helper function to get the date + time stored in $_SESSION['ntas_currenttime']
     *
     * @return null|string timestamp formated as 'Y-m-d H:i:s' or null if not set
     */
    public static function getCurrentTime(): ?string
    {
        // TODO replace references to $_SESSION['ntas_currenttime'] by a call to this function
        return $_SESSION['ntas_currenttime'] ?? null;
    }

    /**
     * Helper function to get the date stored in $_SESSION['ntas_currenttime']
     *
     * @return null|string
     */
