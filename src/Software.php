...
class Software extends CommonDBTM implements TreeBrowseInterface, AssignableItemInterface, DefaultSearchRequestInterface
{
    ...
    public function getSpecificMassiveActions($checkitem = null)
    {
        ...
        if (
            $isadmin
            && (countElementsInTable("ntas_rules", ['sub_type' => 'RuleSoftwareCategory']) > 0)
        ) {
        ...
        if (
            ...
            && (countElementsInTable("ntas_rules", ['sub_type' => 'RuleDictionnarySoftware']) > 0)
        ) {
    ...
    public static function dropdownLicenseToInstall($myname, $entity_restrict)
    {
        ...
        $iterator = $DB->request([
            ...
            ] + getEntitiesRestrictCriteria('ntas_softwarelicenses', 'entities_id', $entity_restrict, true),
    ...
    public function addOrRestoreFromTrash(
        ...
        $iterator = $DB->request([
            ...
            'WHERE'  => [
                ...
                'is_template'        => 0,
            ] + getEntitiesRestrictCriteria('ntas_softwares', 'entities_id', $entity, true),
        ]);
    ...
    public function putInTrash($ID, $comment = '')
    {
        ...
        // change category of the software on deletion (if defined in ntas_configs)
        if (
    ...
    public function showMergeCandidates()
    {
        ...
        $iterator = $DB->request([
            ...
            'WHERE'     => [
                'ntas_softwares.id'           => ['!=', $ID],
                'ntas_softwares.name'         => $this->fields['name'],
                'ntas_softwares.is_deleted'   => 0,
                'ntas_softwares.is_template'  => 0,
            ] + getEntitiesRestrictCriteria(
                'ntas_softwares',
                'entities_id',
                getSonsOf("ntas_entities", $this->fields["entities_id"]),
                false
            ),
            'ORDERBY'   => 'entity',
        ]);
    ...
    private function merge($item): bool
    {
        ...
        $req = $DB->request(['FROM' => "ntas_softwareversions", 'WHERE' => ["softwares_id" => $item]]);
        $i   = 0;
