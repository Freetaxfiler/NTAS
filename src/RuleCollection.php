    ...
    public function getRuleListCriteria($options = [])
    {
        ...
        if ($this->isRuleRecursive()) {
            ...
            if (!$p['childrens']) {
                ...
                $sons = getSonsOf('ntas_entities', $this->entity);
        ...
        if (method_exists($this, 'collectionFilter')) {
            $filter_opts = [];
            if (isset($options['_ntas_tab'])) {
                $filter_opts['_ntas_tab'] = $options['_ntas_tab'];
            }
...
TWIG, $twig_params);
    ...
    public static function previewImportRules()
    {
        ...
        $_SESSION['ntas_import_rules']         = $rules;
        ...
        $_SESSION['ntas_import_rules_refused'] = $rules_refused_for_session;
    ...
    public static function processImportRules()
    {
        ...
        // get session vars
        $rules         = $_SESSION['ntas_import_rules'];
        $rules_refused = $_SESSION['ntas_import_rules_refused'];
        $rr_keys       = array_keys($rules_refused);
        unset($_SESSION['ntas_import_rules'], $_SESSION['ntas_import_rules_refused']);
    ...
    public function prepareInputDataForTestProcess($condition = 0)
    {
        ...
        $limit = [];
        if ($condition > 0) {
            $limit = ['ntas_rules.condition' => ['&', (int) $condition]];
        }
        $input = [];

        $iterator = $DB->request([
            'SELECT'          => 'ntas_rulecriterias.criteria',
            'DISTINCT'        => true,
            ...
            'WHERE'           => [
                'ntas_rules.is_active'  => 1,
                'ntas_rules.sub_type'   => static::getRuleClassName(),
            ] + $limit,
        ]);
