...
class GLPIKey
{
     ...
     */
    protected $fields = [
        'ntas_apiclients.app_token',
        'ntas_authldaps.rootdn_passwd',
        'ntas_mailcollectors.passwd',
        'ntas_oauthclients.secret',
        'ntas_snmpcredentials.auth_passphrase',
        'ntas_snmpcredentials.priv_passphrase',
        'ntas_users.api_token',
        'ntas_users.cookie_token',
        'ntas_users.password_forget_token',
        'ntas_users.personal_token',
    ];
     ...
     *
     * @param string $ntas_version
     *
     * @return string|null
     */
    public function getExpectedKeyPath(string $ntas_version): ?string
    {
        if (version_compare($ntas_version, '9.4.6', '<')) {
            return null;
        } elseif (version_compare($ntas_version, '9.5.x', '<')) {
    ...
    {
        $ntas_key_read_errors = $this->getKeyFileReadErrors();
        if ($ntas_key_read_errors !== []) {
            TemplateRenderer::getInstance()->display(
                '/central/messages.html.twig',
                [
                    'messages' => [
                        'errors' => $ntas_key_read_errors,
                    ],
                ]
            );
        }
