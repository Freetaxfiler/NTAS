...
class Migration
{
    ...
    public function renameTable($oldtable, $newtable)
    {
        if (!$this->db->tableExists("$newtable") && $this->db->tableExists("$oldtable")) {
            ...
            if (
                str_starts_with($oldtable, 'ntas_plugin_')
                || str_starts_with($newtable, 'ntas_plugin_')
            ) {
                return;
            }
            $message = sprintf(
     ...
     * @since 0.84
     *
     * @param array $rule     Array of fields of ntas_rules
     * @param array $criteria Array of Array of fields of ntas_rulecriterias
     * @param array $actions  Array of Array of fields of ntas_ruleactions
     *
    ...
    public function createRule(array $rule, array $criteria, array $actions)
    {
        ...
        if (countElementsInTable('ntas_rules', ['name' => $rule['name']])) {
            return 0;
        }
        ...
        $this->db->insert('ntas_rules', $values);
        ...
        foreach ($criteria as $criterion) {
            ...
            $this->db->insert('ntas_rulecriterias', $values);
        }
        ...
        foreach ($actions as $action) {
            ...
            $this->db->insert('ntas_ruleactions', $values);
        }
    ...
    public function updateDisplayPrefs($toadd = [], $todel = [], bool $only_default = false)
    {
        ...
        $this->addDebugMessage(sprintf(__('Data migration - %s'), 'ntas_displaypreferences'));
                ...
                'LEFT JOIN' => [
                    'ntas_profilerights' => [
                        'ON' => [
                            ...
                            'ntas_profiles'      => 'id',
                            [
                                'AND' => ['ntas_profilerights.name' => $name],
                            ],
                        ],
                    ],
                ],
                'WHERE'     => [
                    'ntas_profilerights.id' => null,
                ],
            ]
        );
    ...
    public function replaceRight($name, $rights, $requiredrights = ['config' => READ | UPDATE])
    {
        ...
        $i = 1;
        foreach ($requiredrights as $reqright => $reqvalue) {
            $join["ntas_profilerights as right$i"] = [
                'ON' => [
                    "right$i"       => 'profiles_id',
                    'ntas_profiles' => 'id',
                    [
    ...
    private function updateProfileLastRightsUpdate(int $profile_id): void
    {
        ...
        if (!$this->db->fieldExists('ntas_profiles', 'last_rights_update')) {
            return;
        }
        ...
        foreach ($this->search_opts as $itemtype => $changes) {
            foreach ($changes as $p) {
                ...
                if (is_a($itemtype, 'CommonITILObject', true)) {
                    $tables = [
                        'ntas_' . strtolower($itemtype) . 'templatehiddenfields',
                        'ntas_' . strtolower($itemtype) . 'templatemandatoryfields',
                        'ntas_' . strtolower($itemtype) . 'templatepredefinedfields',
                    ];
                    foreach ($tables as $table) {
                        if (!$this->db->tableExists($table)) {
                            continue;
    ...
    public function addCrontask(string $itemtype, string $name, int $frequency, ?int $param = null, array $options = []): void
    {
        ...
        $this->db->insert('ntas_crontasks', $input);
    }
