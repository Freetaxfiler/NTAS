    ...
    public static function getHTMLTableCellsForItem(
        ...
        switch ($item::class) {
                    ...
                    switch ($options['order']) {
                        ...
                        case 'ip':
                            $criteria['LEFT JOIN'] = [
                                'ntas_ipaddresses'   => [
                                    ...
                                    $table               => 'id', [
                                        'AND' => [
                                            'ntas_ipaddresses.itemtype'   => self::class,
                                            'ntas_ipaddresses.is_deleted' => 0,
                                        ],
                                    ],
                                ],
                            ];
                            $criteria['ORDERBY'] = [
                                new QueryExpression("ISNULL (" . $DB::quoteName('ntas_ipaddresses.id') . ")"),
                                'ntas_ipaddresses.binary_3',
                                'ntas_ipaddresses.binary_2',
                                'ntas_ipaddresses.binary_1',
                                'ntas_ipaddresses.binary_0',
                            ];
                            break;
                        ...
                        case 'alias':
                            $criteria['LEFT JOIN'] = [
                                'ntas_networkaliases'   => [
                                    'ON'  => [
                                        'ntas_networkaliases'   => 'networknames_id',
                                        $table                  => 'id',
                                    ],
                                ],
                            ];
                            $criteria['ORDERBY'] = [
                                new QueryExpression("ISNULL (" . $DB::quoteName('ntas_networkaliases.name') . ")"),
                                'ntas_networkaliases.name',
                            ];
                            break;
                    }
                }
            ...
            case NetworkEquipment::class:
                ...
                $criteria['WHERE'] = [
                    'ntas_networkports.itemtype'  => NetworkEquipment::class,
...
TWIG, $twig_params);
    ...
    public static function countForItem(CommonDBTM $item): int
    {
        ...
        switch ($item::class) {
            ...
            case NetworkEquipment::class:
                $result = $DB->request([
                    'SELECT'          => ['COUNT DISTINCT' => 'ntas_networknames.id AS cpt'],
                    ...
                    'WHERE'           => [
                        'ntas_networkports.itemtype'     => $item->getType(),
                        'ntas_networkports.items_id'     => $item->getID(),
                        'ntas_networkports.is_deleted'   => 0,
                        'ntas_networknames.is_deleted'   => 0,
                    ],
                ])->current();
