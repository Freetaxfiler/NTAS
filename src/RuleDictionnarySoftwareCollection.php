...
TWIG, $twig_params);
    ...
    private function getIteratorCriteriaForRulesReplay(array $params): array
    {
        ...
        $criteria = [
            'SELECT'          => [
                'ntas_softwares.name',
                'ntas_manufacturers.name AS manufacturer',
                'ntas_softwares.manufacturers_id AS manufacturers_id',
                'ntas_softwares.entities_id AS entities_id',
                'ntas_softwares.is_helpdesk_visible AS helpdesk',
                'ntas_softwares.softwarecategories_id AS softwarecategories_id',
            ],
            'DISTINCT'        => true,
            'FROM'            => 'ntas_softwares',
            'LEFT JOIN'       => [
        ...
        if (isset($params['manufacturer']) && $params['manufacturer']) {
            $criteria['WHERE']['ntas_softwares.manufacturers_id'] = $params['manufacturer'];
        }
    ...
    public function replayDictionnaryOnSoftwaresByID(array $IDs, $res_rule = [])
    {
        ...
        foreach ($IDs as $ID) {
            $iterator = $DB->request([
                ...
                'FROM'      => 'ntas_softwares AS gs',
                'LEFT JOIN' => [
                    'ntas_manufacturers AS gm' => [
                        'ON' => [
                            'gs'  => 'manufacturers_id',
                            'gm'  => 'id',
                        ],
        ...
        if (
            ...
            || (!isset($res_rule["name"])
              ...
              && in_array(
                  ...
                  getAncestorsOf('ntas_entities', $entity)
              ))
        ) {
    ...
    public function putOldSoftsInTrash(array $soft_ids)
    {
        ...
        if (count($soft_ids) > 0) {
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    'ntas_softwares.id',
                    'COUNT' => 'ntas_softwareversions.softwares_id AS cpt',
                ],
                'FROM'      => 'ntas_softwares',
                'LEFT JOIN' => [
                ...
                'WHERE'     => [
                    'ntas_softwares.id'  => $soft_ids,
                    'is_deleted'         => 0,
                ],
                'GROUPBY'   => 'ntas_softwares.id',
                'HAVING'    => ['cpt' => 0],
            ]);
    ...
    public function moveVersions($ID, $new_software_id, $version_id, $old_version, $new_version, $entity)
    {
        ...
        if ($new_versionID != $version_id) {
            ...
            if ($new_versionID == -1) {
                ...
                //Change ID of the version in ntas_items_softwareversions
                $DB->update(
    ...
    public function moveLicenses($old_software_id, $new_software_id)
    {
        ...
        if (
            !countElementsInTable('ntas_softwares', ['id' => $old_software_id])
            || !countElementsInTable('ntas_softwares', ['id' => $new_software_id])
        ) {
            return false;
        }
