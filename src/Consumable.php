...
class Consumable extends CommonDBChild
{
        ...
        switch ($ma->getAction()) {
            case 'give':
                ...
                $entity_restrict = $consumable_item->isRecursive()
                    ? getSonsOf('ntas_entities', $consumable_item->getEntityID())
    ...
    public static function showForUser(User $user)
    {
        ...
        $query = [
            'SELECT' => [
                'ntas_consumables.*',
                'ntas_consumableitems.name AS itemname',
                'ntas_consumableitems.ref AS ref',
                'ntas_consumableitems.entities_id AS entities_id',
                'ntas_consumableitems.is_recursive AS is_recursive',
            ],
            'FROM' => self::getTable(),
            'LEFT JOIN' => [
                'ntas_consumableitems' => [
                    'ON' => [
                        ...
                        'ntas_consumables'     => 'consumableitems_id',
                    ],
                ],
            ],
            'WHERE'  => [
                'ntas_consumables.items_id' => $items_id,
                'ntas_consumables.itemtype' => $itemtype,
                'NOT' => ['ntas_consumables.date_out' => 'NULL'],
            ] + getEntitiesRestrictCriteria('ntas_consumableitems', '', '', true),
        ];
        ...
        $iterator = $DB->request([
            ...
            'WHERE'  => [
                ...
                'consumableitems_id' => new QuerySubQuery([
                    'SELECT' => 'id',
                    'FROM'   => 'ntas_consumableitems',
                    'WHERE'  => getEntitiesRestrictCriteria('ntas_consumableitems'),
                ]),
            ],
    ...
    {
        return countElementsInTable(['ntas_consumables'], ['ntas_consumables.consumableitems_id' => $item->getField('id')]);
    }
    ...
    {
        return countElementsInTable(['ntas_consumables'], [
            'ntas_consumables.itemtype' => 'User',
            'ntas_consumables.items_id' => $item->getField('id'),
            'NOT' => ['ntas_consumables.date_out' => 'NULL'],
        ]);
    }
    ...
    public static function convertFiltersValuesToSqlCriteria(array $filters = []): array
    {
        ...
        $like_filters = [
            'id'        => 'ntas_consumables.id',
            'itemname'  => 'ntas_consumableitems.name',
            'ref'       => 'ntas_consumableitems.ref',
            'date_in'   => 'ntas_consumables.date_in',
            'date_out'  => 'ntas_consumables.date_out',
        ];
