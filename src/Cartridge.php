...
TWIG, ['counts' => $counts, 'highlight' => $highlight]);
    ...
    public static function showForCartridgeItem(CartridgeItem $cartitem, $show_old = 0)
    {
        ...
        $where = ['ntas_cartridges.cartridgeitems_id' => $tID];
        $order = [
            'ntas_cartridges.date_use ASC',
            'ntas_cartridges.date_out DESC',
            'ntas_cartridges.date_in',
        ];

        if (!$show_old) { // NEW
            $where['ntas_cartridges.date_out'] = null;
            $order = [
                'ntas_cartridges.date_out ASC',
                'ntas_cartridges.date_use ASC',
                'ntas_cartridges.date_in',
            ];
        } else { //OLD
            $where['NOT'] = ['ntas_cartridges.date_out' => null];
        }
        ...
        $iterator = $DB->request([
            'SELECT' => [
                'ntas_cartridges.*',
                'ntas_printers.id AS printID',
                'ntas_printers.name AS printname',
                'ntas_printers.init_pages_counter',
            ],
            'FROM'   => self::getTable(),
            'LEFT JOIN' => [
                'ntas_printers'   => [
...
TWIG, $twig_params);
    ...
    public static function showForPrinter(Printer $printer, $old = 0)
    {
        ...
        $where = ['ntas_cartridges.printers_id' => $instID];
        if ($old) {
            $where['NOT'] = ['ntas_cartridges.date_out' => null];
        } else {
            $where['ntas_cartridges.date_out'] = null;
        ...
        $iterator = $DB->request([
            'SELECT'    => [
                'ntas_cartridgeitems.id AS tID',
                'ntas_cartridgeitems.is_deleted',
                'ntas_cartridgeitems.ref AS ref',
                'ntas_cartridgeitems.name AS type',
                'ntas_cartridges.id',
                'ntas_cartridges.pages AS pages',
                'ntas_cartridges.date_use AS date_use',
                'ntas_cartridges.date_out AS date_out',
                'ntas_cartridges.date_in AS date_in',
                'ntas_cartridgeitemtypes.name AS typename',
            ],
            'FROM'      => self::getTable(),
            'LEFT JOIN' => [
                'ntas_cartridgeitems'      => [
...
TWIG, ['printer_id' => $printer->getID()]);
    ...
    {
        return countElementsInTable(['ntas_cartridges'], ['ntas_cartridges.cartridgeitems_id' => $item->getField('id')]);
    }
    ...
    {
        return countElementsInTable(['ntas_cartridges'], ['ntas_cartridges.printers_id' => $item->getField('id')]);
    }
