...
class NetworkAlias extends FQDNLabel
{
    ...
    public static function showForFQDN(FQDN $item, $withtemplate)
    {
        ...
        if ($number < 1) {
            ...
            $iterator = $DB->request([
                'SELECT'    => [
                    'ntas_networkaliases.id AS alias_id',
                    'ntas_networkaliases.name AS alias',
                    'ntas_networknames.id AS address_id',
                    'ntas_networkaliases.comment AS comment',
                ],
                'FROM'      => 'ntas_networkaliases',
                'INNER JOIN' => [
                    'ntas_networknames'  => [
                ...
                'WHERE'     => ['ntas_networkaliases.fqdns_id' => $item->getID()],
                'ORDERBY'   => $order,
