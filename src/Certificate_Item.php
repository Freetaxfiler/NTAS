                ...
                if (count($iterator)) {
                    foreach ($iterator as $data) {
                        ...
                        if (Session::isMultiEntitiesMode()) {
                            $entry['entity'] = $item->isEntityAssign() ? Dropdown::getDropdownName("ntas_entities", $data['entity']) : '-';
                        }
                        $entries[] = $entry;
                    }
                }
    ...
    public static function showForItem(CommonDBTM $item, $withtemplate = 0)
    {
        ...
        foreach ($certificates as $data) {
            ...
            $entry = [
                ...
                'name' => $link,
                'type' => Dropdown::getDropdownName("ntas_certificatetypes", $data["certificatetypes_id"]),
                'dns_name' => $data['dns_name'],
                'dns_suffix' => $data['dns_suffix'],
                'date_creation' => $data['date_creation'],
                'status' => Dropdown::getDropdownName("ntas_states", $data["states_id"]),
            ];
            if (Session::isMultiEntitiesMode()) {
                $entry['entity'] = Dropdown::getDropdownName("ntas_entities", $data['entities_id']);
            }
