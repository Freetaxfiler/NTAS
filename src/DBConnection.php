...
class DBConnection extends CommonGLPI
{
            ...
            if ($DBread->connected) {
                ...
                switch ($CFG_GLPI['use_slave_for_search']) {
                    ...
                    case 2: // If synced (current user changes or profile in read only)
                        if (!isset($_SESSION['ntas_maxhistory'])) {
                            // No change yet
                            return $DBread;
                        }
                        $slave  = $DBread->request($sql)->current();
                        if (
                            isset($slave['maxid'])
                            && ($slave['maxid'] >= $_SESSION['ntas_maxhistory'])
                        ) {
    ...
    public static function getHistoryMaxDate($DBconnection)
    {
        ...
        if ($DBconnection->connected) {
            /** @var mysqli_result $result */
            $result = $DBconnection->doQuery("SELECT UNIX_TIMESTAMP(MAX(`date_mod`)) AS max_date FROM `ntas_logs`");
