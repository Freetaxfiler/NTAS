...
class Auth extends CommonGLPI
{
        ...
        if (count($result) === 1) {
            ...
            if (self::checkPassword($password, $password_db)) {
                ...
                if (
                    ...
                    && $row['lock_date'] < $_SESSION['ntas_currenttime']
                ) {
                    $user = new User();
                    $user->update(
                        [
                ...
                if (
                    ...
                    && $row['password_expiration_date'] < $_SESSION['ntas_currenttime']
                ) {
        ...
        switch ($authtype) {
            ...
            case self::EXTERNAL:
                ...
                if (self::isValidLogin($login)) {
                    ...
                    $_SESSION['ntas_remote_user'] = $login_string; // store raw remote user
    ...
    public function getAuthMethods()
    {
        ...
        $this->authtypes = [
            'ldap' => getAllDataFromTable('ntas_authldaps'),
            'mail' => getAllDataFromTable('ntas_authmails'),
        ];
    }
            ...
            if (
                ...
                if ((!isset($this->user->fields['authtype']) || $this->user->fields['authtype'] === self::LDAP) && Toolbox::canUseLdap()) {
                    ...
                    if (
                        ...
                        foreach (getAllDataFromTable('ntas_authldaps', ['is_active' => 1]) as $ldap_config) {
    ...
    public static function checkAlternateAuthSystems($redirect = false, $redirect_string = '')
    {
        ...
        // MoYo : checking REQUEST create a security hole for me !
        $ssovariable = Dropdown::getDropdownName('ntas_ssovariables', $CFG_GLPI["ssovariables_id"]);
        if (
            $CFG_GLPI["ssovariables_id"]
