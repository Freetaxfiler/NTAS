...
class RuleImportAsset extends Rule
{
    ...
    public function getCriterias()
    {
        ...
        $criteria = [
            ...
            'virtualmachinetypes_id' => [
                'table'     => 'ntas_virtualmachinetypes',
                'field'     => 'name',
    ...
    public function handleLinkCriteriaPort(CommonDBTM $item, array &$it_criteria)
    {
        ...
        if ($is_ip) {
            $it_criteria['LEFT JOIN']['ntas_networkports'] = [
                'ON'  => [
                    $itemtable           => 'id',
                    'ntas_networkports'  => 'items_id', [
                        'AND' => ['ntas_networkports.itemtype' => $itemtype],
                    ],
                ],
            ];
            $it_criteria['LEFT JOIN']['ntas_networknames'] = [
                'ON'  => [
                    'ntas_networkports'  => 'id',
                    'ntas_networknames'  => 'items_id', [
                        'AND' => ['ntas_networknames.itemtype' => 'NetworkPort'],
                    ],
                ],
            ];
            $it_criteria['LEFT JOIN']['ntas_ipaddresses'] = [
                'ON'  => [
                    'ntas_networknames'  => 'id',
                    'ntas_ipaddresses'   => 'items_id', [
                        'AND' => ['ntas_ipaddresses.itemtype' => 'NetworkName'],
                    ],
                ],
            ];
    ...
    public function handleOneJoinPerCriteria(CommonDBTM $item, array &$it_criteria)
    {
        ...
        foreach ($this->complex_criteria as $criterion) {
            if ($criterion->fields['criteria'] == 'ip') {
                ...
                $it_criteria['LEFT JOIN']['ntas_networkports AS ' . $astable] = [
                    'ON'  => [
                        $itemtable  => 'id',
                        $astable    => 'items_id', [
            ...
            switch ($criterion->fields['criteria']) {
                ...
                case 'ip':
                    ...
                    if (!$this->link_criteria_port) {
                        $ntable = "networkports_" . $criterion->fields['criteria'];
                        $it_criteria['SELECT'][] = $ntable . ".id AS portid_" . $criterion->fields['criteria'];
                    } elseif (!in_array('ntas_networkports.id AS portid', $it_criteria['SELECT'])) {
                        $it_criteria['SELECT'][] = 'ntas_networkports.id AS portid';
                    }

                    $it_criteria['WHERE'][] = ['ntas_ipaddresses.name' => $input['ip']];
                    break;
                ...
                case 'tag':
                    if (isset($input['tag']) && isset($input['deviceid'])) {
                        $it_criteria['LEFT JOIN']['ntas_agents'] = [
                            'ON'  => [
                                'ntas_agents'  => 'items_id',
                                $itemtable     => 'id',
                            ],
                        ];
                        $it_criteria['WHERE'][] = [
                            'ntas_agents.deviceid' => $input['deviceid'],
                            'ntas_agents.tag' => $input['tag'],
                        ];
                    }
                    break;
                ...
                case 'device_id':
                    ...
                    $it_criteria['WHERE'][] = [
                        'ntas_agents.device_id' => $input['device_id'],
                    ];
                    break;

                case 'domain':
                    $it_criteria['LEFT JOIN']['ntas_domains'] = [
                        'ON'  => [
                            'ntas_domains' => 'id',
                    ...
                    $it_criteria['WHERE'][] = [
                        'ntas_domains.name'  => $input['domains_id'],
                    ];
                    break;
