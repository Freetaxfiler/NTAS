...
TWIG, $twig_params);
    ...
    public static function dropdownForOneSoftware($options = [])
    {
        ...
        $criteria = [
            ...
            'WHERE'     => [
                'ntas_softwareversions.softwares_id'   => $p['softwares_id'],
            ],
            'ORDERBY'   => 'name',
        ];

        if (count($p['used'])) {
            $criteria['WHERE']['NOT'] = ['ntas_softwareversions.id' => $p['used']];
        }
...
TWIG, $twig_params);
        ...
        foreach ($iterator as $data) {
            ...
            $entries[] = [
                ...
                'version' => $sv->getLink(),
                'status' => $data['sname'],
                'os' => Dropdown::getDropdownName('ntas_operatingsystems', $data['operatingsystems_id']),
                'arch' => $data['arch'],
                'installations' => $nb,
