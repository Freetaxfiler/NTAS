...
abstract class CommonDBVisible extends CommonDBTM
{
    ...
    public function showVisibility(): bool
    {
        ...
        foreach ($this->groups as $val) {
            foreach ($val as $data) {
                $name    = Dropdown::getDropdownName('ntas_groups', $data['groups_id']);
                $tooltip = Dropdown::getDropdownComments('ntas_groups', (int) $data['groups_id']);
                $recipient = sprintf(
                    __s('%1$s %2$s'),
                    htmlescape($name),
        ...
        foreach ($this->entities as $val) {
            foreach ($val as $data) {
                $name    = Dropdown::getDropdownName('ntas_entities', $data['entities_id']);
                $tooltip = Dropdown::getDropdownComments('ntas_entities', (int) $data['entities_id']);
                $recipient = sprintf(
                    __s('%1$s %2$s'),
                    htmlescape($name),
                    Html::showToolTip($tooltip, ['display' => false])
        ...
        foreach ($this->profiles as $val) {
            foreach ($val as $data) {
                $name    = Dropdown::getDropdownName('ntas_profiles', $data['profiles_id']);
                $tooltip = Dropdown::getDropdownComments('ntas_profiles', (int) $data['profiles_id']);
                $recipient = sprintf(
                    __s('%1$s %2$s'),
                    htmlescape($name),
                    Html::showToolTip($tooltip, ['display' => false])
