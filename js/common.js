...
/* global L */
/* global fuzzy */
/* global ntas_html_dialog */
/* global ntas_toast_info, ntas_toast_warning, ntas_toast_error */
/* global _ */
...
$(() => {
    ...
    $(document).on('click', '[data-glpi-clipboard-text]', function() {
        ...
        if (navigator.clipboard === undefined) {
            ...
            ntas_toast_error(__("Unable to copy to clipboard (insecure context)."));
        } else {
            navigator.clipboard.writeText(text);
            ntas_toast_info(__("Copied to clipboard"));
        }
    });
});
        ...
        $.each(messages, (level, level_messages) => {
            $.each(level_messages, (index, message) => {
                switch (parseInt(level)) {
                    case 1:
                        ntas_toast_error(message);
                        break;
                    case 2:
                        ntas_toast_warning(message);
                        break;
                    default:
                        ntas_toast_info(message);
                }
            });
        });
    });
