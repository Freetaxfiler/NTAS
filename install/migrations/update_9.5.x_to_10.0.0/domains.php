...
$migration->addField('ntas_domains', 'is_template', 'bool', [
    'after' => 'comment',
]);
$migration->addField('ntas_domains', 'template_name', 'string', [
    'after' => 'is_template',
]);
$migration->addKey('ntas_domains', 'is_template');
/** /Add templates to domains  */

/** Active domains */
$migration->addField('ntas_domains', 'is_active', 'bool', ['after' => 'template_name']);
$migration->addKey('ntas_domains', 'is_active');
...
$migration->dropField('ntas_domains', 'others');

// Add fields descriptor field
if (!$DB->fieldExists('ntas_domainrecordtypes', 'fields')) {
    $migration->addField(
    ...
    foreach (DomainRecordType::getDefaults() as $type) {
        if (countElementsInTable('ntas_domainrecordtypes', ['name' => $type['name']]) === 0) {
            continue;
        }
        $migration->addPostQuery(
            $DB->buildUpdate(
...
if (countElementsInTable('ntas_domainrecordtypes', ['name' => 'CAA']) === 0) {
    foreach (DomainRecordType::getDefaults() as $type) {
        if ($type['name'] === 'CAA') {
            unset($type['id']);
            $migration->addPostQuery(
