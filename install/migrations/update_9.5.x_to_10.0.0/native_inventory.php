...
if (!$DB->tableExists('ntas_agenttypes')) {
    $query = "CREATE TABLE `ntas_agenttypes` (
...
if (!$DB->tableExists('ntas_agents')) {
    $query = "CREATE TABLE `ntas_agents` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
    ...
    $migration->dropKey('ntas_agents', 'items_id');
    $migration->dropKey('ntas_agents', 'itemtype');
    $migration->dropForeignKeyContraint('ntas_agents', 'agenttypes_id');
    $migration->migrationOneTable('ntas_agents');
    $migration->addKey('ntas_agents', 'agenttypes_id');
    $migration->addKey('ntas_agents', 'entities_id');
    $migration->addKey('ntas_agents', 'is_recursive');
    $migration->addKey('ntas_agents', ['itemtype', 'items_id'], 'item');
    $migration->addField(
        'ntas_agents',
...
if (!$DB->tableExists('ntas_rulematchedlogs')) {
    $query = "CREATE TABLE `ntas_rulematchedlogs` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
    ...
    $migration->addKey('ntas_rulematchedlogs', 'agents_id');
    $migration->addKey('ntas_rulematchedlogs', 'rules_id');
}

//locked fields
if (!$DB->tableExists('ntas_lockedfields')) {
    $query = "CREATE TABLE `ntas_lockedfields` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `itemtype` varchar(100) DEFAULT NULL,
         `items_id` int {$default_key_sign} NOT NULL DEFAULT '0',
    ...
    $migration->dropKey('ntas_lockedfields', 'item');
    $migration->migrationOneTable('ntas_lockedfields');
    $migration->addField('ntas_lockedfields', 'value', 'string');
    $migration->addKey('ntas_lockedfields', 'date_creation');
}
$ADDTODISPLAYPREF['Lockedfield'] = [3, 13, 5];


//transfer configuration per entity
if (!$DB->fieldExists('ntas_entities', 'transfers_id')) {
    $migration->addField(
        'ntas_entities',
        'transfers_id',
    ...
    $migration->addKey('ntas_entities', 'transfers_id');
}

//agent URL configuration per entity
if (!$DB->fieldExists('ntas_entities', 'agent_base_url')) {
    $migration->addField(
        'ntas_entities',
        'agent_base_url',
        'string'
...
if (!$DB->fieldExists('ntas_networkequipments', 'autoupdatesystems_id')) {
    $migration->addField(
    ...
    $migration->addKey('ntas_networkequipments', 'autoupdatesystems_id');
}

if (!$DB->fieldExists('ntas_networkequipments', 'sysdescr')) {
    $migration->addField(
        'ntas_networkequipments',
        'sysdescr',
        'text',
...
if (!$DB->fieldExists('ntas_networkequipments', 'cpu')) {
    $migration->addField(
        'ntas_networkequipments',
        'cpu',
        'integer',
...
if (!$DB->fieldExists('ntas_networkequipments', 'uptime')) {
    $migration->addField(
        'ntas_networkequipments',
        'uptime',
        'string',
...
if (!$DB->fieldExists('ntas_networkequipments', 'last_inventory_update')) {
    $migration->addField(
        'ntas_networkequipments',
...
if (!$DB->fieldExists('ntas_printers', 'sysdescr')) {
    $migration->addField(
        'ntas_printers',
        'sysdescr',
        'text',
...
if (!$DB->fieldExists('ntas_printers', 'last_inventory_update')) {
    $migration->addField(
        'ntas_printers',
        'last_inventory_update',
        'timestamp',
...
if (!$DB->fieldExists('ntas_computers', 'last_inventory_update')) {
    $migration->addField(
        'ntas_computers',
        'last_inventory_update',
        'timestamp',
...
if (!$DB->fieldExists('ntas_phones', 'last_inventory_update')) {
    $migration->addField(
        'ntas_phones',
        'last_inventory_update',
        'timestamp',
...
foreach ($netport_fields as $netport_field => $definition) {
    if (!$DB->fieldExists('ntas_networkports', $netport_field)) {
        $migration->addField('ntas_networkports', $netport_field, $definition);
    }
}

if (!$DB->tableExists('ntas_unmanageds')) {
    $query = "CREATE TABLE `ntas_unmanageds` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `entities_id` int {$default_key_sign} NOT NULL DEFAULT '0',
         `is_recursive` tinyint NOT NULL DEFAULT '0',
         `name` varchar(255) DEFAULT NULL,
    ...
    $migration->addKey('ntas_unmanageds', 'is_recursive');
}
$ADDTODISPLAYPREF['Unmanaged'] = [2, 4, 3, 5, 7, 10, 18, 14, 15, 9];

if (!$DB->tableExists('ntas_networkporttypes')) {
    $query = "CREATE TABLE `ntas_networkporttypes` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `entities_id` int {$default_key_sign} NOT NULL DEFAULT '0',
         `is_recursive` tinyint NOT NULL DEFAULT '0',
    ...
    $migration->addKey('ntas_networkporttypes', 'is_recursive');
}

$ADDTODISPLAYPREF['NetworkPortType'] = [10, 11, 12];

if (!$DB->tableExists('ntas_networkporttypes') || countElementsInTable(NetworkPortType::getTable()) === 0) {
    if (!$DB->tableExists('ntas_networkporttypes')) {
...
if (!$DB->tableExists('ntas_printers_cartridgeinfos')) {
    $query = "CREATE TABLE `ntas_printers_cartridgeinfos` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
...
if (!$DB->tableExists('ntas_printerlogs')) {
    $query = "CREATE TABLE `ntas_printerlogs` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `printers_id` int {$default_key_sign} NOT NULL,
    ...
    foreach (['date_creation', 'date_mod'] as $date_field) {
        if (!$DB->fieldExists('ntas_printerlogs', $date_field)) {
            $migration->addField(
                'ntas_printerlogs',
                $date_field,
                'timestamp',
            ...
            $migration->addKey('ntas_printerlogs', $date_field);
        }
    }
    // In GLPI 10.0.0-rc2 or earlier, `date` had timestamp datatype.
    $migration->changeField('ntas_printerlogs', 'date', 'date', 'date');

    $migration->dropKey('ntas_printerlogs', 'printers_id');

    if (!isIndex('ntas_printerlogs', 'unicity')) {
        ...
        $migration->addKey('ntas_printerlogs', ['printers_id', 'date'], 'unicity', 'UNIQUE');
    }
}

if (!$DB->tableExists('ntas_networkportconnectionlogs')) {
    $query = "CREATE TABLE `ntas_networkportconnectionlogs` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `date` timestamp NULL DEFAULT NULL,
    ...
    $migration->addKey('ntas_networkportconnectionlogs', 'networkports_id_destination');
    $migration->addKey('ntas_networkportconnectionlogs', 'networkports_id_source');
}

if (!$DB->tableExists('ntas_networkportmetrics')) {
    $query = "CREATE TABLE `ntas_networkportmetrics` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `date` date DEFAULT NULL,
         `ifinbytes` bigint NOT NULL DEFAULT '0',
         `ifinerrors` bigint NOT NULL DEFAULT '0',
    ...
    foreach (['date_creation', 'date_mod'] as $date_field) {
        if (!$DB->fieldExists('ntas_networkportmetrics', $date_field)) {
            $migration->addField(
                'ntas_networkportmetrics',
                $date_field,
                'timestamp',
                [
                    'update' => $DB->quoteName('date'),
            ...
            $migration->addKey('ntas_networkportmetrics', $date_field);
        }
    }
    // In GLPI 10.0.0-rc2 or earlier, `date` had timestamp datatype.
    $migration->changeField('ntas_networkportmetrics', 'date', 'date', 'date');

    $migration->dropKey('ntas_networkportmetrics', 'networkports_id');

    if (!isIndex('ntas_networkportmetrics', 'unicity')) {
        // Preserve only last insert for a given date.
        $to_preserve_sql = new QueryExpression(
            sprintf(
                'SELECT MAX(%s) as %s FROM %s GROUP BY %s, DATE(%s)',
                $DB->quoteName('id'),
                $DB->quoteName('id'),
                $DB->quoteName('ntas_networkportmetrics'),
        ...
        $migration->addKey('ntas_networkportmetrics', ['networkports_id', 'date'], 'unicity', 'UNIQUE');
    }
}

if (!$DB->tableExists('ntas_refusedequipments')) {
    $query = "CREATE TABLE `ntas_refusedequipments` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `name` varchar(255) DEFAULT NULL,
         `itemtype` varchar(100) DEFAULT NULL,
         `entities_id` int {$default_key_sign} NOT NULL DEFAULT '0',
    ...
    $migration->addKey('ntas_refusedequipments', 'entities_id');
    $migration->addKey('ntas_refusedequipments', 'agents_id');
    $migration->addKey('ntas_refusedequipments', 'rules_id');
    $migration->addKey('ntas_refusedequipments', 'date_creation');
    $migration->addKey('ntas_refusedequipments', 'date_mod');
    if (!$DB->fieldExists('ntas_refusedequipments', 'autoupdatesystems_id')) {
        $migration->addField(
            'ntas_networkequipments',
            'autoupdatesystems_id',
            "int {$default_key_sign} NOT NULL DEFAULT '0'",
...
if (!$DB->tableExists('ntas_usbvendors')) {
    $query = "CREATE TABLE `ntas_usbvendors` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `entities_id` int {$default_key_sign} NOT NULL DEFAULT '0',
         `is_recursive` tinyint NOT NULL DEFAULT '0',
    ...
    $migration->dropKey('ntas_usbvendors', 'vendorid');
    $migration->migrationOneTable('ntas_usbvendors');
    $migration->addKey('ntas_usbvendors', 'is_recursive');
}
$ADDTODISPLAYPREF['USBVendor'] = [10, 11];

if (!$DB->tableExists('ntas_pcivendors')) {
    $query = "CREATE TABLE `ntas_pcivendors` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
         `entities_id` int {$default_key_sign} NOT NULL DEFAULT '0',
         `is_recursive` tinyint NOT NULL DEFAULT '0',
         `vendorid` varchar(4) NOT NULL,
    ...
    $migration->dropKey('ntas_pcivendors', 'vendorid');
    $migration->migrationOneTable('ntas_pcivendors');
    $migration->addKey('ntas_pcivendors', 'is_recursive');
}
$ADDTODISPLAYPREF['PCIVendor'] = [10, 11];

if (!$DB->tableExists('ntas_snmpcredentials')) {
    $query = "CREATE TABLE `ntas_snmpcredentials` (
         `id` int {$default_key_sign} NOT NULL AUTO_INCREMENT,
...
if (countElementsInTable('ntas_snmpcredentials') === 0) {
    $migration->addPostQuery(
        $DB->buildInsert(
...
$cred_tables = ['ntas_printers', 'ntas_networkequipments', 'ntas_unmanageds'];
...
if (!$DB->fieldExists('ntas_printers', 'autoupdatesystems_id')) {
    $migration->addField(
        'ntas_printers',
        'autoupdatesystems_id',
        "int {$default_key_sign} NOT NULL DEFAULT '0'",
    ...
    $migration->addKey('ntas_printers', 'autoupdatesystems_id');
}

// Other autoupdatesystems_id additions
$autoupdatesystems_tables = ['ntas_databaseinstances', 'ntas_monitors', 'ntas_peripherals', 'ntas_phones'];
