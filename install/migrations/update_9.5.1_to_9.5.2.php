<?php

/**
 * Naipunya Enterprise Service Management
 * Copyright (C) 2026 Naipunya Tax and Accounting Solutions Pvt.Ltd.
 */

use function Safe\preg_match_all;

/**
 * Update from 9.5.1 to 9.5.2
 *
 * @return bool
 **/
function update951to952()
{
    /**
     * @var DBmysql $DB
     * @var Migration $migration
     */
    global $DB, $migration;

    $updateresult     = true;

    $migration->setVersion('9.5.2');

    /* Fix document_item migration */
    $migration->displayMessage("Building inline images data in " . Document_Item::getTable());

    $now = date('Y-m-d H:i:s');

    $itemtypes = [
        'ITILFollowup' => 'content',
        'ITILSolution' => 'content',
        'Reminder'     => 'text',
        'KnowbaseItem' => 'answer',
    ];

    foreach (['Change', 'Problem', 'Ticket'] as $itiltype) {
        $itemtypes[$itiltype] = 'content';
        $itemtypes[$itiltype . 'Task'] = 'content';
    }

    $docs_input = [];
    foreach ($itemtypes as $itemtype => $field) {
        // Check ticket and child items (followups, tasks, solutions) contents
        $regexPattern = 'document\.send\.php\?docid=[0-9]+';
        $user_field = is_a($itemtype, CommonITILObject::class, true) ? 'users_id_recipient' : 'users_id';
        $result = $DB->request([
            'SELECT' => ['id', $field, $user_field],
            'FROM'   => $itemtype::getTable(),
            'WHERE'  => [
                $field => ['REGEXP', $regexPattern],
            ],
        ]);

        foreach ($result as $data) {
            $matches = [];
            preg_match_all('/document\\.send\\.php\\?docid=([0-9]+)/', $data[$field], $matches);

            // No inline documents found in this item, skip to next
            if (count($matches[1]) === 0) {
                continue;
            }

            foreach ($matches[1] as $docid) {
                $document = $DB->request(
                    [
                        'SELECT' => ['entities_id'],
                        'FROM'   => 'ntas_documents',
                        'WHERE'  => [
                            'id' => $docid,
                        ],
                    ]
                );
                if ($document->count() === 0) {
                    continue; // Invalid link to document
                }
                $entities_id = $document->current()['entities_id'];

                $doc_input = [
                    'documents_id'      => $docid,
                    'itemtype'          => $itemtype,
                    'items_id'          => $data['id'],
                    'timeline_position' => in_array($itemtype, ['Change', 'Problem', 'Ticket']) ? -1 : 0,
                    'users_id'          => $data[$user_field],
                    'entities_id'       => $entities_id,
                    'date_creation'     => $now,
                    'date_mod'          => $now,
                ];

                $docs_input[] = $doc_input;
            }
        }
    }

    foreach ($docs_input as $doc_input) {
        $unicity_fields = [
            'documents_id'      => $doc_input['documents_id'],
            'itemtype'          => $doc_input['itemtype'],
            'items_id'          => $doc_input['items_id'],
            'timeline_position' => $doc_input['timeline_position'],
        ];
        if (countElementsInTable('ntas_documents_items', $unicity_fields) > 0) {
            continue; // Already declared in DB
        }
        $DB->insert('ntas_documents_items', $doc_input);
    }
    /* /Fix document_item migration */

    /* Register missing DomainAlert crontask */
    $migration->addCrontask(
        'Domain',
        'DomainsAlert',
        DAY_TIMESTAMP,
    );
    /* /Register missing DomainAlert crontask */

    //add option to collect only unread mail
    $migration->addField('ntas_mailcollectors', 'collect_only_unread', 'bool', ['value' => 0]);

    /* Appliances rewrite */
    $migration->addField('ntas_appliances', 'is_helpdesk_visible', 'bool', ['after' => 'otherserial', 'value' => 1]);
    $migration->addKey('ntas_appliances', 'is_helpdesk_visible');
    $migration->addField('ntas_states', 'is_visible_appliance', 'bool', [
        'value' => 1,
        'after' => 'is_visible_contract',
    ]);
    $migration->addKey('ntas_states', 'is_visible_appliance');

    if ($DB->tableExists('ntas_appliancerelations')) {
        $migration->dropKey('ntas_appliancerelations', 'relations_id');
        $migration->changeField('ntas_appliancerelations', 'relations_id', 'items_id', 'integer');
        $migration->addField(
            'ntas_appliancerelations',
            'itemtype',
            'VARCHAR(100) COLLATE utf8_unicode_ci NOT NULL',
            ['after' => 'appliances_items_id']
        );
        $migration->addKey('ntas_appliancerelations', 'itemtype');
        $migration->addKey('ntas_appliancerelations', 'items_id');
        $migration->addKey('ntas_appliancerelations', [
            'itemtype',
            'items_id',
        ], 'item');
        $migration->migrationOneTable('ntas_appliancerelations');
        $migration->renameTable('ntas_appliancerelations', 'ntas_appliances_items_relations');
    }

    if ($DB->fieldExists('ntas_appliances', 'relationtype')) {
        $iterator = $DB->request([
            'SELECT' => ['items.id', 'app.relationtype'],
            'FROM'   => 'ntas_appliances_items AS items',
            'LEFT JOIN' => [
                'ntas_appliances AS app' => [
                    'ON'  => [
                        'app'    => 'id',
                        'items'  => 'appliances_id',
                    ],
                ],
            ],
        ]);
        foreach ($iterator as $row) {
            $itemtype = null;
            switch ($row['relationtype']) {
                case 1:
                    $itemtype = 'Location';
                    break;
                case 2:
                    $itemtype = 'Network';
                    break;
                case 3:
                    $itemtype = 'Domain';
                    break;
            }

            $migration->addPostQuery(
                $DB->buildUpdate(
                    'ntas_appliances_items_relations',
                    [
                        'itemtype'  => $itemtype,
                    ],
                    [
                        'appliances_items_id'   => $row['id'],
                    ]
                )
            );
        }
        $migration->dropField('ntas_appliances', 'relationtype');
    }
    /* /Appliances rewrite */

    // ************ Keep it at the end **************
    $migration->executeMigration();

    return $updateresult;
}
