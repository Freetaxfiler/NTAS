<?php

/**
 * Naipunya Enterprise Service Management
 * Copyright (C) 2026 Naipunya Tax and Accounting Solutions Pvt.Ltd.
 */

use Glpi\DBAL\QueryExpression;

/**
 * Update from 9.1 to 9.2
 *
 * @return bool
 **/
function update91xto920()
{
    /**
     * @var DBmysql $DB
     * @var Migration $migration
     */
    global $DB, $migration;

    $current_config   = Config::getConfigurationValues('core');
    $updateresult     = true;
    $ADDTODISPLAYPREF = [];

    $migration->setVersion('9.2');

    // add business criticity
    $migration->addField("ntas_infocoms", "businesscriticities_id", "integer");
    $migration->migrationOneTable('ntas_infocoms');
    $migration->addKey("ntas_infocoms", "businesscriticities_id");

    if (!$DB->tableExists("ntas_businesscriticities")) {
        $query = "CREATE TABLE `ntas_businesscriticities` (
        `id` int NOT NULL AUTO_INCREMENT,
        `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
        `entities_id` int NOT NULL DEFAULT '0',
        `is_recursive` tinyint NOT NULL DEFAULT '0',
        `comment` text COLLATE utf8_unicode_ci,
        `date_mod` datetime DEFAULT NULL,
        `date_creation` datetime DEFAULT NULL,
        `businesscriticities_id` int NOT NULL DEFAULT '0',
        `completename` text COLLATE utf8_unicode_ci,
        `level` int NOT NULL DEFAULT '0',
        `ancestors_cache` longtext COLLATE utf8_unicode_ci,
        `sons_cache` longtext COLLATE utf8_unicode_ci,
        PRIMARY KEY (`id`),
        KEY `name` (`name`),
        KEY `unicity` (`businesscriticities_id`,`name`),
        KEY `date_mod` (`date_mod`),
        KEY `date_creation` (`date_creation`)
                ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    // Issue #1250 - Add decimal to monitor size
    $migration->changeField('ntas_monitors', 'size', 'size', 'DECIMAL(5,2) NOT NULL DEFAULT "0"');

    //Make software license type a tree dropdown
    $migration->addField("ntas_softwarelicensetypes", "softwarelicensetypes_id", "integer");
    $migration->addField("ntas_softwarelicensetypes", "level", "integer");
    $migration->addField("ntas_softwarelicensetypes", "ancestors_cache", "longtext");
    $migration->addField("ntas_softwarelicensetypes", "sons_cache", "longtext");
    $migration->addField("ntas_softwarelicensetypes", "entities_id", "integer");
    $migration->addField("ntas_softwarelicensetypes", "is_recursive", "bool");
    $tree = $migration->addField("ntas_softwarelicensetypes", "completename", "text");
    $migration->migrationOneTable('ntas_softwarelicensetypes');
    $migration->addKey("ntas_softwarelicensetypes", "softwarelicensetypes_id");

    //First time the dropdown is changed from CommonDropdown to CommonTreeDropdown
    if ($tree) {
        $DB->update(
            "ntas_softwarelicensetypes",
            [
                'completename' =>  new QueryExpression(DBmysql::quoteName("name")),
                'is_recursive' => "1",
            ],
            [true]
        );
    }

    // give READ right on components to profiles having UPDATE right
    $DB->update(
        "ntas_profilerights",
        [
            'rights' => new QueryExpression($DB->quoteName("rights") . " | " . READ),
        ],
        [
            new QueryExpression(DBmysql::quoteName("rights") . " & " . DBmysql::quoteValue(UPDATE)),
            'name' => "device",
        ]
    );

    $migration->displayMessage(sprintf(__('Add of - %s to database'), 'Knowbase item link to tickets'));
    if (!$DB->tableExists('ntas_knowbaseitems_items')) {
        $query = "CREATE TABLE `ntas_knowbaseitems_items` (
                 `id` int NOT NULL AUTO_INCREMENT,
                 `knowbaseitems_id` int NOT NULL,
                 `itemtype` varchar(100) COLLATE utf8_unicode_ci NOT NULL,
                 `items_id` int NOT NULL DEFAULT '0',
                 `date_creation` datetime DEFAULT NULL,
                 `date_mod` datetime DEFAULT NULL,
                 PRIMARY KEY (`id`),
                 UNIQUE KEY `unicity` (`itemtype`,`items_id`,`knowbaseitems_id`),
                 KEY `itemtype` (`itemtype`),
                 KEY `item_id` (`items_id`),
                 KEY `item` (`itemtype`,`items_id`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    $migration->displayMessage(sprintf(__('Add of - %s to database'), 'Knowbase item revisions'));
    if (!$DB->tableExists('ntas_knowbaseitems_revisions')) {
        $query = "CREATE TABLE `ntas_knowbaseitems_revisions` (
                 `id` int NOT NULL AUTO_INCREMENT,
                 `knowbaseitems_id` int NOT NULL,
                 `revision` int NOT NULL,
                 `name` text COLLATE utf8_unicode_ci,
                 `answer` longtext COLLATE utf8_unicode_ci,
                 `language` varchar(5) COLLATE utf8_unicode_ci DEFAULT NULL,
                 `users_id` int NOT NULL DEFAULT '0',
                 `date_creation` datetime DEFAULT NULL,
                 PRIMARY KEY (`id`),
                 UNIQUE KEY `unicity` (`knowbaseitems_id`, `revision`, `language`),
                 KEY `revision` (`revision`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    $migration->addField("ntas_knowbaseitemtranslations", "users_id", "integer");
    $migration->migrationOneTable("ntas_knowbaseitemtranslations");
    $migration->addKey("ntas_knowbaseitemtranslations", "users_id");

    //set kb translations users...
    $knowitems_iterator = $DB->request([
        'SELECT' => ['ntas_knowbaseitems.id', 'ntas_knowbaseitems.users_id'],
        'FROM' => 'ntas_knowbaseitems',
        'INNER JOIN' => [
            "ntas_knowbaseitemtranslations" => [
                'FKEY' => [
                    'ntas_knowbaseitemtranslations' => 'knowbaseitems_id',
                    'ntas_knowbaseitems'            => 'id',
                ],
            ],
        ],
    ]);
    foreach ($knowitems_iterator as $knowitems) {
        $DB->update(
            "ntas_knowbaseitemtranslations",
            ['users_id' => $knowitems['users_id']],
            ['knowbaseitems_id' => $knowitems['id']]
        );
    }

    $migration->addField("ntas_knowbaseitemtranslations", "date_mod", "DATETIME");
    $migration->addField("ntas_knowbaseitemtranslations", "date_creation", "DATETIME");

    $migration->displayMessage(sprintf(__('Add of - %s to database'), 'Knowbase item comments'));
    if (!$DB->tableExists('ntas_knowbaseitems_comments')) {
        $query = "CREATE TABLE `ntas_knowbaseitems_comments` (
                 `id` int NOT NULL AUTO_INCREMENT,
                 `knowbaseitems_id` int NOT NULL,
                 `users_id` int NOT NULL DEFAULT '0',
                 `language` varchar(5) COLLATE utf8_unicode_ci DEFAULT NULL,
                 `comment` text COLLATE utf8_unicode_ci NOT NULL,
                 `parent_comment_id` int DEFAULT NULL,
                 `date_creation` datetime DEFAULT NULL,
                 `date_mod` datetime DEFAULT NULL,
                 PRIMARY KEY (`id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    $DB->update(
        "ntas_profilerights",
        [
            'rights' => new QueryExpression(
                DBmysql::quoteName("rights") . " | " . DBmysql::quoteValue(KnowbaseItem::COMMENTS)
            ),
        ],
        ['name' => "knowbase"]
    );

    // add kb category to task categories
    $migration->addField("ntas_taskcategories", "knowbaseitemcategories_id", "integer");
    $migration->migrationOneTable("ntas_taskcategories");
    $migration->addKey("ntas_taskcategories", "knowbaseitemcategories_id");

    // #1476 - Add users_id on ntas_documents_items
    $migration->addField("ntas_documents_items", "users_id", "integer", ['null' => true]);
    $migration->migrationOneTable("ntas_documents_items");
    $migration->addKey("ntas_documents_items", "users_id");
    $migration->addPostQuery(
        $DB->buildUpdate(
            'ntas_documents_items',
            [
                'ntas_documents_items.users_id' => new QueryExpression(DBmysql::quoteName('ntas_documents.users_id')),
            ],
            [
                'ntas_documents_items.documents_id' => new QueryExpression(DBmysql::quoteName('ntas_documents.id')),
            ],
            [
                'LEFT JOIN' => [
                    'ntas_documents' => [
                        'ON' => [
                            'ntas_documents_items' => 'documents_id',
                            'ntas_documents' => 'id',
                        ],
                    ],
                ],
            ]
        ),
        "9.2 update set users_id on ntas_documents_items"
    );

    //add product number
    $product_types = ['Computer',
        'Printer',
        'NetworkEquipment',
        'Phone',
        'Peripheral',
        'Monitor',
    ];

    foreach ($product_types as $type) {
        if (class_exists($type . 'Model')) {
            $table = getTableForItemType($type . 'Model');
            $migration->addField($table, 'product_number', 'string', ['after' => 'comment']);
            $migration->migrationOneTable($table);
            $migration->addKey($table, 'product_number');
        }
    }

    // add fields on every item_device tables
    $tables = ['ntas_items_devicecases',
        'ntas_items_devicecontrols',
        'ntas_items_devicedrives',
        'ntas_items_devicegraphiccards',
        'ntas_items_deviceharddrives',
        'ntas_items_devicememories',
        'ntas_items_devicemotherboards',
        'ntas_items_devicenetworkcards',
        'ntas_items_devicepcis',
        'ntas_items_devicepowersupplies',
        'ntas_items_deviceprocessors',
        'ntas_items_devicesoundcards',
    ];

    //add serial, location and state on each devices items
    foreach ($tables as $table) {
        $migration->addField($table, "otherserial", "varchar(255) NULL DEFAULT NULL");
        $migration->addField($table, "locations_id", "int NOT NULL DEFAULT '0'");
        $migration->addField($table, "states_id", "int NOT NULL DEFAULT '0'");
        $migration->migrationOneTable($table);
        $migration->addKey($table, 'otherserial');
        $migration->addKey($table, 'locations_id');
        $migration->addKey($table, 'states_id');
    }

    // Create tables :
    $tables = ['ntas_devicecasemodels',
        'ntas_devicecontrolmodels',
        'ntas_devicedrivemodels',
        'ntas_devicegraphiccardmodels',
        'ntas_deviceharddrivemodels',
        'ntas_devicememorymodels',
        'ntas_devicemotherboardmodels',
        'ntas_devicenetworkcardmodels',
        'ntas_devicepcimodels',
        'ntas_devicepowersupplymodels',
        'ntas_deviceprocessormodels',
        'ntas_devicesoundcardmodels',
        'ntas_devicegenericmodels',
        'ntas_devicebatterymodels',
        'ntas_devicefirmwaremodels',
        'ntas_devicesensormodels',
    ];

    foreach ($tables as $table) {
        if (!$DB->tableExists($table)) {
            $query = "CREATE TABLE `$table` (
                      `id` INT NOT NULL AUTO_INCREMENT,
                      `name` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                      `comment` TEXT NULL COLLATE 'utf8_unicode_ci',
                      `product_number` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                      PRIMARY KEY (`id`),
                      INDEX `name` (`name`),
                      INDEX `product_number` (`product_number`)
                   ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
            $DB->doQuery($query);
        }
    }

    // Add a field in ntas_device* tables :
    $tables = ['ntas_devicecases'         => 'devicecasemodels_id',
        'ntas_devicecontrols'      => 'devicecontrolmodels_id',
        'ntas_devicedrives'        => 'devicedrivemodels_id',
        'ntas_devicegraphiccards'  => 'devicegraphiccardmodels_id',
        'ntas_deviceharddrives'    => 'deviceharddrivemodels_id',
        'ntas_devicememories'      => 'devicememorymodels_id',
        'ntas_devicemotherboards'  => 'devicemotherboardmodels_id',
        'ntas_devicenetworkcards'  => 'devicenetworkcardmodels_id',
        'ntas_devicepcis'          => 'devicepcimodels_id',
        'ntas_devicepowersupplies' => 'devicepowersupplymodels_id',
        'ntas_deviceprocessors'    => 'deviceprocessormodels_id',
        'ntas_devicesoundcards'    => 'devicesoundcardmodels_id',
    ];

    foreach ($tables as $table => $field) {
        $migration->addField($table, $field, 'int DEFAULT NULL', ['after' => 'is_recursive']);
        $migration->migrationOneTable($table);
        $migration->addKey($table, $field);
    }

    if (!$DB->tableExists('ntas_devicegenerics')) {
        $query = "CREATE TABLE `ntas_devicegenerics` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `designation` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `devicegenerictypes_id` int NOT NULL DEFAULT '0',
                  `comment` text COLLATE utf8_unicode_ci,
                  `manufacturers_id` int NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `locations_id` int NOT NULL DEFAULT '0',
                  `states_id` int NOT NULL DEFAULT '0',
                  `devicegenericmodels_id` int DEFAULT NULL,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `designation` (`designation`),
                  KEY `manufacturers_id` (`manufacturers_id`),
                  KEY `devicegenerictypes_id` (`devicegenerictypes_id`),
                  KEY `entities_id` (`entities_id`),
                  KEY `is_recursive` (`is_recursive`),
                  KEY `locations_id` (`locations_id`),
                  KEY `states_id` (`states_id`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`),
                  KEY `devicegenericmodels_id` (`devicegenericmodels_id`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_items_devicegenerics')) {
        $query = "CREATE TABLE `ntas_items_devicegenerics` (
                   `id` INT NOT NULL AUTO_INCREMENT,
                   `items_id` INT NOT NULL DEFAULT '0',
                   `itemtype` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                   `devicegenerics_id` INT NOT NULL DEFAULT '0',
                   `is_deleted` TINYINT NOT NULL DEFAULT '0',
                   `is_dynamic` TINYINT NOT NULL DEFAULT '0',
                   `entities_id` INT NOT NULL DEFAULT '0',
                   `is_recursive` TINYINT NOT NULL DEFAULT '0',
                   `serial` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                   `otherserial` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                   `locations_id` INT NOT NULL DEFAULT '0',
                   `states_id` INT NOT NULL DEFAULT '0',
                   PRIMARY KEY (`id`),
                   INDEX `computers_id` (`items_id`),
                   INDEX `devicegenerics_id` (`devicegenerics_id`),
                   INDEX `is_deleted` (`is_deleted`),
                   INDEX `is_dynamic` (`is_dynamic`),
                   INDEX `entities_id` (`entities_id`),
                   INDEX `is_recursive` (`is_recursive`),
                   INDEX `serial` (`serial`),
                   INDEX `item` (`itemtype`, `items_id`),
                   INDEX `otherserial` (`otherserial`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_devicegenerictypes')) {
        $query = "CREATE TABLE `ntas_devicegenerictypes` (
                  `id` INT NOT NULL AUTO_INCREMENT,
                  `name` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                  `comment` TEXT NULL COLLATE 'utf8_unicode_ci',
                   PRIMARY KEY (`id`),
                   INDEX `name` (`name`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_devicebatteries')) {
        $query = "CREATE TABLE `ntas_devicebatteries` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `designation` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `manufacturers_id` int NOT NULL DEFAULT '0',
                  `voltage` int DEFAULT NULL,
                  `capacity` int DEFAULT NULL,
                  `devicebatterytypes_id` int NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `devicebatterymodels_id` int DEFAULT NULL,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `designation` (`designation`),
                  KEY `manufacturers_id` (`manufacturers_id`),
                  KEY `entities_id` (`entities_id`),
                  KEY `is_recursive` (`is_recursive`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`),
                  KEY `devicebatterymodels_id` (`devicebatterymodels_id`),
                  KEY `devicebatterytypes_id` (`devicebatterytypes_id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_items_devicebatteries')) {
        $query = "CREATE TABLE `ntas_items_devicebatteries` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `items_id` int NOT NULL DEFAULT '0',
                  `itemtype` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `devicebatteries_id` int NOT NULL DEFAULT '0',
                  `manufacturing_date` date DEFAULT NULL,
                  `is_deleted` tinyint NOT NULL DEFAULT '0',
                  `is_dynamic` tinyint NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `serial` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `otherserial` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `locations_id` int NOT NULL DEFAULT '0',
                  `states_id` int NOT NULL DEFAULT '0',
                  PRIMARY KEY (`id`),
                  KEY `computers_id` (`items_id`),
                  KEY `devicebatteries_id` (`devicebatteries_id`),
                  KEY `is_deleted` (`is_deleted`),
                  KEY `is_dynamic` (`is_dynamic`),
                  KEY `entities_id` (`entities_id`),
                  KEY `is_recursive` (`is_recursive`),
                  KEY `serial` (`serial`),
                  KEY `item` (`itemtype`,`items_id`),
                  KEY `otherserial` (`otherserial`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_devicebatterytypes')) {
        $query = "CREATE TABLE `ntas_devicebatterytypes` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`)
               ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_devicefirmwares')) {
        $query = "CREATE TABLE `ntas_devicefirmwares` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `designation` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `manufacturers_id` int NOT NULL DEFAULT '0',
                  `date` date DEFAULT NULL,
                  `version` varchar(255) DEFAULT NULL,
                  `devicefirmwaretypes_id` int NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `devicefirmwaremodels_id` int DEFAULT NULL,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `designation` (`designation`),
                  KEY `manufacturers_id` (`manufacturers_id`),
                  KEY `entities_id` (`entities_id`),
                  KEY `is_recursive` (`is_recursive`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`),
                  KEY `devicefirmwaremodels_id` (`devicefirmwaremodels_id`),
                  KEY `devicefirmwaretypes_id` (`devicefirmwaretypes_id`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }
    if (!$DB->tableExists('ntas_items_devicefirmwares')) {
        $query = "CREATE TABLE `ntas_items_devicefirmwares` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `items_id` int NOT NULL DEFAULT '0',
                  `itemtype` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `devicefirmwares_id` int NOT NULL DEFAULT '0',
                  `is_deleted` tinyint NOT NULL DEFAULT '0',
                  `is_dynamic` tinyint NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `serial` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `otherserial` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `locations_id` int NOT NULL DEFAULT '0',
                  `states_id` int NOT NULL DEFAULT '0',
                  PRIMARY KEY (`id`),
                  KEY `computers_id` (`items_id`),
                  KEY `devicefirmwares_id` (`devicefirmwares_id`),
                  KEY `is_deleted` (`is_deleted`),
                  KEY `is_dynamic` (`is_dynamic`),
                  KEY `entities_id` (`entities_id`),
                  KEY `is_recursive` (`is_recursive`),
                  KEY `serial` (`serial`),
                  KEY `item` (`itemtype`,`items_id`),
                  KEY `otherserial` (`otherserial`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }
    if (!$DB->tableExists('ntas_devicefirmwaretypes')) {
        $query = "CREATE TABLE `ntas_devicefirmwaretypes` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`)
               ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);

        $DB->insert("ntas_devicefirmwaretypes", [
            'id'              => "1",
            'name'            => "BIOS",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);

        $DB->insert("ntas_devicefirmwaretypes", [
            'id'              => "2",
            'name'            => "UEFI",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);

        $DB->insert("ntas_devicefirmwaretypes", [
            'id'              => "3",
            'name'            => "Firmware",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);
    }

    //Device sensors
    if (!$DB->tableExists('ntas_devicesensors')) {
        $query = "CREATE TABLE `ntas_devicesensors` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `designation` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `devicesensortypes_id` int NOT NULL DEFAULT '0',
                  `devicesensormodels_id` int NOT NULL DEFAULT '0',
                  `comment` text COLLATE utf8_unicode_ci,
                  `manufacturers_id` int NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `locations_id` int NOT NULL DEFAULT '0',
                  `states_id` int NOT NULL DEFAULT '0',
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `designation` (`designation`),
                  KEY `manufacturers_id` (`manufacturers_id`),
                  KEY `devicesensortypes_id` (`devicesensortypes_id`),
                  KEY `entities_id` (`entities_id`),
                  KEY `is_recursive` (`is_recursive`),
                  KEY `locations_id` (`locations_id`),
                  KEY `states_id` (`states_id`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_items_devicesensors')) {
        $query = "CREATE TABLE `ntas_items_devicesensors` (
                   `id` INT NOT NULL AUTO_INCREMENT,
                   `items_id` INT NOT NULL DEFAULT '0',
                   `itemtype` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                   `devicesensors_id` INT NOT NULL DEFAULT '0',
                   `is_deleted` TINYINT NOT NULL DEFAULT '0',
                   `is_dynamic` TINYINT NOT NULL DEFAULT '0',
                   `entities_id` INT NOT NULL DEFAULT '0',
                   `is_recursive` TINYINT NOT NULL DEFAULT '0',
                   `serial` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                   `otherserial` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                   `locations_id` INT NOT NULL DEFAULT '0',
                   `states_id` INT NOT NULL DEFAULT '0',
                   PRIMARY KEY (`id`),
                   INDEX `computers_id` (`items_id`),
                   INDEX `devicesensors_id` (`devicesensors_id`),
                   INDEX `is_deleted` (`is_deleted`),
                   INDEX `is_dynamic` (`is_dynamic`),
                   INDEX `entities_id` (`entities_id`),
                   INDEX `is_recursive` (`is_recursive`),
                   INDEX `serial` (`serial`),
                   INDEX `item` (`itemtype`, `items_id`),
                   INDEX `otherserial` (`otherserial`)
                )
                COLLATE='utf8_unicode_ci'
                ENGINE=InnoDB;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_devicesensortypes')) {
        $query = "CREATE TABLE `ntas_devicesensortypes` (
                  `id` INT NOT NULL AUTO_INCREMENT,
                  `name` VARCHAR(255) NULL DEFAULT NULL COLLATE 'utf8_unicode_ci',
                  `comment` TEXT NULL COLLATE 'utf8_unicode_ci',
                   PRIMARY KEY (`id`),
                   INDEX `name` (`name`)
                )
                COLLATE='utf8_unicode_ci' ENGINE=InnoDB;";
        $DB->doQuery($query);
    }

    //Father/son for Software licenses
    $migration->addField("ntas_softwarelicenses", "softwarelicenses_id", "integer", ['after' => 'softwares_id']);
    $new = $migration->addField("ntas_softwarelicenses", "completename", "text", ['after' => 'softwarelicenses_id']);
    $migration->addField("ntas_softwarelicenses", "level", "integer", ['after' => 'completename']);
    $migration->migrationOneTable("ntas_softwarelicenses");
    if ($new) {
        $DB->update(
            "ntas_softwarelicenses",
            [
                'completename' => new QueryExpression(DBmysql::quoteName("name")),
            ],
            [true]
        );
    }

    // add template key to itiltasks
    $migration->addField("ntas_tickettasks", "tasktemplates_id", "integer");
    $migration->migrationOneTable('ntas_tickettasks');
    $migration->addKey("ntas_tickettasks", "tasktemplates_id");

    $migration->addField("ntas_problemtasks", "tasktemplates_id", "integer");
    $migration->migrationOneTable('ntas_problemtasks');
    $migration->addKey("ntas_problemtasks", "tasktemplates_id");

    $migration->addField("ntas_changetasks", "tasktemplates_id", "integer");
    $migration->migrationOneTable('ntas_changetasks');
    $migration->addKey("ntas_changetasks", "tasktemplates_id");

    // add missing fields to tasktemplate
    $migration->addField("ntas_tasktemplates", "state", "integer");
    $migration->addField("ntas_tasktemplates", "is_private", "bool");
    $migration->addField("ntas_tasktemplates", "users_id_tech", "integer");
    $migration->addField("ntas_tasktemplates", "groups_id_tech", "integer");
    $migration->migrationOneTable('ntas_tasktemplates');
    $migration->addKey("ntas_tickettasks", "is_private");
    $migration->addKey("ntas_tickettasks", "users_id_tech");
    $migration->addKey("ntas_tickettasks", "groups_id_tech");

    // #1735 - Add new notifications
    $new_notifications  = [
        'requester_user'  => ['label'      => 'New user in requesters',
            'targets_id' => Notification::AUTHOR,
        ],
        'requester_group' => ['label'      => 'New group in requesters',
            'targets_id' => Notification::REQUESTER_GROUP,
        ],
        'observer_user'   => ['label'      => 'New user in observers',
            'targets_id' => Notification::OBSERVER,
        ],
        'observer_group'  => ['label'      => 'New group in observers',
            'targets_id' => Notification::OBSERVER_GROUP,
        ],
        'assign_user'     => ['label'      => 'New user in assignees',
            'targets_id' => Notification::ASSIGN_TECH,
        ],
        'assign_group'    => ['label'      => 'New group in assignees',
            'targets_id' => Notification::ITEM_TECH_GROUP_IN_CHARGE,
        ],
        'assign_supplier' => ['label'      => 'New supplier in assignees',
            'targets_id' => Notification::SUPPLIER,
        ],
    ];

    if ($DB->fieldExists("ntas_notifications", "mode", false)) {
        $notificationtemplates_id = $DB->request([
            'FROM' => 'ntas_notificationtemplates',
            'WHERE' => [
                'name'     => 'Tickets',
                'itemtype' => 'Ticket',
            ],
        ])->current()['id'] ?? 0;

        if ($notificationtemplates_id > 0) {
            foreach ($new_notifications as $event => $notif_options) {
                $DB->insert(
                    'ntas_notifications',
                    [
                        'name'                     => $notif_options['label'],
                        'itemtype'                 => 'Ticket',
                        'event'                    => $event,
                        'mode'                     => 'mailing',
                        'notificationtemplates_id' => $notificationtemplates_id,
                        'comment'                  => null,
                        'entities_id'              => 0,
                        'is_recursive'             => 1,
                        'is_active'                => 1,
                        'date_creation'            => new QueryExpression('NOW()'),
                        'date_mod'                 => new QueryExpression('NOW()'),
                    ]
                );
                $notifications_id = $DB->insertId();

                $DB->insert(
                    'ntas_notificationtargets',
                    [
                        'items_id'         => $notif_options['targets_id'],
                        'type'             => 1,
                        'notifications_id' => $notifications_id,
                    ]
                );
            }
        }
    }

    $migration->addField('ntas_states', 'is_visible_certificate', 'bool', ['value' => 1]);
    $migration->addKey('ntas_states', 'is_visible_certificate');

    /** ************ New SLM structure ************ */
    if (!$DB->tableExists('ntas_olas')) {
        $query = "CREATE TABLE `ntas_olas` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `entities_id` int NOT NULL DEFAULT '0',
                  `is_recursive` tinyint NOT NULL DEFAULT '0',
                  `type` int NOT NULL DEFAULT '0',
                  `comment` text COLLATE utf8_unicode_ci,
                  `number_time` int NOT NULL,
                  `calendars_id` int NOT NULL DEFAULT '0',
                  `date_mod` datetime DEFAULT NULL,
                  `definition_time` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `end_of_working_day` tinyint NOT NULL DEFAULT '0',
                  `date_creation` datetime DEFAULT NULL,
                  `slms_id` int NOT NULL DEFAULT '0',
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`),
                  KEY `calendars_id` (`calendars_id`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`),
                  KEY `slms_id` (`slms_id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_olalevelactions')) {
        $query = "CREATE TABLE `ntas_olalevelactions` (
               `id` int NOT NULL AUTO_INCREMENT,
               `olalevels_id` int NOT NULL DEFAULT '0',
               `action_type` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               `field` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               `value` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               PRIMARY KEY (`id`),
               KEY `olalevels_id` (`olalevels_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_olalevelcriterias')) {
        $query = "CREATE TABLE `ntas_olalevelcriterias` (
               `id` int NOT NULL AUTO_INCREMENT,
               `olalevels_id` int NOT NULL DEFAULT '0',
               `criteria` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               `condition` int NOT NULL DEFAULT '0' COMMENT 'see define.php PATTERN_* and REGEX_* constant',
               `pattern` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               PRIMARY KEY (`id`),
               KEY `olalevels_id` (`olalevels_id`),
               KEY `condition` (`condition`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_olalevels')) {
        $query = "CREATE TABLE `ntas_olalevels` (
               `id` int NOT NULL AUTO_INCREMENT,
               `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               `olas_id` int NOT NULL DEFAULT '0',
               `execution_time` int NOT NULL,
               `is_active` tinyint NOT NULL DEFAULT '1',
               `entities_id` int NOT NULL DEFAULT '0',
               `is_recursive` tinyint NOT NULL DEFAULT '0',
               `match` char(10) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'see define.php *_MATCHING constant',
               `uuid` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
               PRIMARY KEY (`id`),
               KEY `name` (`name`),
               KEY `is_active` (`is_active`),
               KEY `olas_id` (`olas_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_olalevels_tickets')) {
        $query = "CREATE TABLE `ntas_olalevels_tickets` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `tickets_id` int NOT NULL DEFAULT '0',
                  `olalevels_id` int NOT NULL DEFAULT '0',
                  `date` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `tickets_id` (`tickets_id`),
                  KEY `olalevels_id` (`olalevels_id`),
                  KEY `unicity` (`tickets_id`,`olalevels_id`)
               ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);

        $DB->updateOrInsert("ntas_crontasks", [
            'frequency'       => "604800",
            'param'           => null,
            'state'           => "1",
            'mode'            => "1",
            'allowmode'       => "3",
            'hourmin'         => "0",
            'hourmax'         => "24",
            'logs_lifetime'   => "30",
            'lastrun'         => null,
            'lastcode'        => null,
            'comment'         => null,
        ], [
            'itemtype'  => "OlaLevel_Ticket",
            'name'      => "olaticket",
        ]);
    }

    if (!$DB->tableExists('ntas_slms')) {
        // Changing the structure of the table 'ntas_slas'
        $migration->renameTable('ntas_slas', 'ntas_slms');
        $migration->migrationOneTable('ntas_slas');
    }

    // Changing the structure of the table 'ntas_slts'
    if ($DB->tableExists('ntas_slts')) {
        $migration->renameTable('ntas_slts', 'ntas_slas');
        $migration->migrationOneTable('ntas_slts');
        $migration->changeField('ntas_slas', 'slas_id', 'slms_id', 'integer');
        $migration->dropKey('ntas_slas', 'slas_id');
        $migration->addKey('ntas_slas', 'slms_id');
    }

    // Slalevels changes
    if ($DB->fieldExists("ntas_slalevels", "slts_id")) {
        $migration->changeField('ntas_slalevels', 'slts_id', 'slas_id', 'integer');
        $migration->migrationOneTable('ntas_slalevels');
        $migration->dropKey('ntas_slalevels', 'slts_id');
        $migration->addKey('ntas_slalevels', 'slas_id');
    }

    // Ticket changes
    if (!$DB->fieldExists("ntas_tickets", "ola_waiting_duration", false)) {
        $migration->addField(
            "ntas_tickets",
            "ola_waiting_duration",
            "integer",
            ['after' => 'sla_waiting_duration']
        );
        $migration->migrationOneTable('ntas_tickets');
    }

    if (!$DB->fieldExists("ntas_tickets", "olas_tto_id", false)) {
        $migration->addField("ntas_tickets", "olas_tto_id", "integer", ['after' => 'ola_waiting_duration']);
        $migration->migrationOneTable('ntas_tickets');
        $migration->addKey('ntas_tickets', 'olas_tto_id');
    }

    if (!$DB->fieldExists("ntas_tickets", "olas_ttr_id", false)) {
        $migration->addField("ntas_tickets", "olas_ttr_id", "integer", ['after' => 'olas_tto_id']);
        $migration->migrationOneTable('ntas_tickets');
        $migration->addKey('ntas_tickets', 'olas_ttr_id');
    }

    if (!$DB->fieldExists("ntas_tickets", "ttr_olalevels_id", false)) {
        $migration->addField("ntas_tickets", "ttr_olalevels_id", "integer", ['after' => 'olas_ttr_id']);
        $migration->migrationOneTable('ntas_tickets');
    }

    if (!$DB->fieldExists("ntas_tickets", "internal_time_to_resolve", false)) {
        $migration->addField(
            "ntas_tickets",
            "internal_time_to_resolve",
            "datetime",
            ['after' => 'ttr_olalevels_id']
        );
        $migration->migrationOneTable('ntas_tickets');
        $migration->addKey('ntas_tickets', 'internal_time_to_resolve');
    }

    if (!$DB->fieldExists("ntas_tickets", "internal_time_to_own", false)) {
        $migration->addField(
            "ntas_tickets",
            "internal_time_to_own",
            "datetime",
            ['after' => 'internal_time_to_resolve']
        );
        $migration->migrationOneTable('ntas_tickets');
        $migration->addKey('ntas_tickets', 'internal_time_to_own');
    }

    if ($DB->fieldExists("ntas_tickets", "slts_tto_id")) {
        $migration->changeField("ntas_tickets", "slts_tto_id", "slas_tto_id", "integer");
        $migration->migrationOneTable('ntas_tickets');
        $migration->addKey('ntas_tickets', 'slas_tto_id');
        $migration->dropKey('ntas_tickets', 'slts_tto_id');
    }

    if ($DB->fieldExists("ntas_tickets", "slts_ttr_id")) {
        $migration->changeField("ntas_tickets", "slts_ttr_id", "slas_ttr_id", "integer");
        $migration->migrationOneTable('ntas_tickets');
        $migration->addKey('ntas_tickets', 'slas_ttr_id');
        $migration->dropKey('ntas_tickets', 'slts_ttr_id');
    }
    if ($DB->fieldExists("ntas_tickets", "due_date")) {
        $migration->changeField('ntas_tickets', 'due_date', 'time_to_resolve', 'datetime');
        $migration->migrationOneTable('ntas_tickets');
        $migration->dropKey('ntas_tickets', 'due_date');
        $migration->addKey('ntas_tickets', 'time_to_resolve');
    }

    //Change changes
    if ($DB->fieldExists("ntas_changes", "due_date")) {
        $migration->changeField('ntas_changes', 'due_date', 'time_to_resolve', 'datetime');
        $migration->migrationOneTable('ntas_changes');
        $migration->dropKey('ntas_changes', 'due_date');
        $migration->addKey('ntas_changes', 'time_to_resolve');
    }

    //Problem changes
    if ($DB->fieldExists("ntas_problems", "due_date")) {
        $migration->changeField('ntas_problems', 'due_date', 'time_to_resolve', 'datetime');
        $migration->migrationOneTable('ntas_problems');
        $migration->dropKey('ntas_problems', 'due_date');
        $migration->addKey('ntas_problems', 'time_to_resolve');
    }

    // ProfileRights changes
    $DB->update(
        "ntas_profilerights",
        ['name' => "slm"],
        ['name' => "sla"]
    );

    //Sla rules criterias migration
    $DB->update(
        "ntas_rulecriterias",
        ['criteria' => "slas_ttr_id"],
        ['criteria' => "slts_ttr_id"]
    );

    $DB->update(
        "ntas_rulecriterias",
        ['criteria' => "slas_tto_id"],
        ['criteria' => "slts_tto_id"]
    );

    // Sla rules actions migration
    $DB->update(
        "ntas_ruleactions",
        ['field' => "slas_ttr_id"],
        ['field' => "slts_ttr_id"]
    );

    $DB->update(
        "ntas_ruleactions",
        ['field' => "slas_tto_id"],
        ['field' => "slts_tto_id"]
    );

    /************** Auto login **************/
    $migration->addConfig([
        'login_remember_time'      => 604800,
        'login_remember_default'   => 1,
    ]);

    if ($DB->tableExists('ntas_bookmarks')) {
        $migration->renameTable("ntas_bookmarks", "ntas_savedsearches");

        $migration->addField("ntas_savedsearches", "last_execution_time", "int NULL DEFAULT NULL");
        $migration->addField(
            "ntas_savedsearches",
            "do_count",
            "tinyint NOT NULL DEFAULT '2' COMMENT 'Do or do not count results on list display; see SavedSearch::COUNT_* constants'"
        );
        $migration->addField(
            "ntas_savedsearches",
            "last_execution_date",
            "DATETIME NULL DEFAULT NULL"
        );
        $migration->addField("ntas_savedsearches", "counter", "int NOT NULL DEFAULT '0'");
        $migration->migrationOneTable("ntas_savedsearches");
        $migration->addKey("ntas_savedsearches", 'last_execution_time');
        $migration->addKey("ntas_savedsearches", 'do_count');
        $migration->addKey("ntas_savedsearches", 'last_execution_date');
    }
    //ensure do_count is set to AUTO
    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_savedsearches",
            ['do_count' => SavedSearch::COUNT_AUTO],
            [new QueryExpression('true')]
        ),
        'Set count auto on saved searches'
    );

    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_savedsearches",
            ['entities_id' => 0],
            ['entities_id' => -1]
        ),
        'Update entities_id in saved searches'
    );

    if (
        !countElementsInTable(
            'ntas_rules',
            ['sub_type' => 'RuleSoftwareCategory',
                'uuid'     => '500717c8-2bd6e957-53a12b5fd38869.86003425',
            ]
        )
    ) {
        $migration->createRule(
            [
                'name'         => 'Import category from inventory tool',
                'uuid'         => '500717c8-2bd6e957-53a12b5fd38869.86003425',
                'entities_id'  => 0,
                'is_recursive' => 1,
                'sub_type'     => 'RuleSoftwareCategory',
                'match'        => Rule::AND_MATCHING,
                'condition'    => 1,
            ],
            [
                [
                    'criteria'  => 'name',
                    'condition' => '0',
                    'pattern'   => '*',
                ],
            ],
            [
                [
                    'action_type' => 'assign',
                    'field'       => '_import_category',
                    'value'       => '1',
                ],
            ]
        );
    }

    if ($DB->tableExists('ntas_queuedmails')) {
        $migration->renameTable("ntas_queuedmails", "ntas_queuednotifications");
    }

    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_crontasks",
            ['itemtype' => "QueuedNotification"],
            ['itemtype' => "QueuedMail"]
        )
    );
    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_crontasks",
            ['name' => "queuednotification"],
            ['name' => "queuedmail"]
        )
    );
    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_crontasks",
            ['name' => "queuednotificationclean"],
            ['name' => "queuedmailclean"]
        )
    );
    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_profilerights",
            ['name' => "queuednotification"],
            ['name' => "queuedmail"]
        )
    );

    if (isset($current_config['use_mailing']) && !isset($current_config['use_notifications'])) {
        /** Notifications modes */
        $migration->addConfig([
            'use_notifications'                 => $current_config['use_mailing'],
            'notifications_mailing'    => $current_config['use_mailing'],
            'notifications_ajax'       => 0,
            'notifications_ajax_check_interval' => '5',
            'notifications_ajax_sound' => null,
            'notifications_ajax_icon_url'       => '/pics/glpi.png',
        ]);
        $migration->removeConfig(['use_mailing']);
    }

    if (!$DB->tableExists('ntas_notifications_notificationtemplates')) {
        $query = "CREATE TABLE `ntas_notifications_notificationtemplates` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `notifications_id` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `mode` varchar(20) COLLATE utf8_unicode_ci NOT NULL COMMENT 'See Notification_NotificationTemplate::MODE_* constants',
                  `notificationtemplates_id` int NOT NULL DEFAULT '0',
                  PRIMARY KEY (`id`),
                  UNIQUE KEY `unicity` (`notifications_id`, `mode`, `notificationtemplates_id`),
                  KEY `notifications_id` (`notifications_id`),
                  KEY `notificationtemplates_id` (`notificationtemplates_id`),
                  KEY `mode` (`mode`)
                ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if ($DB->fieldExists("ntas_notifications", "mode", false)) {
        // TODO can be done when DB::updateOrInsert() supports SELECT
        $query = "REPLACE INTO `ntas_notifications_notificationtemplates`
                       (`notifications_id`, `mode`, `notificationtemplates_id`)
                       SELECT `id`, `mode`, `notificationtemplates_id`
                       FROM `ntas_notifications`";
        $DB->doQuery($query);

        //migrate any existing mode before removing the field
        $migration->dropField('ntas_notifications', 'mode');
        $migration->dropField('ntas_notifications', 'notificationtemplates_id');

        $migration->migrationOneTable("ntas_notifications");
    }

    $migration->addField(
        'ntas_queuednotifications',
        'mode',
        'varchar(20) COLLATE utf8_unicode_ci NOT NULL COMMENT \'See Notification_NotificationTemplate::MODE_* constants\''
    );
    $migration->migrationOneTable("ntas_queuednotifications");
    $migration->addKey('ntas_queuednotifications', 'mode');
    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_queuednotifications",
            ['mode' => Notification_NotificationTemplate::MODE_MAIL],
            [true]
        ),
        "9.2 set default mode in queue"
    );

    $migration->addPostQuery(
        $DB->buildUpdate(
            "ntas_notifications_notificationtemplates",
            ['mode' => Notification_NotificationTemplate::MODE_MAIL],
            ['mode' => "mail"]
        ),
        "9.2 set default mode in notifications templates"
    );

    // Migration Bookmark -> SavedSearch_Alert
    //TRANS: %s is the table or item to migrate
    if ($DB->tableExists('ntas_bookmarks_users')) {
        $migration->renameTable("ntas_bookmarks_users", "ntas_savedsearches_users");
        $migration->changeField(
            'ntas_savedsearches_users',
            'bookmarks_id',
            'savedsearches_id',
            'int NOT NULL DEFAULT "0"'
        );
    }

    if (!$DB->tableExists('ntas_savedsearches_alerts')) {
        $query = "CREATE TABLE `ntas_savedsearches_alerts` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `savedsearches_id` int NOT NULL DEFAULT '0',
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `is_active` tinyint NOT NULL DEFAULT '0',
                  `operator` tinyint NOT NULL,
                  `value` int NOT NULL,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`),
                  KEY `is_active` (`is_active`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`),
                  UNIQUE KEY `unicity` (`savedsearches_id`,`operator`, `value`)
                 ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    $migration->displayMessage(sprintf(__('Data migration - %s'), 'ntas_displaypreferences'));

    $ADDTODISPLAYPREF['SavedSearch'] = [8, 9, 3, 10, 11];
    foreach ($ADDTODISPLAYPREF as $type => $tab) {
        $rank = 1;
        foreach ($tab as $newval) {
            $DB->updateOrInsert("ntas_displaypreferences", [
                'rank'      => $rank++,
            ], [
                'users_id'  => "0",
                'itemtype'  => $type,
                'num'       => $newval,
            ]);
        }
    }

    if (countElementsInTable('ntas_logs') < 2000000) {
        //add index only if this sounds... possible.
        $migration->addKey("ntas_logs", "id_search_option");
    } else {
        //Just display a Warning to the user.
        $migration->addWarningMessage("An index must be added in the 'id_search_option' field "
         . "of the 'ntas_logs table'; but your ntas_logs table is "
         . "too huge. You'll have to add it on your database "
         . "with the following query:\n"
         . "'ALTER TABLE ntas_logs ADD INDEX id_search_option(id_search_option);'");
    }

    // count cron task
    if (
        !countElementsInTable(
            'ntas_crontasks',
            ['itemtype' => 'SavedSearch', 'name' => 'countAll']
        )
    ) {
        $DB->insert(
            "ntas_crontasks",
            [
                'itemtype'        => "SavedSearch",
                'name'            => "countAll",
                'frequency'       => "604800",
                'param'           => null,
                'state'           => "0",
                'mode'            => "1",
                'allowmode'       => "3",
                'hourmin'         => "0",
                'hourmax'         => "24",
                'logs_lifetime'   => "30",
                'lastrun'         => null,
                'lastcode'        => null,
                'comment'         => null,
            ]
        );
    };

    // alerts cron task
    if (
        !countElementsInTable(
            'ntas_crontasks',
            ['itemtype' => 'SavedSearch_Alert', 'name' => 'savedsearchesalerts']
        )
    ) {
        $DB->insert(
            "ntas_crontasks",
            [
                'itemtype'        => "SavedSearch_Alert",
                'name'            => "savedsearchesalerts",
                'frequency'       => "86400",
                'param'           => null,
                'state'           => "0",
                'mode'            => "1",
                'allowmode'       => "3",
                'hourmin'         => "0",
                'hourmax'         => "24",
                'logs_lifetime'   => "30",
                'lastrun'         => null,
                'lastcode'        => null,
                'comment'         => null,
            ]
        );
    }

    if (
        !countElementsInTable(
            'ntas_notifications',
            ['itemtype' => 'SavedSearch_Alert']
        )
    ) {
        $DB->insert(
            "ntas_notifications",
            [
                'id'              => null,
                'name'            => "Saved searches",
                'entities_id'     => "0",
                'itemtype'        => "SavedSearch_Alert",
                'event'           => "alert",
                'comment'         => "",
                'is_recursive'    => "1",
                'is_active'       => "1",
                'date_creation'   => new QueryExpression("NOW()"),
                'date_mod'        => new QueryExpression("NOW()"),
            ]
        );
        $notid = $DB->insertId();

        $DB->insert(
            "ntas_notificationtemplates",
            [
                'name'            => "Saved searches alerts",
                'itemtype'        => "SavedSearch_Alert",
                'date_mod'        => new QueryExpression("NOW()"),
            ]
        );
        $nottid = $DB->insertId();

        $where =  [
            'notifications_id'         => $notid,
            'mode'                     => Notification_NotificationTemplate::MODE_MAIL,
            'notificationtemplates_id' => $nottid,
        ];
        if (countElementsInTable('ntas_notifications_notificationtemplates', $where)) {
            $DB->updateOrInsert("ntas_notifications_notificationtemplates", [
                'id'                       => null,
            ], [
                'notifications_id'         => $notid,
                'mode'                     => Notification_NotificationTemplate::MODE_MAIL,
                'notificationtemplates_id' => $nottid,
            ]);
        }

        $DB->insert(
            "ntas_notificationtargets",
            [
                'id'               => null,
                'items_id'         => "19",
                'type'             => "1",
                'notifications_id' => $notid,
            ]
        );

        $query = "INSERT INTO `ntas_notificationtemplatetranslations`
                       (`notificationtemplates_id`, `language`,`subject`,
                              `content_text`,
                              `content_html`)
                     VALUES ($notid, '', '##savedsearch.action## ##savedsearch.name##',
                     '##savedsearch.type## ###savedsearch.id## - ##savedsearch.name##

##savedsearch.message##

##lang.savedsearch.url##
##savedsearch.url##

Regards,',
                     '&lt;table&gt;
                     &lt;tbody&gt;
                     &lt;tr&gt;&lt;th colspan=\"2\"&gt;&lt;a href=\"##savedsearch.url##\"&gt;##savedsearch.type## ###savedsearch.id## - ##savedsearch.name##&lt;/a&gt;&lt;/th&gt;&lt;/tr&gt;
                     &lt;tr&gt;&lt;td colspan=\"2\"&gt;&lt;a href=\"##savedsearch.url##\"&gt;##savedsearch.message##&lt;/a&gt;&lt;/td&gt;&lt;/tr&gt;
                     &lt;tr&gt;
                     &lt;td&gt;##lang.savedsearch.url##&lt;/td&gt;
                     &lt;td&gt;##savedsearch.url##&lt;/td&gt;
                     &lt;/tr&gt;
                     &lt;/tbody&gt;
                     &lt;/table&gt;
                     &lt;p&gt;&lt;span style=\"font-size: small;\"&gt;Hello &lt;br /&gt;Regards,&lt;/span&gt;&lt;/p&gt;')";

        $DB->doQuery($query);
    }

    // Create a dedicated token for api
    if (!$DB->fieldExists('ntas_users', 'api_token')) {
        $migration->addField('ntas_users', 'api_token', 'string', ['after' => 'personal_token_date']);
        $migration->addField('ntas_users', 'api_token_date', 'datetime', ['after' => 'api_token']);
        $migration->addWarningMessage(
            "Api users tokens has been reset, if you use REST/XMLRPC api with personal token for authentication, please reset your user's token."
        );
    }

    if (!$DB->tableExists('ntas_items_operatingsystems')) {
        $query = "CREATE TABLE `ntas_items_operatingsystems` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `items_id` int NOT NULL DEFAULT '0',
                  `itemtype` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `operatingsystems_id` int NOT NULL DEFAULT '0',
                  `operatingsystemversions_id` int NOT NULL DEFAULT '0',
                  `operatingsystemservicepacks_id` int NOT NULL DEFAULT '0',
                  `operatingsystemarchitectures_id` int NOT NULL DEFAULT '0',
                  `operatingsystemkernelversions_id` int NOT NULL DEFAULT '0',
                  `license_number` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `license_id` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `operatingsystemeditions_id` int NOT NULL DEFAULT '0',
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `items_id` (`items_id`),
                  KEY `item` (`itemtype`,`items_id`),
                  KEY `operatingsystems_id` (`operatingsystems_id`),
                  KEY `operatingsystemservicepacks_id` (`operatingsystemservicepacks_id`),
                  KEY `operatingsystemversions_id` (`operatingsystemversions_id`),
                  KEY `operatingsystemarchitectures_id` (`operatingsystemarchitectures_id`),
                  KEY `operatingsystemkernelversions_id` (`operatingsystemkernelversions_id`),
                  KEY `operatingsystemeditions_id` (`operatingsystemeditions_id`),
                  UNIQUE KEY `unicity`(`items_id`,`itemtype`, `operatingsystems_id`,
                                       `operatingsystemarchitectures_id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_operatingsystemkernels')) {
        $query = "CREATE TABLE `ntas_operatingsystemkernels` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_operatingsystemkernelversions')) {
        $query = "CREATE TABLE `ntas_operatingsystemkernelversions` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `operatingsystemkernels_id` int NOT NULL DEFAULT '0',
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`),
                  KEY `operatingsystemkernels_id` (`operatingsystemkernels_id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_operatingsystemeditions')) {
        $query = "CREATE TABLE `ntas_operatingsystemeditions` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                  `comment` text COLLATE utf8_unicode_ci,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if ($DB->fieldExists('ntas_computers', 'operatingsystems_id')) {
        //migrate data from computers table, and drop old fields
        // TODO can be done when DB::updateOrInsert() supports SELECT
        $query = "REPLACE INTO `ntas_items_operatingsystems`
                       (`itemtype`, `items_id`, `operatingsystems_id`, `operatingsystemversions_id`,
                        `operatingsystemservicepacks_id`, `operatingsystemarchitectures_id`,
                        `license_number`, `license_id`)
                       SELECT 'Computer', `id`, `operatingsystems_id`, `operatingsystemversions_id`,
                              `operatingsystemservicepacks_id`, `operatingsystemarchitectures_id`,
                              `os_license_number`, `os_licenseid`
                FROM `ntas_computers`
                WHERE `operatingsystems_id` != 0
                      OR `operatingsystemservicepacks_id` != 0
                      OR `operatingsystemarchitectures_id` != 0
                      OR `os_license_number` IS NOT NULL
                      OR `os_kernel_version` IS NOT NULL
                      OR `os_licenseid` IS NOT NULL";
        $DB->doQuery($query);

        //migrate kernel versions.
        $mapping = [];
        foreach (
            $DB->request(['SELECT' => ['id', 'os_kernel_version'],
                'FROM'   => 'ntas_computers',
                'NOT'   => ['os_kernel_version' => null],
            ]) as $data
        ) {
            $key = md5($data['os_kernel_version']);
            if (!isset($mapping[$key])) {
                $mapping[$key] = [];
            }
            $DB->insert('ntas_operatingsystemkernelversions', ['name' => $data['os_kernel_version']]);
            $kver_id = $DB->insertId();
            $mapping[$key][$data['id']] = $kver_id;
        }

        foreach ($mapping as $map) {
            foreach ($map as $computers_id => $kver_id) {
                $DB->update(
                    "ntas_items_operatingsystems",
                    ['operatingsystemkernelversions_id' => $kver_id],
                    [
                        'itemtype' => "Computer",
                        'items_id' => $computers_id,
                    ]
                );
            }
        }

        $migration->dropKey('ntas_computers', 'operatingsystems_id');
        $migration->dropField('ntas_computers', 'operatingsystems_id');
        $migration->dropKey('ntas_computers', 'operatingsystemservicepacks_id');
        $migration->dropField('ntas_computers', 'operatingsystemservicepacks_id');
        $migration->dropKey('ntas_computers', 'operatingsystemversions_id');
        $migration->dropField('ntas_computers', 'operatingsystemversions_id');
        $migration->dropKey('ntas_computers', 'operatingsystemarchitectures_id');
        $migration->dropField('ntas_computers', 'operatingsystemarchitectures_id');
        $migration->dropField('ntas_computers', 'os_license_number');
        $migration->dropField('ntas_computers', 'os_licenseid');
        $migration->dropField('ntas_computers', 'os_kernel_version');
    }

    if (!$DB->fieldExists('ntas_items_operatingsystems', 'is_deleted')) {
        $migration->addField(
            'ntas_items_operatingsystems',
            'is_deleted',
            "tinyint NOT NULL DEFAULT '0'"
        );
        $migration->addKey('ntas_items_operatingsystems', 'is_deleted');
    }

    if (!$DB->fieldExists('ntas_items_operatingsystems', 'is_dynamic')) {
        $migration->addField(
            'ntas_items_operatingsystems',
            'is_dynamic',
            "tinyint NOT NULL DEFAULT '0'"
        );
        $migration->addKey('ntas_items_operatingsystems', 'is_dynamic');
    }

    if (!$DB->fieldExists('ntas_items_operatingsystems', 'entities_id')) {
        $migration->addField(
            'ntas_items_operatingsystems',
            'entities_id',
            "int NOT NULL DEFAULT '0'"
        );
        $migration->addKey('ntas_items_operatingsystems', 'entities_id');
    }

    //add db version
    $migration->addConfig(['dbversion' => '9.1.3']);

    // Add certificates management
    if (!$DB->tableExists('ntas_certificates')) {
        $query = "CREATE TABLE `ntas_certificates` (
        `id` INT NOT NULL AUTO_INCREMENT,
        `name` VARCHAR(255) COLLATE utf8_unicode_ci  DEFAULT NULL,
        `serial` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
        `otherserial` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
        `entities_id` INT NOT NULL DEFAULT '0',
        `is_recursive` TINYINT NOT NULL DEFAULT '0',
        `comment` text COLLATE utf8_unicode_ci,
        `is_deleted` tinyint NOT NULL DEFAULT '0',
        `is_template` tinyint NOT NULL DEFAULT '0',
        `template_name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
        `certificatetypes_id`  INT NOT NULL DEFAULT '0' COMMENT 'RELATION to ntas_certificatetypes (id)',
        `dns_name` VARCHAR(255) COLLATE utf8_unicode_ci  DEFAULT NULL,
        `dns_suffix` VARCHAR(255) COLLATE utf8_unicode_ci  DEFAULT NULL,
        `users_id_tech` INT NOT NULL DEFAULT '0' COMMENT 'RELATION to ntas_users (id)',
        `groups_id_tech` INT NOT NULL DEFAULT '0' COMMENT 'RELATION to ntas_groups (id)',
        `locations_id` INT NOT NULL DEFAULT '0' COMMENT 'RELATION to ntas_locations (id)',
        `manufacturers_id` INT NOT NULL DEFAULT '0' COMMENT 'RELATION to ntas_manufacturers (id)',
        `users_id` int NOT NULL DEFAULT '0',
        `groups_id` int NOT NULL DEFAULT '0',
        `is_autosign` TINYINT NOT NULL DEFAULT '0',
        `date_expiration` DATE DEFAULT NULL,
        `states_id` INT NOT NULL DEFAULT '0' COMMENT 'RELATION to states (id)',
        `command` TEXT COLLATE utf8_unicode_ci,
        `certificate_request` TEXT COLLATE utf8_unicode_ci,
        `certificate_item` TEXT COLLATE utf8_unicode_ci,
        `date_creation` DATETIME DEFAULT NULL,
        `date_mod` DATETIME DEFAULT NULL,
        PRIMARY KEY (`id`),
        KEY `name` (`name`),
        KEY `entities_id` (`entities_id`),
        KEY `is_template` (`is_template`),
        KEY `is_deleted` (`is_deleted`),
        KEY `certificatetypes_id` (`certificatetypes_id`),
        KEY `users_id_tech` (`users_id_tech`),
        KEY `groups_id_tech` (`groups_id_tech`),
        KEY `groups_id` (`groups_id`),
        KEY `users_id` (`users_id`),
        KEY `locations_id` (`locations_id`),
        KEY `manufacturers_id` (`manufacturers_id`),
        KEY `states_id` (`states_id`),
        KEY `date_creation` (`date_creation`),
        KEY `date_mod` (`date_mod`)
      ) ENGINE = InnoDB DEFAULT CHARSET = utf8 COLLATE = utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_certificates_items')) {
        $query = "CREATE TABLE `ntas_certificates_items` (
           `id` INT NOT NULL AUTO_INCREMENT,
           `certificates_id` INT NOT NULL DEFAULT '0',
           `items_id` INT NOT NULL DEFAULT '0' COMMENT 'RELATION to various tables, according to itemtype (id)',
           `itemtype` VARCHAR(100) COLLATE utf8_unicode_ci NOT NULL COMMENT 'see .class.php file',
           `date_creation` DATETIME DEFAULT NULL,
           `date_mod` DATETIME DEFAULT NULL,
           PRIMARY KEY (`id`),
           UNIQUE KEY `unicity` (`certificates_id`, `itemtype`, `items_id`),
           KEY `device` (`items_id`, `itemtype`),
           KEY `item` (`itemtype`, `items_id`),
           KEY `date_creation` (`date_creation`),
           KEY `date_mod` (`date_mod`)
        ) ENGINE = InnoDB DEFAULT CHARSET = utf8 COLLATE = utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_certificatetypes')) {
        $query = "CREATE TABLE `ntas_certificatetypes` (
           `id` INT NOT NULL AUTO_INCREMENT,
           `entities_id` INT NOT NULL DEFAULT '0',
           `is_recursive` TINYINT NOT NULL DEFAULT '0',
           `name` VARCHAR(255) COLLATE utf8_unicode_ci DEFAULT NULL,
           `comment` TEXT COLLATE utf8_unicode_ci,
           `date_creation` DATETIME DEFAULT NULL,
           `date_mod` DATETIME DEFAULT NULL,
           PRIMARY KEY (`id`),
           KEY `entities_id` (`entities_id`),
           KEY `is_recursive` (`is_recursive`),
           KEY `name` (`name`),
           KEY `date_creation` (`date_creation`),
           KEY `date_mod` (`date_mod`)
        ) ENGINE = InnoDB DEFAULT CHARSET = utf8 COLLATE = utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (countElementsInTable("ntas_profilerights", ['name' => 'certificate']) == 0) {
        //new right for certificate
        //give full rights to profiles having config right
        $prights = $DB->request(['FROM' => 'ntas_profilerights', 'WHERE' => ['name' => 'config']]);
        foreach ($prights as $profrights) {
            if ($profrights['rights'] === (READ | UPDATE)) {
                $rightValue = CREATE | READ | UPDATE | DELETE  | PURGE | READNOTE | UPDATENOTE | UNLOCK;
            } else {
                $rightValue = 0;
            }

            $DB->insert(
                "ntas_profilerights",
                [
                    'id'           => null,
                    'profiles_id'  => $profrights['profiles_id'],
                    'name'         => "certificate",
                    'rights'       => $rightValue,
                ]
            );
        }
    }

    // add alert for certificates
    $migration->addField(
        "ntas_entities",
        'use_certificates_alert',
        "integer",
        ['value' => -2,
            'after' => 'send_licenses_alert_before_delay',
        ]
    );
    $migration->addField(
        "ntas_entities",
        'send_certificates_alert_before_delay',
        "integer",
        ['value'     => -2,
            'after'     => 'use_certificates_alert',
            'update'    => '0', // No delay for root entity
            'condition' => 'WHERE `id` = 0',
        ]
    );
    $migration->addCrontask(
        'Certificate',
        'certificate',
        DAY_TIMESTAMP,
        options: [
            'mode' => 1, // CronTask::MODE_INTERNAL
        ]
    );
    if (!countElementsInTable('ntas_notifications', ['itemtype' => 'Certificate'])) {
        $DB->insert(
            "ntas_notifications",
            [
                'id'              => null,
                'name'            => "Certificates",
                'entities_id'     => "0",
                'itemtype'        => "Certificate",
                'event'           => "alert",
                'comment'         => "",
                'is_recursive'    => "1",
                'is_active'       => "1",
                'date_creation'   => new QueryExpression("NOW()"),
                'date_mod'        => new QueryExpression("NOW()"),
            ]
        );
        $notid = $DB->insertId();

        $DB->insert(
            "ntas_notificationtemplates",
            [
                'name'      => "Certificates",
                'itemtype'  => "Certificate",
                'date_mod'  => new QueryExpression("NOW()"),
            ]
        );
        $nottid = $DB->insertId();

        $where = [
            'notifications_id'         => $notid,
            'mode'                     => Notification_NotificationTemplate::MODE_MAIL,
            'notificationtemplates_id' => $nottid,
        ];
        if (!countElementsInTable('ntas_notifications_notificationtemplates', $where)) {
            $DB->updateOrInsert("ntas_notifications_notificationtemplates", [
                'id'                       => null,
            ], [
                'notifications_id'         => $notid,
                'mode'                     => Notification_NotificationTemplate::MODE_MAIL,
                'notificationtemplates_id' => $nottid,
            ]);
        }

        $query = "INSERT INTO `ntas_notificationtemplatetranslations`
                  (`notificationtemplates_id`, `language`, `subject`, `content_text`, `content_html`)
                VALUES ($notid, '', '##certificate.action##  ##certificate.entity##',
                        '##lang.certificate.entity## : ##certificate.entity##

##FOREACHcertificates##

##lang.certificate.serial## : ##certificate.serial##

##lang.certificate.expirationdate## : ##certificate.expirationdate##

##certificate.url##
 ##ENDFOREACHcertificates##','&lt;p&gt;
##lang.certificate.entity## : ##certificate.entity##&lt;br /&gt;
##FOREACHcertificates##
&lt;br /&gt;##lang.certificate.name## : ##certificate.name##&lt;br /&gt;
##lang.certificate.serial## : ##certificate.serial##&lt;br /&gt;
##lang.certificate.expirationdate## : ##certificate.expirationdate##
&lt;br /&gt; &lt;a href=\"##certificate.url##\"&gt; ##certificate.url##
&lt;/a&gt;&lt;br /&gt; ##ENDFOREACHcertificates##&lt;/p&gt;')";

        $DB->doQuery($query);

        $DB->insert(
            "ntas_notificationtargets",
            [
                'id'                 => null,
                'notifications_id'   => $notid,
                'type'               => Notification::USER_TYPE,
                'items_id'           => Notification::ITEM_TECH_IN_CHARGE,
            ]
        );
        $DB->insert(
            "ntas_notificationtargets",
            [
                'id'                 => null,
                'notifications_id'   => $notid,
                'type'               => Notification::USER_TYPE,
                'items_id'           => Notification::ITEM_TECH_GROUP_IN_CHARGE,
            ]
        );
    }

    /************** Simcard component **************/
    $migration->addField("ntas_states", "is_visible_line", "bool", ["after" => "is_visible_softwarelicense"]);

    if (!$DB->tableExists('ntas_lineoperators')) {
        $query = "CREATE TABLE IF NOT EXISTS `ntas_lineoperators` (
                   `id` int NOT NULL AUTO_INCREMENT,
                   `name` varchar(255) NOT NULL DEFAULT '',
                   `comment` text COLLATE utf8_unicode_ci,
                   `mcc` int DEFAULT NULL,
                   `mnc` int DEFAULT NULL,
                   `entities_id`      INT NOT NULL DEFAULT 0,
                   `is_recursive`     TINYINT NOT NULL DEFAULT 0,
                   `date_mod` datetime DEFAULT NULL,
                   `date_creation` datetime DEFAULT NULL,
                   PRIMARY KEY (`id`),
                   KEY `name` (`name`),
                   KEY `entities_id`  (`entities_id`),
                   KEY `is_recursive` (`is_recursive`),
                   KEY `date_mod` (`date_mod`),
                   KEY `date_creation` (`date_creation`),
                   UNIQUE KEY `unicity` (`mcc`,`mnc`)
                ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_linetypes')) {
        $query = "CREATE TABLE IF NOT EXISTS `ntas_linetypes` (
         `id` int NOT NULL AUTO_INCREMENT,
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
         `comment` text COLLATE utf8_unicode_ci,
         `date_mod` datetime DEFAULT NULL,
         `date_creation` datetime DEFAULT NULL,
         PRIMARY KEY (`id`),
         KEY `name` (`name`),
         KEY `date_mod` (`date_mod`),
         KEY `date_creation` (`date_creation`)
         ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_lines')) {
        $query = "CREATE TABLE `ntas_lines` (
            `id`                   INT NOT NULL auto_increment,
            `name`                 VARCHAR(255) NOT NULL DEFAULT '',
            `entities_id`          INT NOT NULL DEFAULT 0,
            `is_recursive`         TINYINT NOT NULL DEFAULT 0,
            `is_deleted`           TINYINT NOT NULL DEFAULT 0,
            `caller_num`           VARCHAR(255) NOT NULL DEFAULT '',
            `caller_name`          VARCHAR(255) NOT NULL DEFAULT '',
            `users_id`             INT NOT NULL DEFAULT 0,
            `groups_id`            INT NOT NULL DEFAULT 0,
            `lineoperators_id`     INT NOT NULL DEFAULT 0,
            `locations_id`         INT NOT NULL DEFAULT '0',
            `states_id`            INT NOT NULL DEFAULT '0',
            `linetypes_id`         INT NOT NULL DEFAULT '0',
            `date_creation`        DATETIME DEFAULT NULL,
            `date_mod`             DATETIME DEFAULT NULL,
            `comment`              TEXT DEFAULT NULL,
            PRIMARY KEY            (`id`),
            KEY `entities_id`      (`entities_id`),
            KEY `is_recursive`     (`is_recursive`),
            KEY `users_id`         (`users_id`),
            KEY `lineoperators_id` (`lineoperators_id`)
            ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_devicesimcardtypes')) {
        $query = "CREATE TABLE IF NOT EXISTS `ntas_devicesimcardtypes` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `name` varchar(255) NOT NULL DEFAULT '',
                  `comment` text COLLATE utf8_unicode_ci,
                  `date_mod` datetime DEFAULT NULL,
                  `date_creation` datetime DEFAULT NULL,
                  PRIMARY KEY (`id`),
                  KEY `name` (`name`),
                  KEY `date_mod` (`date_mod`),
                  KEY `date_creation` (`date_creation`)
                ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    if (!countElementsInTable('ntas_devicesimcardtypes', ['name' => 'Full SIM'])) {
        $DB->insert("ntas_devicesimcardtypes", [
            'id'              => null,
            'name'            => "Full SIM",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);
    }
    if (!countElementsInTable('ntas_devicesimcardtypes', ['name' => 'Mini SIM'])) {
        $DB->insert("ntas_devicesimcardtypes", [
            'id'              => null,
            'name'            => "Mini SIM",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);
    }
    if (!countElementsInTable('ntas_devicesimcardtypes', ['name' => 'Micro SIM'])) {
        $DB->insert("ntas_devicesimcardtypes", [
            'id'              => null,
            'name'            => "Micro SIM",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);
    }
    if (!countElementsInTable('ntas_devicesimcardtypes', ['name' => 'Nano SIM'])) {
        $DB->insert("ntas_devicesimcardtypes", [
            'id'              => null,
            'name'            => "Nano SIM",
            'comment'         => null,
            'date_mod'        => null,
            'date_creation'   => null,
        ]);
    }

    if (!$DB->tableExists('ntas_devicesimcards')) {
        $query = "CREATE TABLE IF NOT EXISTS `ntas_devicesimcards` (
               `id` int NOT NULL AUTO_INCREMENT,
               `designation` varchar(255) DEFAULT NULL,
               `comment` text CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL,
               `entities_id` int NOT NULL DEFAULT '0',
               `is_recursive` tinyint NOT NULL DEFAULT '0',
               `manufacturers_id` int NOT NULL DEFAULT '0',
               `voltage` int DEFAULT NULL,
               `devicesimcardtypes_id` int NOT NULL DEFAULT '0',
               `date_mod` datetime DEFAULT NULL,
               `date_creation` datetime DEFAULT NULL,
               `allow_voip` tinyint NOT NULL DEFAULT '0',
               PRIMARY KEY (`id`),
               KEY `designation` (`designation`),
               KEY `entities_id` (`entities_id`),
               KEY `is_recursive` (`is_recursive`),
               KEY `devicesimcardtypes_id` (`devicesimcardtypes_id`),
               KEY `date_mod` (`date_mod`),
               KEY `date_creation` (`date_creation`),
               KEY `manufacturers_id` (`manufacturers_id`)
            ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (!$DB->tableExists('ntas_items_devicesimcards')) {
        $query = "CREATE TABLE IF NOT EXISTS `ntas_items_devicesimcards` (
                  `id` int NOT NULL AUTO_INCREMENT,
                  `items_id` int NOT NULL DEFAULT '0' COMMENT 'RELATION to various table, according to itemtype (id)',
                  `itemtype` varchar(100) COLLATE utf8_unicode_ci NOT NULL,
                  `devicesimcards_id` int NOT NULL DEFAULT '0',
                  `is_deleted` tinyint NOT NULL DEFAULT '0',
                  `is_dynamic` tinyint NOT NULL DEFAULT '0',
                  `entities_id` int NOT NULL DEFAULT '0',
                  `serial` varchar(255) NULL DEFAULT NULL,
                  `otherserial` varchar(255) NULL DEFAULT NULL,
                  `states_id` int NOT NULL DEFAULT '0',
                  `locations_id` int NOT NULL DEFAULT '0',
                  `lines_id` int NOT NULL DEFAULT '0',
                  `pin` varchar(255) NOT NULL DEFAULT '',
                  `pin2` varchar(255) NOT NULL DEFAULT '',
                  `puk` varchar(255) NOT NULL DEFAULT '',
                  `puk2` varchar(255) NOT NULL DEFAULT '',
                  PRIMARY KEY (`id`),
                  KEY `item` (`itemtype`,`items_id`),
                  KEY `devicesimcards_id` (`devicesimcards_id`),
                  KEY `is_deleted` (`is_deleted`),
                  KEY `is_dynamic` (`is_dynamic`),
                  KEY `entities_id` (`entities_id`),
                  KEY `serial` (`serial`),
                  KEY `otherserial` (`otherserial`),
                  KEY `states_id` (`states_id`),
                  KEY `locations_id` (`locations_id`),
                  KEY `lines_id` (`lines_id`)
                ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci";
        $DB->doQuery($query);
    }

    if (countElementsInTable("ntas_profilerights", ['name' => 'line']) == 0) {
        //new right for line
        //give full rights to profiles having config right
        $prights = $DB->request(['FROM' => 'ntas_profilerights', 'WHERE' => ['name' => 'config']]);
        foreach ($prights as $profrights) {
            if ($profrights['rights'] === (READ | UPDATE)) {
                $rightValue = CREATE | READ | UPDATE | DELETE | PURGE | READNOTE | UPDATENOTE;
            } else {
                $rightValue = 0;
            }
            $DB->insert(
                "ntas_profilerights",
                [
                    'id'           => null,
                    'profiles_id'  => $profrights['profiles_id'],
                    'name'         => "line",
                    'rights'       => $rightValue,
                ]
            );
        }
    }

    if (countElementsInTable("ntas_profilerights", ['name' => 'lineoperator']) == 0) {
        //new right for lineoperator
        //give full rights to profiles having config right
        $prights = $DB->request(['FROM' => 'ntas_profilerights', 'WHERE' => ['name' => 'config']]);
        foreach ($prights as $profrights) {
            if ($profrights['rights'] === (READ | UPDATE)) {
                $rightValue = CREATE | READ | UPDATE | DELETE | PURGE;
            } else {
                $rightValue = 0;
            }
            $DB->insert(
                "ntas_profilerights",
                [
                    'id'           => null,
                    'profiles_id'  => $profrights['profiles_id'],
                    'name'         => "lineoperator",
                    'rights'       => $rightValue,
                ]
            );
        }
    }

    if (countElementsInTable("ntas_profilerights", ['name' => 'devicesimcard_pinpuk']) == 0) {
        //new right for simcard pin and puk
        //give full rights to profiles having config right
        $prights = $DB->request(['FROM' => 'ntas_profilerights', 'WHERE' => ['name' => 'config']]);
        foreach ($prights as $profrights) {
            if ($profrights['rights'] === (READ | UPDATE)) {
                $rightValue = READ | UPDATE;
            } else {
                $rightValue = 0;
            }
            $DB->insert(
                "ntas_profilerights",
                [
                    'id'           => null,
                    'profiles_id'  => $profrights['profiles_id'],
                    'name'         => "devicesimcard_pinpuk",
                    'rights'       => $rightValue,
                ]
            );
        }
    }

    //Firmware for phones
    if ($DB->fieldExists('ntas_phones', 'firmware')) {
        $iterator = $DB->request([
            'SELECT' => ['id', 'firmware'],
            'FROM'   => 'ntas_phones',
            'NOT'    => ['firmware' => null],
        ]);

        $firmwares = [];
        foreach ($iterator as $row) {
            if (!isset($firmwares[$row['firmware']])) {
                $firmware_result = $DB->request([
                    'FROM' => 'ntas_devicefirmwares',
                    'WHERE' => ['designation' => $row['firmware']],
                ]);
                if ($firmware_result->count() === 1) {
                    $firmwares[$row['firmware']] = $firmware_result->current()['id'];
                } else {
                    $DB->insert(
                        'ntas_devicefirmwares',
                        [
                            'designation'              => $row['firmware'],
                            'devicefirmwaretypes_id'   => '3', //type "firmware"
                        ]
                    );
                    $firmwares[$row['firmware']] = $DB->insertId();
                }
            }

            //add link
            $DB->insert(
                'ntas_items_devicefirmwares',
                [
                    'itemtype'           => 'Phone',
                    'items_id'           => $row['id'],
                    'devicefirmwares_id' => $firmwares[$row['firmware']],
                ]
            );
        }

        $migration->dropField('ntas_phones', 'firmware');
    }

    //Firmware for network equipements
    if ($DB->tableExists('ntas_networkequipmentfirmwares')) {
        $mapping = [];
        $iterator = $DB->request(['FROM' => 'ntas_networkequipmentfirmwares']);
        foreach ($iterator as $row) {
            $DB->insert(
                'ntas_devicefirmwares',
                [
                    'designation'              => $row['name'],
                    'comment'                  => $row['comment'],
                    'devicefirmwaretypes_id'   => 3, //type "Firmware"
                    'date_creation'            => $row['date_creation'],
                    'date_mod'                 => $row['date_mod'],
                ]
            );
            $mapping[$row['id']] = $DB->insertId();
        }

        $iterator = $DB->request(['FROM' => 'ntas_networkequipments']);
        foreach ($iterator as $row) {
            if (isset($mapping[$row['networkequipmentfirmwares_id']])) {
                $DB->insert(
                    'ntas_items_devicefirmwares',
                    [
                        'itemtype'           => 'NetworkEquipment',
                        'items_id'           => $row['id'],
                        'devicefirmwares_id' => $mapping[$row['networkequipmentfirmwares_id']],
                    ]
                );
            }
        }

        $migration->dropKey('ntas_networkequipments', 'networkequipmentfirmwares_id');
        $migration->dropField('ntas_networkequipments', 'networkequipmentfirmwares_id');
        $migration->dropTable('ntas_networkequipmentfirmwares');
    }

    // add projecttemplate
    if (!$DB->fieldExists('ntas_projects', 'projecttemplates_id')) {
        $migration->addField("ntas_projects", "projecttemplates_id", "integer");
        $migration->addField("ntas_projects", "is_template", "bool");
        $migration->addField("ntas_projects", "template_name", "string");
        $migration->addKey("ntas_projects", "projecttemplates_id");
    }

    if (!$DB->fieldExists('ntas_projecttasks', 'projecttasktemplates_id')) {
        $migration->addField("ntas_projecttasks", "projecttasktemplates_id", "integer");
        $migration->addField("ntas_projecttasks", "is_template", "bool");
        $migration->addField("ntas_projecttasks", "template_name", "string");
        $migration->addKey("ntas_projecttasks", "projecttasktemplates_id");
    }

    if (!$DB->tableExists('ntas_projecttasktemplates')) {
        $query = "CREATE TABLE `ntas_projecttasktemplates` (
                       `id` int NOT NULL AUTO_INCREMENT,
                       `entities_id` int NOT NULL DEFAULT '0',
                       `is_recursive` tinyint NOT NULL DEFAULT '0',
                       `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
                       `description` longtext COLLATE utf8_unicode_ci,
                       `comment` longtext COLLATE utf8_unicode_ci,
                       `projects_id` int NOT NULL DEFAULT '0',
                       `projecttasks_id` int NOT NULL DEFAULT '0',
                       `plan_start_date` datetime DEFAULT NULL,
                       `plan_end_date` datetime DEFAULT NULL,
                       `real_start_date` datetime DEFAULT NULL,
                       `real_end_date` datetime DEFAULT NULL,
                       `planned_duration` int NOT NULL DEFAULT '0',
                       `effective_duration` int NOT NULL DEFAULT '0',
                       `projectstates_id` int NOT NULL DEFAULT '0',
                       `projecttasktypes_id` int NOT NULL DEFAULT '0',
                       `users_id` int NOT NULL DEFAULT '0',
                       `percent_done` int NOT NULL DEFAULT '0',
                       `is_milestone` tinyint NOT NULL DEFAULT '0',
                       `comments` text COLLATE utf8_unicode_ci,
                       `date_mod` datetime DEFAULT NULL,
                       `date_creation` datetime DEFAULT NULL,
                       PRIMARY KEY (`id`),
                       KEY `name` (`name`),
                       KEY `entities_id` (`entities_id`),
                       KEY `is_recursive` (`is_recursive`),
                       KEY `projects_id` (`projects_id`),
                       KEY `projecttasks_id` (`projecttasks_id`),
                       KEY `date_creation` (`date_creation`),
                       KEY `date_mod` (`date_mod`),
                       KEY `users_id` (`users_id`),
                       KEY `plan_start_date` (`plan_start_date`),
                       KEY `plan_end_date` (`plan_end_date`),
                       KEY `real_start_date` (`real_start_date`),
                       KEY `real_end_date` (`real_end_date`),
                       KEY `percent_done` (`percent_done`),
                       KEY `projectstates_id` (`projectstates_id`),
                       KEY `projecttasktypes_id` (`projecttasktypes_id`),
                       KEY `is_milestone` (`is_milestone`)
                     ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
        $DB->doQuery($query);
    }

    //add editor in followupps
    if (!$DB->fieldExists('ntas_ticketfollowups', 'users_id_editor')) {
        $migration->addField("ntas_ticketfollowups", "users_id_editor", "int NOT NULL DEFAULT '0'", ['after' => 'users_id']);
        $migration->addKey("ntas_ticketfollowups", "users_id_editor");
    }

    //add editor in *tasks
    if (!$DB->fieldExists('ntas_tickettasks', 'users_id_editor')) {
        $migration->addField("ntas_tickettasks", "users_id_editor", "int NOT NULL DEFAULT '0'", ['after' => 'users_id']);
        $migration->addKey("ntas_tickettasks", "users_id_editor");
    }
    if (!$DB->fieldExists('ntas_changetasks', 'users_id_editor')) {
        $migration->addField("ntas_changetasks", "users_id_editor", "int NOT NULL DEFAULT '0'", ['after' => 'users_id']);
        $migration->addKey("ntas_changetasks", "users_id_editor");
    }
    if (!$DB->fieldExists('ntas_problemtasks', 'users_id_editor')) {
        $migration->addField("ntas_problemtasks", "users_id_editor", "int NOT NULL DEFAULT '0'", ['after' => 'users_id']);
        $migration->addKey("ntas_problemtasks", "users_id_editor");
    }

    //Add a sync_field in LDAP configuration
    if (!$DB->fieldExists('ntas_authldaps', 'sync_field')) {
        $migration->addField("ntas_authldaps", "sync_field", "string", ['after' => 'login_field', 'null' => true]);
        $migration->addKey('ntas_authldaps', 'sync_field');
    }
    //Add a sync_field for users
    if (!$DB->fieldExists('ntas_users', 'sync_field')) {
        $migration->addField("ntas_users", "sync_field", "string", ['null' => true]);
        $migration->addKey('ntas_users', 'sync_field');
    }

    $migration->addConfig([
        'smtp_max_retries'   => 5,
        'smtp_sender'        => 'NULL',
        'from_email'         => 'NULL',
        'from_email_name'    => 'NULL',
    ]);

    //register telemetry crontask
    $migration->addCrontask(
        'Telemetry',
        'telemetry',
        MONTH_TIMESTAMP,
        options: [
            'state' => 0, // CronTask::STATE_DISABLE
            'mode'  => 1, // CronTask::MODE_INTERNAL
        ]
    );
    $migration->addConfig([
        'instance_uuid'      => Toolbox::getRandomString(40),
        'registration_uuid'  => Toolbox::getRandomString(40),
    ]);

    if (isIndex('ntas_authldaps', 'use_tls')) {
        $query = "ALTER TABLE `ntas_authldaps` DROP INDEX `use_tls`";
        $DB->doQuery($query);
    }

    //Fix some field order from old migrations
    $migration->migrationOneTable('ntas_states');
    $DB->doQuery("ALTER TABLE `ntas_budgets` CHANGE `date_creation` `date_creation` DATETIME NULL DEFAULT NULL AFTER `date_mod`");
    $DB->doQuery("ALTER TABLE `ntas_changetasks` CHANGE `groups_id_tech` `groups_id_tech` INT NOT NULL DEFAULT '0' AFTER `users_id_tech`");
    $DB->doQuery("ALTER TABLE `ntas_problemtasks` CHANGE `groups_id_tech` `groups_id_tech` INT NOT NULL DEFAULT '0' AFTER `users_id_tech`");
    $DB->doQuery("ALTER TABLE `ntas_tickettasks` CHANGE `groups_id_tech` `groups_id_tech` INT NOT NULL DEFAULT '0' AFTER `users_id_tech`");
    $DB->doQuery("ALTER TABLE `ntas_knowbaseitemcategories` CHANGE `sons_cache` `sons_cache` LONGTEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER `level`");
    $DB->doQuery("ALTER TABLE `ntas_requesttypes` CHANGE `is_followup_default` `is_followup_default` TINYINT NOT NULL DEFAULT '0' AFTER `is_helpdesk_default`");
    $DB->doQuery("ALTER TABLE `ntas_requesttypes` CHANGE `is_mailfollowup_default` `is_mailfollowup_default` TINYINT NOT NULL DEFAULT '0' AFTER `is_mail_default`");
    $DB->doQuery("ALTER TABLE `ntas_requesttypes` CHANGE `comment` `comment` TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER `is_ticketfollowup`");
    $DB->doQuery("ALTER TABLE `ntas_requesttypes` CHANGE `date_mod` `date_mod` DATETIME NULL DEFAULT NULL AFTER `comment`");
    $DB->doQuery("ALTER TABLE `ntas_requesttypes` CHANGE `date_creation` `date_creation` DATETIME NULL DEFAULT NULL AFTER `date_mod`");
    $DB->doQuery("ALTER TABLE `ntas_groups` CHANGE `is_task` `is_task` TINYINT NOT NULL DEFAULT '1' AFTER `is_assign`");
    $DB->doQuery("ALTER TABLE `ntas_states` CHANGE `date_mod` `date_mod` DATETIME NULL DEFAULT NULL AFTER `is_visible_certificate`");
    $DB->doQuery("ALTER TABLE `ntas_states` CHANGE `date_creation` `date_creation` DATETIME NULL DEFAULT NULL AFTER `date_mod`");
    $DB->doQuery("ALTER TABLE `ntas_taskcategories` CHANGE `is_active` `is_active` TINYINT NOT NULL DEFAULT '1' AFTER `sons_cache`");
    $DB->doQuery("ALTER TABLE `ntas_users` CHANGE `palette` `palette` CHAR(20) CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER `layout`");
    $DB->doQuery("ALTER TABLE `ntas_users` CHANGE `set_default_requester` `set_default_requester` TINYINT NULL DEFAULT NULL AFTER `ticket_timeline_keep_replaced_tabs`");
    $DB->doQuery("ALTER TABLE `ntas_users` CHANGE `plannings` `plannings` TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT NULL AFTER `highcontrast_css`");

    //Fix bad default values
    $DB->doQuery("ALTER TABLE `ntas_states` CHANGE `is_visible_softwarelicense` `is_visible_softwarelicense` TINYINT NOT NULL DEFAULT '1'");
    $DB->doQuery("ALTER TABLE `ntas_states` CHANGE `is_visible_line` `is_visible_line` TINYINT NOT NULL DEFAULT '1'");

    //Fields added in 0905_91 script but not in empty sql...
    if (!$DB->fieldExists('ntas_changetasks', 'date_creation', false)) {
        $migration->addField('ntas_changetasks', 'date_creation', 'datetime', ['after' => 'date_mod']);
        $migration->addKey('ntas_changetasks', 'date_creation');
    }
    if (!$DB->fieldExists('ntas_networkportfiberchannels', 'date_mod', false)) {
        $migration->addField('ntas_networkportfiberchannels', 'date_mod', 'datetime', ['after' => 'speed']);
        $migration->addKey('ntas_networkportfiberchannels', 'date_mod');
    }
    if (!$DB->fieldExists('ntas_networkportfiberchannels', 'date_creation', false)) {
        $migration->addField('ntas_networkportfiberchannels', 'date_creation', 'datetime', ['after' => 'date_mod']);
        $migration->addKey('ntas_networkportfiberchannels', 'date_creation');
    }
    if (!$DB->fieldExists('ntas_problemtasks', 'date_creation', false)) {
        $migration->addField('ntas_problemtasks', 'date_creation', 'datetime', ['after' => 'date_mod']);
        $migration->addKey('ntas_problemtasks', 'date_creation');
    }
    if (!$DB->fieldExists('ntas_slms', 'date_creation', false)) {
        $migration->addField('ntas_slms', 'date_creation', 'datetime', ['after' => 'date_mod']);
        $migration->addKey('ntas_slms', 'date_creation');
    }
    if (!$DB->fieldExists('ntas_ticketfollowups', 'date_creation', false)) {
        $migration->addField('ntas_ticketfollowups', 'date_creation', 'datetime', ['after' => 'date_mod']);
        $migration->addKey('ntas_ticketfollowups', 'date_creation');
    }
    if (!$DB->fieldExists('ntas_tickettasks', 'date_creation', false)) {
        $migration->addField('ntas_tickettasks', 'date_creation', 'datetime', ['after' => 'date_mod']);
        $migration->addKey('ntas_tickettasks', 'date_creation');
    }
    if (!$DB->fieldExists('ntas_softwarelicenses', 'contact_num', false)) {
        $migration->addField(
            "ntas_softwarelicenses",
            "contact_num",
            "varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL",
            ['after' => 'contact']
        );
    }

    //Fix comments...
    $DB->doQuery("ALTER TABLE `ntas_savedsearches` CHANGE `type` `type` INT NOT NULL DEFAULT '0' COMMENT 'see SavedSearch:: constants'");

    //Fix unicity...
    $tables = [
        'ntas_slalevels_tickets'   => ['tickets_id', 'slalevels_id'],
        'ntas_businesscriticities' => ['businesscriticities_id', 'name'],
        'ntas_documentcategories'  => ['documentcategories_id', 'name'],
        'ntas_olalevels_tickets'   => ['tickets_id', 'olalevels_id'],
        'ntas_states'              => ['states_id', 'name'],
        'ntas_tickets_tickets'     => ['tickets_id_1', 'tickets_id_2'],
        'ntas_tickettemplatehiddenfields'      => ['tickettemplates_id', 'num'],
        'ntas_tickettemplatemandatoryfields'   => ['tickettemplates_id', 'num'],
    ];
    foreach ($tables as $table => $fields) {
        $add = true;
        $result = $DB->doQuery("SHOW INDEX FROM `$table` WHERE Key_name='unicity'");
        if ($result && $DB->numrows($result)) {
            $row = $DB->fetchAssoc($result);
            if ($row['Non_unique'] == 1) {
                $migration->dropKey($table, 'unicity');
                $migration->migrationOneTable($table);
            } else {
                $add = false;
            }
        }

        if ($add) {
            //missing or bad unique key ==> add it.
            $migration->addKey(
                $table,
                $fields,
                'unicity',
                'UNIQUE'
            );
        }
    }

    // unique index added in a previous 9.2-dev
    $migration->dropKey('ntas_tickettemplatepredefinedfields', 'unicity');
    $migration->addKey(
        'ntas_tickettemplatepredefinedfields',
        ['tickettemplates_id', 'num'],
        'tickettemplates_id_id_num'
    );

    //removed field
    if ($DB->fieldExists('ntas_slms', 'resolution_time')) {
        $migration->dropField('ntas_slms', 'resolution_time');
    }

    //wrong type
    $DB->doQuery("ALTER TABLE `ntas_users` CHANGE `keep_devices_when_purging_item` `keep_devices_when_purging_item` TINYINT NULL DEFAULT NULL");

    //missing index
    $migration->addKey('ntas_networknames', 'is_deleted');
    $migration->addKey('ntas_networknames', 'is_dynamic');
    $migration->addKey('ntas_projects', 'is_template');
    $migration->addKey('ntas_projecttasks', 'is_template');
    $migration->dropKey('ntas_savedsearches_users', 'bookmarks_id');
    $migration->addKey('ntas_savedsearches_users', 'savedsearches_id');
    //this one was not on the correct field
    $migration->dropKey('ntas_softwarelicenses', 'is_deleted');
    $migration->migrationOneTable('ntas_softwarelicenses');
    $migration->addKey('ntas_softwarelicenses', 'is_deleted');
    $migration->addKey('ntas_states', 'is_visible_line');
    $migration->addKey('ntas_tasktemplates', 'is_private');
    $migration->addKey('ntas_tasktemplates', 'users_id_tech');
    $migration->addKey('ntas_tasktemplates', 'groups_id_tech');

    //add timeline_position in ticketfollowups
    //add timeline_position in tickettasks
    //add timeline_position in documents_items
    //add timeline_position in ticketvalidations
    $timeline_tables = ['ntas_ticketfollowups', 'ntas_tickettasks', 'ntas_documents_items', 'ntas_ticketvalidations'];
    foreach ($timeline_tables as $tl_table) {
        //add timeline_position in $tl_table
        if (!$DB->fieldExists($tl_table, 'timeline_position')) {
            $migration->addField($tl_table, "timeline_position", "tinyint NOT NULL DEFAULT '0'");
            $where = [
                "$tl_table.tickets_id"  => new QueryExpression(
                    DBmysql::quoteName("ntas_tickets_users.tickets_id")
                ),
                "$tl_table.users_id"    => new QueryExpression(
                    DBmysql::quoteName("ntas_tickets_users.users_id")
                ),
            ];
            if (!$DB->fieldExists($tl_table, 'tickets_id')) {
                $where = [
                    "$tl_table.itemtype"    => "Ticket",
                    "$tl_table.items_id"    => new QueryExpression(
                        DBmysql::quoteName("ntas_tickets_users.tickets_id")
                    ),
                    "$tl_table.users_id"    => new QueryExpression(
                        DBmysql::quoteName("ntas_tickets_users.users_id")
                    ),
                ];
            }

            $update = new QueryExpression(
                DBmysql::quoteName($tl_table) . ", " . DBmysql::quoteName("ntas_tickets_users")
            );
            $migration->addPostQuery(
                $DB->buildUpdate(
                    $update,
                    [
                        "$tl_table.timeline_position" => new QueryExpression("IF("
                     . DBmysql::quoteName("ntas_tickets_users.type") . " NOT IN (1,3) AND "
                     . DBmysql::quoteName("ntas_tickets_users.type") . " IN (2), 4, 1)"),
                    ],
                    $where
                )
            );

            $where = [
                "$tl_table.tickets_id"           => new QueryExpression(
                    DBmysql::quoteName("ntas_groups_tickets.tickets_id")
                ),
                "ntas_groups_users.groups_id"    => new QueryExpression(
                    DBmysql::quoteName("ntas_groups_tickets.groups_id")
                ),
                "$tl_table.users_id"             => new QueryExpression(
                    DBmysql::quoteName("ntas_groups_users.users_id")
                ),
            ];
            if (!$DB->fieldExists($tl_table, 'tickets_id')) {
                $where = [
                    "$tl_table.itemtype"             => "Ticket",
                    "$tl_table.items_id"             => new QueryExpression(
                        DBmysql::quoteName("ntas_groups_tickets.tickets_id")
                    ),
                    "ntas_groups_users.groups_id"    => new QueryExpression(
                        DBmysql::quoteName("ntas_groups_tickets.groups_id")
                    ),
                    "$tl_table.users_id"             => new QueryExpression(
                        DBmysql::quoteName("ntas_groups_users.users_id")
                    ),
                ];
            }

            $update = new QueryExpression(
                DBmysql::quoteName($tl_table) . ", " . DBmysql::quoteName("ntas_groups_tickets")
                . ", " . DBmysql::quoteName("ntas_groups_users")
            );
            $migration->addPostQuery(
                $DB->buildUpdate(
                    $update,
                    [
                        "$tl_table.timeline_position" => new QueryExpression("IF("
                     . DBmysql::quoteName("ntas_groups_tickets.type") . " NOT IN (1,3) AND "
                     . DBmysql::quoteName("ntas_groups_tickets.type") . " IN (2), 4, 1)"),
                    ],
                    $where
                )
            );

            $migration->addPostQuery(
                $DB->buildUpdate(
                    $tl_table,
                    ["$tl_table.timeline_position" => 1],
                    ["$tl_table.timeline_position" => 0]
                )
            );
        }
    }

    // ************ Keep it at the end **************
    $migration->executeMigration();

    return $updateresult;
}
