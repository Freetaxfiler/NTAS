...
if (!$DB->tableExists('ntas_forms_categories')) {
    $DB->doQuery(
...
// Create tables
if (!$DB->tableExists('ntas_forms_forms')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_sections')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_questions')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_comments')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_answerssets')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_destinations_answerssets_formdestinationitems')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_destinations_formdestinations')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_forms_accesscontrols_formaccesscontrols')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_helpdesks_tiles_items_tiles')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_helpdesks_tiles_formtiles')) {
    $DB->doQuery(
...
if (!$DB->tableExists('ntas_helpdesks_tiles_glpipagetiles')) {
    $DB->doQuery(
    ...
    $migration->changeField(
        'ntas_helpdesks_tiles_glpipagetiles',
        'description',
        'description',
        'text',
    );
}
if (!$DB->tableExists('ntas_helpdesks_tiles_externalpagetiles')) {
    $DB->doQuery(
    ...
    // Fix a bad type, description was created as varchar(255) when it should have been a text field.
    $migration->changeField(
        'ntas_helpdesks_tiles_externalpagetiles',
        'description',
        'description',
        'text',
    );
...
if (!$DB->fieldExists("ntas_entities", $field)) {
    $migration->addField(
        'ntas_entities',
        $field,
...
if (!$DB->fieldExists('ntas_forms_forms', 'uuid')) {
    $migration->addField(
        'ntas_forms_forms',
        'uuid',
...
if (!$DB->fieldExists('ntas_forms_forms', 'submit_button_visibility_strategy')) {
    $migration->addField(
        'ntas_forms_forms',
...
if (!$DB->fieldExists('ntas_forms_forms', 'submit_button_conditions')) {
    $migration->addField(
        'ntas_forms_forms',
...
if (!$DB->fieldExists('ntas_forms_questions', 'validation_strategy')) {
    $migration->addField(
        'ntas_forms_questions',
        'validation_strategy',
        "varchar(30) NOT NULL DEFAULT ''",
        [
            'after' => 'conditions',
...
if (!$DB->fieldExists('ntas_forms_questions', 'validation_conditions')) {
    $migration->addField(
        'ntas_forms_questions',
...
if (!$DB->fieldExists('ntas_forms_forms', 'render_layout')) {
    $migration->addField(
        'ntas_forms_forms',
        'render_layout',
        "varchar(30) NOT NULL DEFAULT ''",
...
// Change the type of the 'answers' field in ntas_forms_answerssets
$migration->changeField(
...
// New config values
$migration->addConfig(['ntas_11_form_migration' => 0]);
...
if ($DB->tableExists('ntas_forms_destinations_formdestinations')) {
    $iterator = $DB->request(['FROM' => 'ntas_forms_destinations_formdestinations']);
    foreach ($iterator as $data) {
        ...
        $DB->update(
            'ntas_forms_destinations_formdestinations',
