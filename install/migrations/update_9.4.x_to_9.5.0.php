...
function update94xto950()
{
    ...
    if (!$DB->fieldExists("ntas_items_disks", "encryption_status")) {
        $migration->addField("ntas_items_disks", "encryption_status", "integer", [
            'after'  => "is_dynamic",
            'value'  => 0,
        ]);
    }

    if (!$DB->fieldExists("ntas_items_disks", "encryption_tool")) {
        $migration->addField("ntas_items_disks", "encryption_tool", "string", [
            'after'  => "encryption_status",
        ]);
    }

    if (!$DB->fieldExists("ntas_items_disks", "encryption_algorithm")) {
        $migration->addField("ntas_items_disks", "encryption_algorithm", "string", [
            'after'  => "encryption_tool",
        ]);
    }

    if (!$DB->fieldExists("ntas_items_disks", "encryption_type")) {
        $migration->addField("ntas_items_disks", "encryption_type", "string", [
    ...
    if (!$DB->fieldExists('ntas_suppliers', 'is_active')) {
        $migration->addField(
            'ntas_suppliers',
            'is_active',
            'bool',
        ...
        $migration->addKey('ntas_suppliers', 'is_active');
    ...
    /** Timezones */
    //User timezone
    if (!$DB->fieldExists('ntas_users', 'timezone')) {
        $migration->addField("ntas_users", "timezone", "varchar(50) DEFAULT NULL");
    }
    ...
    $migration->dropKey('ntas_softwarelicenses', 'softwares_id_expire');
    $migration->addKey('ntas_softwarelicenses', [
        'softwares_id',
        'expire',
        'number',
    ], 'softwares_id_expire_number');

    /** Private supplier followup in ntas_entities */
    if (!$DB->fieldExists('ntas_entities', 'suppliers_as_private')) {
        $migration->addField(
            "ntas_entities",
    ...
    if (!$DB->fieldExists('ntas_entities', 'enable_custom_css')) {
        $migration->addField(
            'ntas_entities',
            'enable_custom_css',
            'integer',
    ...
    if (!$DB->fieldExists('ntas_entities', 'custom_css_code')) {
        $migration->addField('ntas_entities', 'custom_css_code', 'text');
    }
    /** /Entities Custom CSS configuration fields */

    /** Clusters */
    if (!$DB->tableExists('ntas_clustertypes')) {
        $query = "CREATE TABLE `ntas_clustertypes` (
    ...
    if (!$DB->tableExists('ntas_clusters')) {
        $query = "CREATE TABLE `ntas_clusters` (
         `id` int NOT NULL AUTO_INCREMENT,
         `entities_id` int NOT NULL DEFAULT '0',
         `is_recursive` tinyint NOT NULL DEFAULT '0',
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
    ...
    if (!$DB->tableExists('ntas_items_clusters')) {
        $query = "CREATE TABLE `ntas_items_clusters` (
         `id` int NOT NULL AUTO_INCREMENT,
    ...
    $migration->addField('ntas_states', 'is_visible_cluster', 'bool', [
        'value' => 1,
        'after' => 'is_visible_pdu',
    ]);
    $migration->addKey('ntas_states', 'is_visible_cluster');
    ...
    foreach (
        [
            'ntas_itiltemplates',
            'ntas_itiltemplatepredefinedfields',
            'ntas_itiltemplatemandatoryfields',
            'ntas_itiltemplatehiddenfields',
        ] as $table
    ) {
    ...
    foreach (
        [
            'ntas_entities'                        => 'itiltemplates_id',
            'ntas_profiles'                        => 'itiltemplates_id',
            'ntas_ticketrecurrents'                => 'itiltemplates_id',
            'ntas_tickettemplatehiddenfields'      => 'itiltemplates_id',
            'ntas_tickettemplatemandatoryfields'   => 'itiltemplates_id',
            'ntas_tickettemplatepredefinedfields'  => 'itiltemplates_id',
        ] as $table => $field
    ) {
    ...
    $migration->changeField('ntas_itilcategories', 'itiltemplates_id_incident', 'tickettemplates_id_incident', 'integer');
    $migration->changeField('ntas_itilcategories', 'itiltemplates_id_demand', 'tickettemplates_id_demand', 'integer');
    ...
    foreach (['change', 'problem'] as $itiltype) {
        if (!$DB->tableExists("ntas_{$itiltype}templates")) {
            $query = "CREATE TABLE `ntas_{$itiltype}templates` (
            `id` int NOT NULL AUTO_INCREMENT,
            `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
            `entities_id` int NOT NULL DEFAULT '0',
            `is_recursive` tinyint NOT NULL DEFAULT '0',
        ...
        if (!$DB->tableExists("ntas_{$itiltype}templatehiddenfields")) {
            $query = "CREATE TABLE `ntas_{$itiltype}templatehiddenfields` (
            `id` int NOT NULL AUTO_INCREMENT,
        ...
        if (!$DB->tableExists("ntas_{$itiltype}templatemandatoryfields")) {
            $query = "CREATE TABLE `ntas_{$itiltype}templatemandatoryfields` (
            `id` int NOT NULL AUTO_INCREMENT,
            `{$itiltype}templates_id` int NOT NULL DEFAULT '0',
            `num` int NOT NULL DEFAULT '0',
            PRIMARY KEY (`id`),
            ...
            $migration->addPostQuery(
                $DB->buildInsert(
                    "ntas_{$itiltype}templatemandatoryfields",
                    [
                        'id'                       => 1,
        ...
        if (!$DB->tableExists("ntas_{$itiltype}templatepredefinedfields")) {
            $query = "CREATE TABLE `ntas_{$itiltype}templatepredefinedfields` (
            `id` int NOT NULL AUTO_INCREMENT,
            `{$itiltype}templates_id` int NOT NULL DEFAULT '0',
            `num` int NOT NULL DEFAULT '0',
            ...
            $migration->dropKey("ntas_{$itiltype}templatepredefinedfields", 'unicity');
        }
    }
    /** /ITIL templates */

    /** add templates for followups */
    if (!$DB->tableExists('ntas_itilfollowuptemplates')) {
        $query = "CREATE TABLE `ntas_itilfollowuptemplates` (
         `id`              INT NOT NULL AUTO_INCREMENT,
         `date_creation`   TIMESTAMP NULL DEFAULT NULL,
         `date_mod`        TIMESTAMP NULL DEFAULT NULL,
         `entities_id`     INT NOT NULL DEFAULT '0',
    ...
    if (!$DB->fieldExists('ntas_documents_items', 'date_creation')) {
        $migration->addField('ntas_documents_items', 'date_creation', 'timestamp', ['null' => true]);
        $migration->addPostQuery(
            $DB->buildUpdate(
        ...
        $migration->addKey('ntas_documents_items', 'date_creation');
    }
    /** /Add "date_creation" field on document_items */
    ...
    foreach ($elements_to_fix as $data) {
        ...
        $DB->update('ntas_dcrooms', $data, ['id' => $data['id']]);
    }
    /** /Make datacenter pictures path relative */

    /** ITIL templates */
    if (!$DB->fieldExists('ntas_itilcategories', 'changetemplates_id')) {
        $migration->addField("ntas_itilcategories", "changetemplates_id", "integer", [
            'after'  => "tickettemplates_id_demand",
            'value'  => 0,
        ]);
    }
    if (!$DB->fieldExists('ntas_itilcategories', 'problemtemplates_id')) {
        $migration->addField("ntas_itilcategories", "problemtemplates_id", "integer", [
            'after'  => "changetemplates_id",
            'value'  => 0,
        ]);
    }

    $migration->addKey('ntas_itilcategories', 'changetemplates_id');
    $migration->addKey('ntas_itilcategories', 'problemtemplates_id');
    $migration->addKey('ntas_tickettemplatehiddenfields', 'tickettemplates_id');
    $migration->addKey('ntas_tickettemplatemandatoryfields', 'tickettemplates_id');
    $migration->addKey('ntas_tickettemplatepredefinedfields', 'tickettemplates_id');
    /** /ITiL templates */

    /** /Add Externals events for planning */
    if (!$DB->tableExists('ntas_planningexternalevents')) {
        $query = "CREATE TABLE `ntas_planningexternalevents` (
         `id` int NOT NULL AUTO_INCREMENT,
    ...
    if (!$DB->fieldExists('ntas_planningexternalevents', 'planningexternaleventtemplates_id')) {
        $migration->addField('ntas_planningexternalevents', 'planningexternaleventtemplates_id', 'int', [
            'after' => 'id',
        ]);
        $migration->addKey('ntas_planningexternalevents', 'planningexternaleventtemplates_id');
    }
    if (!$DB->fieldExists('ntas_planningexternalevents', 'is_recursive')) {
        $migration->addField('ntas_planningexternalevents', 'is_recursive', 'bool', [
            'after' => 'entities_id',
            'value' => 1,
        ]);
        $migration->addKey('ntas_planningexternalevents', 'is_recursive');
    }
    if (!$DB->fieldExists('ntas_planningexternalevents', 'users_id_guests')) {
        $migration->addField('ntas_planningexternalevents', 'users_id_guests', 'text', [
            'after' => 'users_id',
        ]);
    }

    if (!$DB->tableExists('ntas_planningeventcategories')) {
        $query = "CREATE TABLE `ntas_planningeventcategories` (
         `id` int NOT NULL AUTO_INCREMENT,
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
    ...
    if (!$DB->fieldExists('ntas_planningeventcategories', 'color')) {
        $migration->addField("ntas_planningeventcategories", "color", "string", [
            'after'  => "name",
        ]);
    }

    if (!$DB->tableExists('ntas_planningexternaleventtemplates')) {
        $query = "CREATE TABLE `ntas_planningexternaleventtemplates` (
         `id` int NOT NULL AUTO_INCREMENT,
         `entities_id` int NOT NULL DEFAULT '0',
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
         `text` text COLLATE utf8_unicode_ci,
    ...
    if (!$DB->fieldExists('ntas_entities', 'autopurge_delay')) {
        $migration->addField("ntas_entities", "autopurge_delay", "integer", [
    ...
    if (!$DB->fieldExists("ntas_projects", "auto_percent_done")) {
        $migration->addField("ntas_projects", "auto_percent_done", "bool", [
            'after'  => "percent_done",
        ]);
    }
    if (!$DB->fieldExists("ntas_projecttasks", "auto_percent_done")) {
        $migration->addField("ntas_projecttasks", "auto_percent_done", "bool", [
            'after'  => "percent_done",
        ]);
    }
    /** /Add "code" field on ntas_itilcategories */
    if (!$DB->fieldExists("ntas_itilcategories", "code")) {
        $migration->addField("ntas_itilcategories", "code", "string", [
            'after'  => "groups_id",
        ]);
    }
    /** /Add "code" field on ntas_itilcategories */
    ...
    if (!$DB->fieldExists('ntas_softwarelicenses', 'allow_overquota')) {
        if ($migration->addField('ntas_softwarelicenses', 'allow_overquota', 'bool')) {
            $migration->addKey('ntas_softwarelicenses', 'allow_overquota');
        }
    }
    ...
    if (!$DB->tableExists('ntas_items_softwareversions')) {
        $migration->renameTable('ntas_computers_softwareversions', 'ntas_items_softwareversions');
        ...
        $migration->changeField('ntas_items_softwareversions', 'is_deleted_computer', 'is_deleted_item', 'bool');
        $migration->changeField('ntas_items_softwareversions', 'is_template_computer', 'is_template_item', 'bool');
        $migration->addKey('ntas_items_softwareversions', 'itemtype');
        $migration->dropKey('ntas_items_softwareversions', 'computers_id');
        $migration->addKey('ntas_items_softwareversions', 'items_id', 'items_id');
        $migration->addKey('ntas_items_softwareversions', [
            'itemtype',
            'items_id',
        ], 'item');
        $migration->dropKey('ntas_items_softwareversions', 'unicity');
        $migration->migrationOneTable('ntas_items_softwareversions');
        $migration->addKey('ntas_items_softwareversions', [
            'itemtype',
            'items_id',
            'softwareversions_id',
    ...
    if (!$DB->tableExists('ntas_items_softwarelicenses')) {
        $migration->renameTable('ntas_computers_softwarelicenses', 'ntas_items_softwarelicenses');
        $migration->changeField(
        ...
        $migration->addKey('ntas_items_softwarelicenses', 'itemtype');
        $migration->dropKey('ntas_items_softwarelicenses', 'computers_id');
        $migration->addKey('ntas_items_softwarelicenses', 'items_id', 'items_id');
        $migration->addKey('ntas_items_softwarelicenses', [
            'itemtype',
            'items_id',
        ], 'item');
    }
    ...
    if (!$DB->fieldExists('ntas_tickettasks', 'sourceitems_id')) {
        if ($migration->addField('ntas_tickettasks', 'sourceitems_id', "int NOT NULL DEFAULT '0'")) {
            $migration->addKey('ntas_tickettasks', 'sourceitems_id');
        }
    }
    /** /Add source item id to TicketTask. Used by tasks created by merging tickets */
    ...
    // Impact dependencies
    if (!$DB->tableExists('ntas_impactrelations')) {
        $query = "CREATE TABLE `ntas_impactrelations` (
         `id` INT NOT NULL AUTO_INCREMENT,
    ...
    // Impact compounds
    if (!$DB->tableExists('ntas_impactcompounds')) {
        $query = "CREATE TABLE `ntas_impactcompounds` (
            `id` INT NOT NULL AUTO_INCREMENT,
    ...
    // Impact parents
    if (!$DB->tableExists('ntas_impactitems')) {
        $query = "CREATE TABLE `ntas_impactitems` (
            `id` INT NOT NULL AUTO_INCREMENT,
    ...
    $migration->addKey('ntas_entities', 'tickettemplates_id');
    if (!$DB->fieldExists('ntas_entities', 'changetemplates_id')) {
        $migration->addField(
            'ntas_entities',
            'changetemplates_id',
            'integer',
    ...
    $migration->addKey('ntas_entities', 'changetemplates_id');
    if (!$DB->fieldExists('ntas_entities', 'problemtemplates_id')) {
        $migration->addField(
            'ntas_entities',
            'problemtemplates_id',
            'integer',
    ...
    $migration->addKey('ntas_entities', 'problemtemplates_id');

    $migration->addKey('ntas_profiles', 'tickettemplates_id');
    if (!$DB->fieldExists('ntas_profiles', 'changetemplates_id')) {
        $migration->addField(
            'ntas_profiles',
            'changetemplates_id',
            'integer',
    ...
    $migration->addKey('ntas_profiles', 'changetemplates_id');
    if (!$DB->fieldExists('ntas_profiles', 'problemtemplates_id')) {
        $migration->addField(
            'ntas_profiles',
            'problemtemplates_id',
            'integer',
    ...
    $migration->addKey('ntas_profiles', 'problemtemplates_id');
    /** /Default template configuration for changes and problems */

    /** Add Apple File System (All Apple devices since 2017) */
    if (countElementsInTable('ntas_filesystems', ['name' => 'APFS']) === 0) {
        $DB->insert('ntas_filesystems', [
            'name'   => 'APFS',
        ]);
    }
    /** /Add Apple File System (All Apple devices since 2017) */
    ...
    /** Fix indexes */
    $migration->dropKey('ntas_tickettemplatepredefinedfields', 'tickettemplates_id_id_num');
    /** /Fix indexes */

    /** Kanban */
    if (!$DB->tableExists('ntas_items_kanbans')) {
        $query = "CREATE TABLE `ntas_items_kanbans` (
         `id` int NOT NULL AUTO_INCREMENT,
    ...
    if (!$DB->fieldExists('ntas_users', 'refresh_views')) {
        $migration->changeField('ntas_users', 'refresh_ticket_list', 'refresh_views', 'int DEFAULT NULL');
    }
    $migration->addPostQuery(
        $DB->buildUpdate(
            'ntas_configs',
    ...
    /** Add ntas_vobjects table for CalDAV server */
    if (!$DB->tableExists('ntas_vobjects')) {
        $query = "CREATE TABLE `ntas_vobjects` (
            `id` INT NOT NULL AUTO_INCREMENT,
            `itemtype` varchar(100) COLLATE utf8_unicode_ci DEFAULT NULL,
            `items_id` int NOT NULL DEFAULT '0',
    ...
    $migration->addField("ntas_entities", "latitude", "string");
    $migration->addField("ntas_entities", "longitude", "string");
    $migration->addField("ntas_entities", "altitude", "string");
    /** Add geolocation to entity */
    ...
    if (!$DB->tableExists('ntas_dashboards_dashboards')) {
        $query = "CREATE TABLE `ntas_dashboards_dashboards` (
         `id` int NOT NULL AUTO_INCREMENT,
    ...
    if (!$DB->tableExists('ntas_dashboards_items')) {
        $query = "CREATE TABLE `ntas_dashboards_items` (
        `id` int NOT NULL AUTO_INCREMENT,
    ...
    if (!$DB->tableExists('ntas_dashboards_rights')) {
        $query = "CREATE TABLE `ntas_dashboards_rights` (
         `id` int NOT NULL AUTO_INCREMENT,
         `dashboards_dashboards_id` int NOT NULL,
         `itemtype` varchar(100) COLLATE utf8_unicode_ci NOT NULL,
    ...
    if (!$DB->fieldExists('ntas_users', 'default_dashboard_central')) {
        $migration->addField("ntas_users", "default_dashboard_central", "varchar(100) DEFAULT NULL");
    }
    if (!$DB->fieldExists('ntas_users', 'default_dashboard_assets')) {
        $migration->addField("ntas_users", "default_dashboard_assets", "varchar(100) DEFAULT NULL");
    }
    if (!$DB->fieldExists('ntas_users', 'default_dashboard_helpdesk')) {
        $migration->addField("ntas_users", "default_dashboard_helpdesk", "varchar(100) DEFAULT NULL");
    }
    if (!$DB->fieldExists('ntas_users', 'default_dashboard_mini_ticket')) {
        $migration->addField("ntas_users", "default_dashboard_mini_ticket", "varchar(100) DEFAULT NULL");
    }

    // default dashboards
    if (countElementsInTable("ntas_dashboards_dashboards") === 0) {
        ...
        foreach ($dashboards_data as $default_dashboard) {
            ...
            $DB->insert('ntas_dashboards_dashboards', $default_dashboard);
    ...
    /** Domains */
    if (!$DB->tableExists('ntas_domaintypes')) {
        $query = "CREATE TABLE `ntas_domaintypes` (
            `id` int NOT NULL        AUTO_INCREMENT,
            `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
            `entities_id` int NOT NULL        DEFAULT '0',
            `is_recursive` tinyint NOT NULL DEFAULT '0',
    ...
    foreach ($dfields as $dfield => $dtype) {
        if (!$DB->fieldExists('ntas_domains', $dfield)) {
            $options = ['after' => $after];
            $migration->addField("ntas_domains", $dfield, $dtype, $options);
        }
        $after = $dfield;
    }
    ...
    foreach ($dindex as $didx) {
        $migration->addKey('ntas_domains', $didx);
    }

    if (!$DB->tableExists('ntas_domains_items')) {
        $query = "CREATE TABLE `ntas_domains_items` (
            `id` int NOT NULL AUTO_INCREMENT,
    ...
    foreach (['Computer', 'NetworkEquipment', 'Printer'] as $itemtype) {
        if ($DB->fieldExists($itemtype::getTable(), 'domains_id')) {
            ...
            if (count($iterator)) {
                ...
                $migration->migrationOneTable('ntas_domains_items');
                foreach ($iterator as $row) {
                    $DB->insert("ntas_domains_items", [
    ...
    if (!$DB->fieldExists('ntas_entities', 'use_domains_alert')) {
        $migration->addField("ntas_entities", "use_domains_alert", "integer", [
            'after'  => "use_reservations_alert",
            'value'  => -2,
        ]);
    }

    if (!$DB->fieldExists('ntas_entities', 'send_domains_alert_close_expiries_delay')) {
        $migration->addField("ntas_entities", "send_domains_alert_close_expiries_delay", "integer", [
            'after'  => "use_domains_alert",
            'value'  => -2,
        ]);
    }

    if (!$DB->fieldExists('ntas_entities', 'send_domains_alert_expired_delay')) {
        $migration->addField("ntas_entities", "send_domains_alert_expired_delay", "integer", [
    ...
    /** Domains relations */
    if (!$DB->tableExists('ntas_domainrelations')) {
        $query = "CREATE TABLE `ntas_domainrelations` (
            `id` int NOT NULL        AUTO_INCREMENT,
            `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
            `entities_id` int NOT NULL        DEFAULT '0',
            `is_recursive` tinyint NOT NULL DEFAULT '0',
    ...
    if (!$DB->fieldExists('ntas_domains_items', 'domainrelations_id')) {
        $migration->addField('ntas_domains_items', 'domainrelations_id', 'integer');
        $migration->addKey('ntas_domains_items', 'domainrelations_id');
    }
    /** /Domains relations */

    /** Domain records */
    if (!$DB->tableExists('ntas_domainrecordtypes')) {
        $query = "CREATE TABLE `ntas_domainrecordtypes` (
            `id` int NOT NULL        AUTO_INCREMENT,
            `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
            `entities_id` int NOT NULL        DEFAULT '0',
            `is_recursive` tinyint NOT NULL DEFAULT '0',
    ...
    if (!$DB->tableExists('ntas_domainrecords')) {
        $query = "CREATE TABLE `ntas_domainrecords` (
            `id` int NOT NULL AUTO_INCREMENT,
            `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
    ...
    if ($DB->fieldExists('ntas_domainrecords', 'status')) {
        $migration->dropField('ntas_domainrecords', 'status');
    }

    /** /Domain records */

    /** Domains expiration notifications */
    if (countElementsInTable('ntas_notifications', ['itemtype' => 'Domain']) === 0) {
        $DB->insert(
...
HTML,
    ...
    if (!$DB->tableExists('ntas_impactcontexts')) {
        $query = "CREATE TABLE `ntas_impactcontexts` (
            `id` INT NOT NULL AUTO_INCREMENT,
        ...
        // Update ntas_impactitems
        $migration->dropField("ntas_impactitems", "zoom");
        $migration->dropField("ntas_impactitems", "pan_x");
        $migration->dropField("ntas_impactitems", "pan_y");
        $migration->dropField("ntas_impactitems", "impact_color");
        $migration->dropField("ntas_impactitems", "depends_color");
        $migration->dropField("ntas_impactitems", "impact_and_depends_color");
        $migration->dropField("ntas_impactitems", "position_x");
        $migration->dropField("ntas_impactitems", "position_y");
        $migration->dropField("ntas_impactitems", "show_depends");
        $migration->dropField("ntas_impactitems", "show_impact");
        $migration->dropField("ntas_impactitems", "max_depth");
        $migration->addField("ntas_impactitems", "impactcontexts_id", "integer");
        $migration->addField("ntas_impactitems", "is_slave", "bool", ['value' => 1]);
        $migration->addKey("ntas_impactitems", "impactcontexts_id", "impactcontexts_id");
    }
    /** /Impact context */
    ...
    $migration->dropKey('ntas_documents_items', 'unicity');
    ...
    $migration->addField("ntas_mailcollectors", "add_cc_to_observer", "boolean");
    /** /add new option to mailcollector */
    ...
    if (!$DB->fieldExists('ntas_users', 'password_last_update')) {
        $migration->addField(
            'ntas_users',
...
HTML,
    ...
    if ($rule->getFromDBByCrit($prev_rule)) {
        ...
        if (
            ...
            $DB->delete('ntas_rulecriterias', ['rules_id' => $rule->fields['id']]);
            $DB->insert(
                'ntas_rulecriterias',
                [
                    'rules_id'  => $rule->fields['id'],
    ...
    if (!$DB->tableExists('ntas_passivedcequipments')) {
        $query = "CREATE TABLE `ntas_passivedcequipments` (
         `id` int NOT NULL AUTO_INCREMENT,
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
         `entities_id` int NOT NULL DEFAULT '0',
         `is_recursive` tinyint NOT NULL DEFAULT '0',
    ...
    if (!$DB->tableExists('ntas_passivedcequipmentmodels')) {
        $query = "CREATE TABLE `ntas_passivedcequipmentmodels` (
         `id` int NOT NULL AUTO_INCREMENT,
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
         `comment` text COLLATE utf8_unicode_ci,
    ...
    if (!$DB->tableExists('ntas_passivedcequipmenttypes')) {
        $query = "CREATE TABLE `ntas_passivedcequipmenttypes` (
         `id` int NOT NULL AUTO_INCREMENT,
         `name` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
         `comment` text COLLATE utf8_unicode_ci,
         `date_mod` timestamp NULL DEFAULT NULL,
    ...
    if (!$DB->fieldExists('ntas_states', 'is_visible_passivedcequipment')) {
        $migration->addField('ntas_states', 'is_visible_passivedcequipment', 'bool', [
            'value' => 1,
            'after' => 'is_visible_rack',
        ]);
        $migration->addKey('ntas_states', 'is_visible_passivedcequipment');
    }
    /** /Passive Datacenter equipments */

    if (!$DB->fieldExists('ntas_profiles', 'managed_domainrecordtypes')) {
        $migration->addField(
            'ntas_profiles',
    ...
    if (!$DB->fieldExists("ntas_entities", "anonymize_support_agents")) {
        $migration->addField(
            "ntas_entities",
    ...
    if (!$DB->tableExists('ntas_remindertranslations')) {
        $query = "CREATE TABLE `ntas_remindertranslations` (
                 `id` int NOT NULL AUTO_INCREMENT,
    ...
    if (!$DB->fieldExists('ntas_states', 'is_visible_contract')) {
        $migration->addField('ntas_states', 'is_visible_contract', 'bool', [
            'value' => 1,
            'after' => 'is_visible_cluster',
        ]);
        $migration->addKey('ntas_states', 'is_visible_contract');
    }

    if (!$DB->fieldExists('ntas_contracts', 'states_id')) {
        $migration->addField('ntas_contracts', 'states_id', 'int', [
            'value' => 0,
            'after' => 'is_template',
        ]);
        $migration->addKey('ntas_contracts', 'states_id');
    }
